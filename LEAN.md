...
**Rounds E0 + E1 (landed, 2026-07-23): the glue layer.** The
compositional backbone of the assembly, both `propext`-ONLY: `glueMT`/
`glue_ok` (binary all-cross-`PO` amalgamation of valid certificates is
valid — wp48's free amalgamation at the certificate level; cross
triangles close by the `PO` row/column facts, cross edges never fire an
obligation given `MTNoPo`, discharged in context by `pofree_cl_all`)
and `glueFam`/`glueFam_ok` (the n-ary FLAT family glue over common
index types, with **`MTOkPool`**: blocks may leave `∃PO`-demands
PENDING against a tagged pool; pool realization makes the glued family
STRICTLY valid — the pending demand's realizer sits in a different
block, and the cross edge is `PO`, exactly the demanded relation).
Intended instantiation: one block per vertical/DR demand component +
...
**Round E2a′ (landed, 2026-07-23): the descending mirror.** The full
D2a–D2c + E2a toolkit for DESCENDING chains (`rho (d i) (d (i+1)) =
ppi`) — the model-side counterpart of the `up = false` kernels that
round B's `munf` already unfolds soundly. Mirrored DIRECTLY (as round
B mirrored round A): a transpose-duality functor exists in principle
(flip `rho`, swap `PP ↔ PPI` in concepts), but the assembly mixes
ascending and descending kernels over the SAME interpretation and
concept, so every statement is proved in the exact shape the block
construction consumes — no transposition layer to thread through. The
mirrored algebra: stepping down is `comp(PP,·)`, rank order reversed
`{PPI} → {PO,EQ} → {DR,PP}` (`dstabRank`); `comp(PP,DR)`/`comp(PP,PP)`
are now the absorbing cells (FORWARD forcing `dforward_forcing_dr/pp`),
`comp(PPI,PPI)` the backward one (`dbackward_absorption_ppi`);
`dexternal_stabilizes`; segment coherence swaps roles (`dseg_ppi` =
the mirror of `seg_pp`: `∀PPI` descends to the far endpoint and
re-enters through the type equality; `dseg_pp` = the mirror of
`seg_ppi`); witness selection duals (`dppi_witness_all_below`,
`ddr/dpp_witness_all_above`); `dbuildChain`; `dexternals_stabilize` +
`dsegment_select` + `dsegment_kk_*`. `seg_eq`/`seg_ex_eq` are
direction-agnostic and needed no duals. Zero `sorry`; file now ~4,643
...
design work):
  ...
  5. **The descending mirror** — LAID by E2a′ (the full dual toolkit:
     `dstabRank`/`dexternal_stabilizes`, forward-forcing/backward-
     absorption duals, `dseg_*`, descending witness selection,
     `dbuildChain`, `dsegment_select`).
