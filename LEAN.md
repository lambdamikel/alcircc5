...
- **Round D2 (remainder — the last stone).** Every satisfiable
  ∀PO-free concept admits a valid certificate within
  `K(C₀) = (1+m)²·2^(2^m)`. Landed: the model-side stabilization/
  forcing package (D2a above). Remaining: **D2b** — subformula closure
  `cl`, model types, and eventually-periodic type sequences along
  infinite chains (classical pigeonhole); **D2c** — certificate
  assembly (descriptors → kernels, stabilized witnesses, the automatic
  PO-coherence vacuity, `MultiTierOk` of the extracted certificate);
  **D2d** — the `K(C₀)` counting and a computable `FinMT` enumerator
  instantiating `codes`. Then `Decidable (Satisfiable C₀)` for
  ∀PO-free `C₀` discharges via `decidableSat_of_codes` — the project's
  first end-to-end kernel-checked decidability theorem. A useful fact proved en route in
  the explainer discussion: a constant-PO kernel interface is *always*
  frame-valid (PO inhabits every cell of its row and column), so the
  mandatory kernel never obstructs presenting finite models — the
  "escape valve" doing formal work.
