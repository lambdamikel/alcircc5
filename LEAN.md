...
- **Round D2 — the extraction (the bulk, the last stone).** Every
  satisfiable ∀PO-free concept admits a valid certificate within
  ...
  PO-coherence (vacuity of the ∀PO Safe test), and the counting; plus a
  computable `FinMT` enumerator instantiating `codes`. Then
  `Decidable (Satisfiable C₀)` for ∀PO-free `C₀` discharges via
  `decidableSat_of_codes` — the project's first end-to-end
  kernel-checked decidability theorem. A useful fact proved en route in
