#!/usr/bin/env python3
"""WP113 -- how much of the cut-leaf residue is the HARD orientation?
...
def sweep(seed, trials, usize, vert=True):
    ...
    for _ in range(trials):
        ...
        for (v, blk) in cuts:
            ...
            if u:
                with_unserved += 1
                f, h = classify(m, val, c0, nodes, v, blk)
                laps += f
                lap_yes += h
            else:
                served_in_set += 1
...
def report(label, r):
    tot, leaves, unserved, served, free, hard = r
    print(f"  {label}")
    ...
    if leaves:
        ...
        n = free + hard
        print(f"    unserved DEMANDS, by orientation  : {n}")
        if n:
            print(f"      FREE (sec.121, proved)          : {free} "
                  f"({100.0*free/n:.1f}%)")
            print(f"      RESIDUE (demand away from blocker): {hard} "
                  f"({100.0*hard/n:.1f}%)")
    return leaves
