...
def main():
    ...
    for lbl, seed, L, p in (("L=4   p=3", 20260826, 4, 3),
        ...
        ok, mx = report(lbl, sweep(seed, 1200, L, p))
        res.append((lbl, ok, mx))
        print()
    print("=" * 72)
    if not all(ok for _, ok, _ in res):
        print("  CONTROL MISSED or ceiling hit -- round counts WITHHELD.")
        return 1
    mx = [m for _, _, m in res]
