...
### `python/wp34_round19_lean_mirror.py` — WP34: the round-19 Lean-mirror harness

Round-19's normative artifact is `formal/Round19Transport.lean`; WP34 is its harness. **A**: subprocess-builds the Lean file and requires returncode 0 with **exactly one** intentional `sorry` (`uSource_eq_frame`) — the build itself is the totality check, and the witness theorems (`defect12_written_gap`, `repair_covers_cobirth`, `repair_matches_frame_on_witnessA`, `witnessB_source_is_birth_step`) are kernel-checked in the same pass. **B**: a line-by-line Python transcription of `uSource` and the operational fold `unfoldAll`, cross-checked on 200 random certificates whose slot targets range over **arbitrary born occurrences** (663 inherited-member targets — the defect-#11/#12 geometry class): 14,338 pairs, **0 mismatches** — the empirical form of the sorry'd obligation. **C**: deterministic adversarial steering control (integer keys, no salted hash): 115/120 frames non-closed. **D**: hash-seed-stable digest. Development log (the round's thesis in action): the mirror caught a padding inconsistency, the core-slot duplication (→ the born-only `Wellformed` clause), and an insufficient fuel bound — three pre-review catches of exactly the defect class reviews 11–13 kept finding. Requires the elan-installed Lean 4.31.0 (`~/.elan/bin/lean`).
