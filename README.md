...
A [tableau calculus with Tri-neighborhood blocking](https://github.com/lambdamikel/alcircc5/blob/master/tableau_ALCIRCC5.pdf) **claims full decidability** of ALCI\_RCC5 (15 pages, fourth revision). The paper proves termination, soundness, and completeness using a three-part blocking condition: (i) same concept label, (ii) same abstract triangle-type set Tri(x)=Tri(y), and (iii) same Tri-neighborhood signature TNbr(x)=TNbr(y). The fourth revision addresses GPT-5.4 Pro's concern about the termination proof (total node creation bound vs. simultaneous active node bound). Three specific technical points are flagged for further scrutiny (see "Eighth approach" below). The result is **unverified** by human experts.
...
Building on the saturation finding above, a complete tableau calculus with Tri-neighborhood blocking is presented in [**A Tableau Calculus for ALCI\_RCC5 with Tri-Neighborhood Blocking (PDF)**](https://github.com/lambdamikel/alcircc5/blob/master/tableau_ALCIRCC5.pdf) (15 pages, fourth revision).
...
**The proof structure:**
- **Termination** (Theorem 4.1): Active nodes have pairwise distinct (L, Tri, TNbr) triples, so the number of simultaneously active nodes is bounded by A(C₀). Each demand is permanently satisfied once witnessed, limiting re-expansion. In the label-stable regime, Tri/TNbr grow monotonically, bounding blocking/unblocking cycles per node. By König's lemma (bounded branching + bounded active periods), the creation tree is finite. The pre-label-stable phase involves Tri-replacement rather than pure growth, making the precise bound more complex; this subtlety is flagged in the honest assessment.
- **Soundness** (Theorem 5.8): Model construction via tree unraveling + triangle-type-filtered disjunctive constraint network. The key lemma (Lemma 5.5) proves that the constraint network has non-empty domains after T-filtering and arc-consistency. The proof uses the disjunctive nature of the domains to break "mirror triangle" symmetries that arise from same-node copies, while all relations in the domains are type-safe (from P(G), where ∀-rules have saturated). Path-consistency then follows, and full RCC5 tractability (patchwork property) gives a globally consistent model.
...
**The mirror triangle issue and its resolution.** An earlier version of the soundness proof used a map-based assignment ρ(d₁,d₂) = E(map(d₁), map(d₂)). Computational investigation ([`intra_subtree_tclosure_check.py`](https://github.com/lambdamikel/alcircc5/blob/master/intra_subtree_tclosure_check.py)) revealed that this assignment is NOT T-closed: when two elements both map to the same node (a "same-map pair"), they both carry the same PO relation to a shared witness, creating "mirror triangles" like (τ\_A, R, τ\_A, PO, σ, PO) that never appear in T(G) — because each σ witness is PO to exactly one τ\_A node. The third revision fixes this by working with disjunctive domains throughout: the constraint network's arc-consistency step removes the problematic PO from cross-subtree edges and replaces it with DR or PPI (which are type-safe alternatives from P(G)). Computational verification confirms all domains remain non-empty for completion graphs of size 8, 10, and 12.

**Honest assessment.** The paper identifies three specific points requiring further scrutiny:
1. **Intra-subtree T-closure (resolved)**: The mirror triangle issue is now addressed by the disjunctive constraint network approach. Every relation in D₀ is type-safe (from P(G)), so the relaxation from a single map-based relation to a disjunctive domain cannot introduce Hintikka-type clashes — dormant ∀-qualifications are *less* of a concern with broader domains. Computational evidence confirms non-empty domains at multiple CG sizes. A fully formal general proof of non-emptiness would further strengthen the argument.
2. **Total node creation bound**: The fourth revision addresses a gap identified by GPT-5.4 Pro's [review](https://github.com/lambdamikel/alcircc5/blob/master/review5/response_to_tableau_ALCIRCC5.pdf): the original proof's claim "at most A(C₀)·n nodes created" conflated simultaneous active nodes with cumulative creation. The revised proof argues via permanent demand satisfaction + bounded blocking/unblocking cycles (clean in the label-stable regime where Tri grows monotonically, more complex in the pre-stable phase where label changes cause Tri-replacement). A clean closed-form bound has not yet been extracted.
