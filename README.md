...
>
> - **Theoretical layer — establishes decidability.** (a) GPT-5.4's [split-forest paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/sibling_interface_descriptors_ALCIRCC5_completed_eqsync_canonical_needpatched.pdf) proves the quotient-to-model direction fully (Thms 1.17–1.19, by LCA induction + König + canonical refinements). (b) Claude's [completeness extraction paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5.pdf) writes out the model-to-quotient direction (the "condensed" part of the split-forest paper), via the key lemma that model-realized relations are automatically relation-safe for endpoint types. Together, (a)+(b) imply ALCI\_RCC5 concept satisfiability is decidable: a concept is satisfiable iff it admits a valid finite rank-*d* quotient, and the space of such quotients is finite and effectively searchable.
>
...
**Closing the completeness gap by Claude (Opus 4.7).** GPT's split-forest paper proves the soundness direction (valid finite rank-$d$ quotient $\Rightarrow$ strong-EQ model) in full detail, but the converse (the model-to-quotient extraction) is given only as a proof sketch at the end. Claude's [completeness extraction paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5.pdf) (11 pages) writes out the full extraction: split-tree presentation $\to$ rank-$d$ state assignment $\to$ descriptor extraction $\to$ witness-menu extraction $\to$ quotient formation $\to$ validity verification. The key lemma is that every relation realized in a valid model is already relation-safe for its endpoint types (because the model satisfies all universals), so Need\_$R$-filtering is vacuous for model-extracted quotients. Combined with GPT's soundness chain (Thms 1.17–1.19), this yields a full decidability argument for ALCI\_RCC5.
