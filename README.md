...
- [`completeness_extraction_ALCIRCC5_v2.tex`](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5_v2.tex) — Claude's completeness extraction v2 paper (17 pages, May 2026, **current — closes the gap**)
...
In May 2026, **GPT-5.5** (a reasoning-engine variant of GPT) reviewed the latest split-forest semantics and Claude's first completeness extraction paper. The review identified **two structural defects** in v1 (`papers/completeness_extraction_ALCIRCC5.pdf`) that prevented it from establishing the model-to-quotient direction. GPT-5.5 then produced its own repaired proof using Büchi automata over a cycle-realization condition (`papers/gpt5.5/ALCIRCC5_coherent_generated_split_forest_decidability.pdf`); Claude subsequently produced a v2 (`papers/completeness_extraction_ALCIRCC5_v2.pdf`, 17 pages) that closes the same gap **without** ω-acceptance machinery, by reducing the cycle-realization condition to finite-graph reachability.
...
Combined with GPT-5.4's soundness chain (split-forest → unfolding → König → canonical refinements → strong-EQ model), v2 yields decidability of concept satisfiability in ALCI\_RCC5. The argument now stands on two papers:

- [`papers/trees/sibling_interface_descriptors_ALCIRCC5_completed_eqsync_canonical_needpatched.pdf`](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/sibling_interface_descriptors_ALCIRCC5_completed_eqsync_canonical_needpatched.pdf) — quotient-to-model (Thms 1.17–1.19), proven in full by GPT-5.4.
- [`papers/completeness_extraction_ALCIRCC5_v2.pdf`](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5_v2.pdf) — model-to-quotient, with the seven repairs above, by Claude.

The v1 paper (`papers/completeness_extraction_ALCIRCC5.pdf`) is **superseded** by v2 and retained for historical reference. Other downstream artefacts (overview paper, DL2026 abstract appendix) have been updated to reference v2.
