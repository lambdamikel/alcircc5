...
- **Part A** (models built by model\_verifier without tree constraints): **604/676** built models (89.3%) have cover-tree structure
- **Part B** (exhaustive enumeration of all valid small models, domain 2–4, with expanded type pool): **765/768** SAT concepts (99.6%) have at least one finite cover-tree model. **11.4M total models** enumerated, of which **8.36M** (73.3%) have cover-tree structure
- **Zero genuine counterexamples** — every SAT concept tested has a finite cover-tree model (or needs no model at tested sizes)
- The initial run found 8 apparent failures, but these were artifacts of a limited type pool (only tableau types, missing types that avoid composition+safe conflicts). After expanding the type pool to include all witness-compatible types, all 8 cases resolved.
