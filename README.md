...
- [**`certificate_checker.py`**](https://github.com/lambdamikel/alcircc5/blob/master/verification/python/certificate_checker.py) — **WP2 + WP4 + WP5: certificate-soundness fuzzer (vertical fragment).** Checks validity conditions (V1), (V3), (V4), (V5), (V9), (V10) of Definition 4.5 on a corpus of generated split-forest certificates. Exercises the three load-bearing round-2 fixes: **G1** (self-loop on $C_{up} \equiv \exists \PP.\top \sqcap \forall \PP.\exists \PP.\top$ — consistent under occurrence-level equality, contradictory under profile-port equality); **G3** ($C_{AB}$ accepted with two equality-mate occurrences having different parents, rejected when forced into a single parent); **G4** (rootless orientation, parent self-loops permitted). Output: [`certificate_checker.txt`](https://github.com/lambdamikel/alcircc5/blob/master/verification/reports/certificate_checker.txt).
...
- [**`mosaic_closure_search.py`**](https://github.com/lambdamikel/alcircc5/blob/master/verification/python/mosaic_closure_search.py) — **WP6: mosaic closure / patchwork lemma counterexample search.** Empirically verifies Lemma 4.6 (patchwork, citing Renz–Nebel 1999): support-closed mosaics — closed under (M1)–(M5) at every position and (SC1)–(SC4) at every context — yield globally consistent atomic RCC5 networks on triples. No counterexample to the patchwork lemma found on the tested mosaic families. Output: [`mosaic_closure_search.txt`](https://github.com/lambdamikel/alcircc5/blob/master/verification/reports/mosaic_closure_search.txt).
