...
| ALCI\_RCC5 (full) | PSPACE-hard | Open (no elementary bound yet) | **Decidable** (split-forest + [completeness extraction v2](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5_v2.pdf)); exact complexity open |
...
> **Status (May 2026).** Eleven approaches to ALCI\_RCC5 decidability have been attempted. The **most promising** is the [cover-tree tableau](https://github.com/lambdamikel/alcircc5/blob/master/papers/cover_tree_tableau_ALCIRCC5.pdf) (Wessel/GPT-5.4/Claude): decompose models into PPI-oriented cover trees where only {DR, PO} remain as open cross-edges (DR rigidity), making sibling constraints trivially arc-consistent. A [working implementation](https://github.com/lambdamikel/alcircc5/blob/master/src/cover_tree_tableau.py) agrees with an independent [quasimodel reasoner](https://github.com/lambdamikel/alcircc5/blob/master/src/alcircc5_reasoner.py) on **911 test concepts with zero mismatches**. Independent [model verification](https://github.com/lambdamikel/alcircc5/blob/master/src/model_verifier.py) constructs concrete models for **678/775 SAT concepts** with zero verification failures, and a [cover-tree decomposition test](https://github.com/lambdamikel/alcircc5/blob/master/src/decomposition_test.py) confirms that **89.0% of built models** have cover-tree structure and **775/775 SAT concepts (100%)** have finite cover-tree models (12.2M models enumerated), with **zero genuine counterexamples**. A [two-tier quotient paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/two_tier_quotient_ALCIRCC5.pdf) (Claude, reviewed by GPT-5.4) proves decidability for the **PO-coherent fragment**. The [completeness extraction v2 paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5_v2.pdf) (Claude, May 2026, supersedes v1 after a GPT-5.5 review found two structural defects) closes the model-to-quotient gap via witness-generated submodels, generated cover edges, support-closed mosaics, and finite-graph reachability discharge — yielding a full decidability argument for ALCI\_RCC5 without ω-acceptance machinery. ALCI\_RCC8 remains **open** (expected to extend via patchwork). Exact complexity is open. See the [conversation log](https://github.com/lambdamikel/alcircc5/blob/master/CONVERSATION.md) for the full history and the [GPT-5.5 review section](#gpt-55-review-and-v2-completeness-extraction-may-2026) for the v1 → v2 story.

> **Review verdict (April 2026, Opus 4.7, adversarial review).** An independent two-round adversarial review by Anthropic's Claude Opus 4.7 identified **twelve minimal soundness counterexamples** to the pre-fix cover-tree tableau (all of the form `∃R₁.∃R₂.A ⊓ ⨆_{R ∈ comp(R₁,R₂)} ∀R.¬A`) and a repair sketch that was adopted verbatim. After the fixes (new `check_role_path_compatibility` stage CT5 plus transitive universal propagation in `compute_safe`), Round 2 re-verified all 12 counterexamples and ran 25+ fresh targeted attacks and 300 pseudo-random depth-3/4 concepts — **0 mismatches** with the cycle-aware quasimodel reasoner. The Round-1 soundness-failure claim is **withdrawn** with respect to the current repository. *On the decidability claim itself:* the Round-1 defects were localised to the cover-tree tableau code and never touched the actual decidability argument, which rests on the v1+v2 sibling-interface pair (the quotient-to-model direction: GPT-5.4's [v1 split-forest paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/sibling_interface_descriptors_ALCIRCC5_completed_eqsync_canonical_needpatched.pdf) supplies the proofs of Thms 1.17–1.19, and Claude's [v2 sibling delta](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/sibling_interface_descriptors_ALCIRCC5_v2.pdf) replaces v1 Def 1.5 / Def 1.7 / Thm 1.20-sketch following the GPT-5.5 review and shows v1's proofs still apply) plus Claude's [completeness extraction v2 paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5_v2.pdf) (the model-to-quotient direction; v1 is superseded after the GPT-5.5 review identified two structural defects — see the [dedicated section](#gpt-55-review-and-v2-completeness-extraction-may-2026)); the review found no counterexample to the decidability theorem, no gap in the split-forest completeness argument, and no disagreement between the quasimodel reasoner and the cover-tree tableau on any post-fix instance. The **decidability claim for ALCI\_RCC5 is not refuted and, on the available evidence, plausibly stands** (no complexity bound is asserted — the PO-coherent fragment's quotient is already 2-EXPTIME, and the full-logic argument via split-forest + completeness extraction gives no tighter bound), with the caveat that random + structural testing never closes soundness proof-theoretically — a community-level proof-refereeing pass on the calculus is still warranted. See the full section: [Independent review and assessment of the decidability proof and calculus](#independent-review-and-assessment-of-the-decidability-proof-and-calculus), the paper [`review_paper/review_cover_tree_tableau.pdf`](https://github.com/lambdamikel/alcircc5/blob/master/review_paper/review_cover_tree_tableau.pdf) (11 pages), and the test harness in [`review_paper/test/`](https://github.com/lambdamikel/alcircc5/blob/master/review_paper/test/).

> **Current assessment (May 2026, Claude).** The decidability argument for ALCI\_RCC5 is now complete across four papers. Three independent layers converge on the same architecture: (1) Claude's implementation (type-set search with per-relation safe checks), (2) the sibling-interface programme — GPT-5.4's [v1 split-forest paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/sibling_interface_descriptors_ALCIRCC5_completed_eqsync_canonical_needpatched.pdf) (the semantic foundation, and the only place the proofs of Thms 1.17–1.19 physically live) plus Claude's [v2 sibling delta paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/sibling_interface_descriptors_ALCIRCC5_v2.pdf) (10 pages, May 2026, a delta over v1 that replaces Def 1.5 / Def 1.7 / Thm 1.20-sketch following the GPT-5.5 review and shows by two preservation lemmas that v1's Thm 1.17–1.19 proofs carry over verbatim), and (3) GPT-5.4's [cover-tree tableau paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/alcircc5_cover_tree_tableau_needall_patched.pdf) (operational calculus). Claude's [completeness extraction v2 paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5_v2.pdf) (27 pages, v2.1, May 2026, wholly supersedes v1 after the GPT-5.5 review) closes the remaining gap: the model-to-quotient direction, via witness-generated submodels, generated cover edges, support-closed mosaics, and finite-graph reachability discharge for transitive role eventualities. The **100% cover-tree model result** (775/775 SAT concepts, 12.2M models) provides strong empirical support. **What remains open:** the exact complexity — the PO-coherent fragment's quotient bound is doubly exponential (2-EXPTIME), not the singly exponential bound needed for EXPTIME. Closing this gap is an independent open problem. Extension to ALCI\_RCC8 (which also has patchwork) is expected to work but has not been written up. **Practical validation:** the cover-tree tableau also solves a realistic [GIS taxonomy example](#a-gis-example-automated-taxonomy-computation) from Wessel's original report7.pdf (2002/2003) — computing the complete subsumption hierarchy for 18 spatio-thematic concepts (countries, cities, rivers, lakes) connected by RCC5 spatial relations, reproducing all **21/21 expected subsumptions** including non-trivial inferences like Hamburg ⊑ GermanCity that require RCC5 composition reasoning through spatial role chains.

> **Theory vs. implementation (April 2026, Claude).** It is worth being precise about which claims are formally proven and which rest on empirical agreement, because the repository deliberately mixes both. Two layers:
>
> - **Theoretical layer — establishes decidability.** (a) The quotient-to-model direction is proven by **two files together**: GPT-5.4's [v1 split-forest paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/sibling_interface_descriptors_ALCIRCC5_completed_eqsync_canonical_needpatched.pdf) physically contains the proofs of Thms 1.17–1.19 (by LCA induction + König + canonical refinements); Claude's [v2 sibling delta paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/sibling_interface_descriptors_ALCIRCC5_v2.pdf) does *not* re-prove these theorems but instead replaces the three v1 items the GPT-5.5 review flagged (Def 1.5: parent self-loops, no ambient root; Def 1.7: PP/PPI eventualities by reachability discharge; Thm 1.20-sketch: dropped, delegated to (b)) and shows by two short preservation lemmas that v1's Thm 1.17–1.19 proofs carry over verbatim under the new definitions. So both files are part of the chain: v1 sibling supplies the proofs, v2 sibling acts as a delta that shows the proofs still apply. (b) Claude's [completeness extraction v2 paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5_v2.pdf) (May 2026, wholly supersedes v1) writes out the model-to-quotient direction via witness-generated submodels, generated cover edges, support-closed mosaics, and finite-graph reachability discharge for transitive role eventualities. Together, (a)+(b) imply ALCI\_RCC5 concept satisfiability is decidable: a concept is satisfiable iff it admits a valid finite rank-*d* quotient, and the space of such quotients is finite and effectively searchable.
>
...
**Closing the completeness gap by Claude (Opus 4.7).** GPT's split-forest paper proves the soundness direction (valid finite rank-$d$ quotient $\Rightarrow$ strong-EQ model) in full detail, but the converse (the model-to-quotient extraction) is given only as a proof sketch at the end. A first attempt at writing out the extraction — [completeness extraction v1](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5.pdf) (April 2026, 11 pages) — was reviewed by **GPT-5.5** in May 2026 and found to contain two structural defects: a Hasse construction that fails on infinite PP-chains (e.g.\ ∃PP.⊤ ⊓ ∀PP.∃PP.⊤), and a witness conflation in the C5 triple-coherence proof. Claude then produced [completeness extraction v2](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5_v2.pdf) (May 2026, 27 pages, v2.1), repairing both defects via seven concrete fixes — pass to a witness-generated submodel, generated cover edges (not Hasse covers), self-loops on the parent function (no ambient root), explicit containment orientation, support-closed mosaics (replacing raw pair tables, fixing the C5 conflation), explicit typed-EQ congruence, and finite-graph reachability discharge for transitive PP/PPI eventualities (avoiding Büchi-automaton machinery). Combined with GPT's soundness chain (Thms 1.17–1.19), this yields a full decidability argument for ALCI\_RCC5. See the [GPT-5.5 review section](#gpt-55-review-and-v2-completeness-extraction-may-2026) for the v1 → v2 audit.
...
**Completeness gap closed (v2, May 2026).** The completeness direction (model → quotient extraction) was condensed in GPT's split-forest paper. A v1 attempt by Claude — [completeness extraction v1](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5.pdf) (April 2026, 11 pages) — was reviewed in May 2026 by GPT-5.5 and found to contain two structural defects (a Hasse construction that breaks on infinite PP-chains, and a witness conflation in the C5 triple-coherence proof). Claude's [completeness extraction v2](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5_v2.pdf) (May 2026, 27 pages, v2.1) repairs both defects via seven concrete fixes (witness-generated submodel, generated cover edges, self-loops on the parent function, explicit containment orientation, support-closed mosaics, typed-EQ congruence, finite-graph reachability discharge). Combined with GPT's soundness chain, this yields decidability of ALCI\_RCC5 without ω-acceptance machinery. No complexity bound (EXPTIME or otherwise) is established. See the [GPT-5.5 review section](#gpt-55-review-and-v2-completeness-extraction-may-2026) for the v1 → v2 story and the [implementation paper (PDF)](https://github.com/lambdamikel/alcircc5/blob/master/papers/cover_tree_tableau_ALCIRCC5.pdf) for cross-validation details.

**Key files:**
- [`trees/sibling_interface_descriptors_ALCIRCC5_v2.tex`](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/sibling_interface_descriptors_ALCIRCC5_v2.tex) — Claude's sibling-interface v2 paper (10 pages, May 2026, **current — soundness side**; preserves GPT-5.4's Thms 1.17–1.19 verbatim, reinterprets v1 Def 1.5 / Def 1.7 / Thm 1.20-sketch)
- [`trees/sibling_interface_descriptors_ALCIRCC5_completed_eqsync_canonical_needpatched.tex`](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/sibling_interface_descriptors_ALCIRCC5_completed_eqsync_canonical_needpatched.tex) — GPT-5.4's v1 split-forest paper (April 2026, *superseded in part by sibling v2* — see GPT-5.5 review section)
...
- [`completeness_extraction_ALCIRCC5_v2.tex`](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5_v2.tex) — Claude's completeness extraction v2 paper (27 pages, v2.1, May 2026, **current — completeness side**)
- [`completeness_extraction_ALCIRCC5.tex`](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5.tex) — Claude's completeness extraction v1 paper (11 pages, April 2026, *superseded* — see GPT-5.5 review section)
- [`gpt5.5/ALCIRCC5_coherent_generated_split_forest_decidability.tex`](https://github.com/lambdamikel/alcircc5/blob/master/papers/gpt5.5/ALCIRCC5_coherent_generated_split_forest_decidability.tex) — GPT-5.5's automata-flavored repaired proof (alternative route, uses Büchi acceptance)
...
The reviewer explicitly *did not* refute the decidability theorem itself — which rests on GPT-5.4's split-forest paper plus Claude's completeness extraction paper — only the soundness of the committed cover-tree code.
...
- [`papers/completeness_extraction_ALCIRCC5_v2.pdf`](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5_v2.pdf) (27 pages, v2.1) — closes the model-to-quotient gap **without** ω-acceptance machinery, by reducing GPT-5.5's cycle-realization condition to finite-graph reachability.
...
- [`papers/completeness_extraction_ALCIRCC5_v2.pdf`](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5_v2.pdf) — **v2 completeness extraction** (Claude, May 2026, 27 pages, v2.1). The model→quotient direction, with the seven repairs above. **Wholly supersedes** the v1 completeness extraction.
