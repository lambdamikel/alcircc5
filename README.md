...
>
> - **Theoretical layer — establishes decidability.** (a) The quotient-to-model direction is proven fully by GPT-5.4's [v1 split-forest paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/sibling_interface_descriptors_ALCIRCC5_completed_eqsync_canonical_needpatched.pdf) (Thms 1.17–1.19, by LCA induction + König + canonical refinements); Claude's [v2 sibling paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/sibling_interface_descriptors_ALCIRCC5_v2.pdf) preserves these theorems verbatim and reinterprets v1 Def 1.5 / Def 1.7 / Thm 1.20-sketch following the GPT-5.5 review (parent self-loops permitted; PP/PPI eventualities by reachability discharge; Thm 1.20-sketch delegated to (b)). (b) Claude's [completeness extraction v2 paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5_v2.pdf) (May 2026, supersedes v1) writes out the model-to-quotient direction via witness-generated submodels, generated cover edges, support-closed mosaics, and finite-graph reachability discharge for transitive role eventualities. Together, (a)+(b) imply ALCI\_RCC5 concept satisfiability is decidable: a concept is satisfiable iff it admits a valid finite rank-*d* quotient, and the space of such quotients is finite and effectively searchable.
>
...
Combined with GPT-5.4's soundness chain (split-forest → unfolding → König → canonical refinements → strong-EQ model), v2 yields decidability of concept satisfiability in ALCI\_RCC5. The argument now stands on two repaired v2 papers:

- [`papers/trees/sibling_interface_descriptors_ALCIRCC5_v2.pdf`](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/sibling_interface_descriptors_ALCIRCC5_v2.pdf) — soundness (quotient → model, Thms 1.17–1.19) preserved verbatim from GPT-5.4's v1; v1 Def 1.5 reinterpreted (parent self-loops, no ambient root); v1 Def 1.7 amended (PP/PPI existentials by reachability discharge instead of witness-menu slots); v1 Thm 1.20 sketch dropped and delegated to the v2 completeness extraction below. By Claude (May 2026, 10 pages).
- [`papers/completeness_extraction_ALCIRCC5_v2.pdf`](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5_v2.pdf) — completeness (model → quotient), with the seven repairs above. By Claude (May 2026, 17 pages).

Both v1 papers are **superseded** by their v2 counterparts and retained for historical reference:

- v1 sibling-interface paper: [`papers/trees/sibling_interface_descriptors_ALCIRCC5_completed_eqsync_canonical_needpatched.pdf`](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/sibling_interface_descriptors_ALCIRCC5_completed_eqsync_canonical_needpatched.pdf) — by GPT-5.4 (April 2026). Thms 1.17–1.19 (soundness) are correct; Def 1.5 (split-tree presentation), Def 1.7 ($\Wit_k$ for $\PP/\PPI$), and the Thm 1.20 proof sketch are the parts the v2 sibling paper repairs.
- v1 completeness extraction: [`papers/completeness_extraction_ALCIRCC5.pdf`](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5.pdf) — by Claude (April 2026). Inherited the same defects from v1 sibling Def 1.5; replaced by v2 completeness extraction.

Other downstream artefacts (overview paper, DL2026 abstract appendix) have been updated to reference both v2 papers.
