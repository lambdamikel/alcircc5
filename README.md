...
| ALCI\_RCC5 (PO-coherent fragment) | PSPACE-hard | 2-EXPTIME (doubly exponential quotient) | **Decidable** ([two-tier quotient paper](https://github.com/lambdamikel/alcircc5/blob/master/two_tier_quotient_ALCIRCC5.pdf)); exact complexity open |
| ALCI\_RCC5 (full) | PSPACE-hard | Open (no complete proof yet) | **Open** — 100% empirical evidence for cover-tree hypothesis |
...
> **Status (April 2026).** Eleven approaches to ALCI\_RCC5 decidability have been attempted. The **most promising** is the [cover-tree tableau](https://github.com/lambdamikel/alcircc5/blob/master/cover_tree_tableau_ALCIRCC5.pdf) (Wessel/GPT-5.4/Claude): decompose models into PPI-oriented cover trees where only {DR, PO} remain as open cross-edges (DR rigidity), making sibling constraints trivially arc-consistent. A [working implementation](https://github.com/lambdamikel/alcircc5/blob/master/cover_tree_tableau.py) agrees with an independent [quasimodel reasoner](https://github.com/lambdamikel/alcircc5/blob/master/alcircc5_reasoner.py) on **911 test concepts with zero mismatches**. Independent [model verification](https://github.com/lambdamikel/alcircc5/blob/master/model_verifier.py) constructs concrete models for **678/775 SAT concepts** with zero verification failures, and a [cover-tree decomposition test](https://github.com/lambdamikel/alcircc5/blob/master/decomposition_test.py) confirms that **89.0% of built models** have cover-tree structure and **775/775 SAT concepts (100%)** have finite cover-tree models (12.2M models enumerated), with **zero genuine counterexamples**. A [two-tier quotient paper](https://github.com/lambdamikel/alcircc5/blob/master/two_tier_quotient_ALCIRCC5.pdf) (Claude, reviewed by GPT-5.4) proves decidability for the **PO-coherent fragment**. The decidability of full ALCI\_RCC5 and ALCI\_RCC8 remains **open**. See the [conversation log](https://github.com/lambdamikel/alcircc5/blob/master/CONVERSATION.md) for the full history.

> **Current assessment (April 2026, Claude).** The computational evidence is the strongest it has ever been. Three independent layers converge on the same architecture: (1) Claude's implementation (type-set search with per-relation safe checks), (2) GPT-5.4's [split-forest paper](https://github.com/lambdamikel/alcircc5/blob/master/trees/sibling_interface_descriptors_ALCIRCC5_completed_eqsync_canonical_needpatched.pdf) (semantic foundation with full per-relation Need\_R families for all R in {DR, PO, PP, PPI}), and (3) GPT-5.4's [cover-tree tableau paper](https://github.com/lambdamikel/alcircc5/blob/master/trees/alcircc5_cover_tree_tableau_needall_patched.pdf) (operational calculus with full Need\_R signature slots). All three agree: PP/PPI form a cover tree, DR/PO are cross-edges discharged by a global side-checker, and universal constraints are filtered per-relation for all four relations. The **100% cover-tree model result** (775/775 SAT concepts across systematic triples, adversarial patterns, and random depth-3 concepts) is strong evidence that the cover-tree hypothesis holds universally. **What remains open:** the completeness direction (model to finite quotient extraction) is condensed in GPT's papers — the 100% empirical result makes it very plausible but does not replace a formal proof. Even with a complete decidability proof, the exact complexity is open: the PO-coherent fragment's quotient bound is doubly exponential (2-EXPTIME), not the singly exponential bound needed for EXPTIME. Closing this gap — or proving a tighter quotient bound for the cover-tree approach — is an independent open problem. Extension to ALCI\_RCC8 (which also has patchwork) is expected to work but has not been written up.
...
- [Split-forest paper](https://github.com/lambdamikel/alcircc5/blob/master/trees/sibling_interface_descriptors_ALCIRCC5_completed_eqsync_canonical_needpatched.pdf): Defines the three-way status partition (Core/Out/Front), rank-k descriptors with local coherence axioms, proves finite-index lemma, finite-prefix arc-consistency theorem, and constructs models via canonical refinements + Konig's lemma.
- [Cover-tree tableau paper](https://github.com/lambdamikel/alcircc5/blob/master/trees/alcircc5_cover_tree_tableau_needall_patched.pdf): Packages the split-forest approach as a hybrid tableau calculus — local tree expansion for PP/PPI eventualities, global side-checker for DR/PO, blocking via rank-d signatures.
...
**Key files:**
- [`trees/sibling_interface_descriptors_ALCIRCC5_completed_eqsync_canonical_needpatched.tex`](https://github.com/lambdamikel/alcircc5/blob/master/trees/sibling_interface_descriptors_ALCIRCC5_completed_eqsync_canonical_needpatched.tex) — GPT-5.4's split-forest paper (the semantic foundation)
- [`trees/alcircc5_cover_tree_tableau_needall_patched.tex`](https://github.com/lambdamikel/alcircc5/blob/master/trees/alcircc5_cover_tree_tableau_needall_patched.tex) — GPT-5.4's cover-tree tableau paper (the algorithmic layer)
...
- [`stress_test_cover_tree.py`](https://github.com/lambdamikel/alcircc5/blob/master/stress_test_cover_tree.py) — Cross-validation script (911 tests)
...
See [**Two-Tier Quotient (PDF)**](https://github.com/lambdamikel/alcircc5/blob/master/two_tier_quotient_ALCIRCC5.pdf) for the full paper (12 pages, fourth revision after three rounds of GPT-5.4 Pro review).
...
See [**MSO Encoding (PDF)**](https://github.com/lambdamikel/alcircc5/blob/master/MSO_encoding_ALCIRCC5.pdf) for the full paper (16 pages).
...
**2. Contextual tableau (GPT-5.4): INCOMPLETE.** Proves full soundness but reduces completeness to FW(C,N), which is **false** — C∞ = (∃PP.⊤) ⊓ (∀PP.∃PP.⊤) is a counterexample. See the [FW counterexample proof (PDF)](https://github.com/lambdamikel/alcircc5/blob/master/FW_proof_ALCIRCC5.pdf).
