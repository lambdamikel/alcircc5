...
> **Status (May 2026, post Option 4).** Eleven approaches to ALCI\_RCC5 decidability have been attempted. The **current best statement** of the decidability argument rests on **two papers**: GPT-5.4's [v1 split-forest paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/sibling_interface_descriptors_ALCIRCC5_completed_eqsync_canonical_needpatched.pdf) (soundness, Thms 1.17–1.19) and GPT-5.5's [repaired split-forest decidability proof](https://github.com/lambdamikel/alcircc5/blob/master/papers/gpt5.5_round2/repaired_split_forest_no_automata_proof.pdf) (16 pages, May 2026, no-automata route, proves **both** directions with ten finite-checkable validity conditions (V1)–(V10) and coarse bound $B(C_0) = 2^{2^{p(n)}}$). Claude's earlier [v1](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5.pdf) and [v2.1](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5_v2.pdf) completeness-extraction papers are **superseded** by the repaired proof after a GPT-5.5 round-2 review identified two structural defects in v1 and three load-bearing gaps (G1, G3, G4) in v2.1. The operational layer (algorithmic realization) is the [cover-tree tableau](https://github.com/lambdamikel/alcircc5/blob/master/papers/cover_tree_tableau_ALCIRCC5.pdf) (Wessel/GPT-5.4/Claude): a [working implementation](https://github.com/lambdamikel/alcircc5/blob/master/src/cover_tree_tableau.py) agrees with an independent [quasimodel reasoner](https://github.com/lambdamikel/alcircc5/blob/master/src/alcircc5_reasoner.py) on **911 test concepts with zero mismatches**; [model verification](https://github.com/lambdamikel/alcircc5/blob/master/src/model_verifier.py) constructs concrete models for **678/775 SAT concepts** with zero verification failures; a [decomposition test](https://github.com/lambdamikel/alcircc5/blob/master/src/decomposition_test.py) confirms **775/775 SAT concepts (100%)** have finite cover-tree models (12.2M enumerated), with **zero genuine counterexamples**. A [two-tier quotient paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/two_tier_quotient_ALCIRCC5.pdf) proves decidability for the **PO-coherent fragment** (2-EXPTIME bound). Six Python verification work packages (WP1–WP6) cross-check the repaired proof's combinatorial claims; all **PASS**, no counterexample found. ALCI\_RCC8 remains **open** (expected to extend via patchwork). Exact complexity is open. See the [conversation log](https://github.com/lambdamikel/alcircc5/blob/master/CONVERSATION.md) for full history and the [Round-2 review section](#round-2-review-and-repaired-proof-adoption-may-2026) for the v2.1 → repaired-proof transition.

> **Review verdict (April 2026, Opus 4.7, adversarial review).** An independent two-round adversarial review by Anthropic's Claude Opus 4.7 identified **twelve minimal soundness counterexamples** to the pre-fix cover-tree tableau (all of the form `∃R₁.∃R₂.A ⊓ ⨆_{R ∈ comp(R₁,R₂)} ∀R.¬A`) and a repair sketch that was adopted verbatim. After the fixes (new `check_role_path_compatibility` stage CT5 plus transitive universal propagation in `compute_safe`), Round 2 re-verified all 12 counterexamples and ran 25+ fresh targeted attacks and 300 pseudo-random depth-3/4 concepts — **0 mismatches** with the cycle-aware quasimodel reasoner. The Round-1 soundness-failure claim is **withdrawn** with respect to the current repository. *On the decidability claim itself:* the Round-1 defects were localised to the cover-tree tableau code and never touched the actual decidability argument, which now rests on GPT-5.4's [v1 split-forest paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/sibling_interface_descriptors_ALCIRCC5_completed_eqsync_canonical_needpatched.pdf) (soundness, Thms 1.17–1.19) plus GPT-5.5's [repaired split-forest decidability proof](https://github.com/lambdamikel/alcircc5/blob/master/papers/gpt5.5_round2/repaired_split_forest_no_automata_proof.pdf) (both directions, May 2026, supersedes Claude's earlier v1 and v2.1 completeness-extraction attempts — see the [Round-2 review section](#round-2-review-and-repaired-proof-adoption-may-2026)); the review found no counterexample to the decidability theorem and no disagreement between the quasimodel reasoner and the cover-tree tableau on any post-fix instance. The **decidability claim for ALCI\_RCC5 is not refuted and, on the available evidence, plausibly stands** (no complexity bound is asserted — the PO-coherent fragment's quotient is already 2-EXPTIME, and the repaired proof's coarse bound is $B(C_0) = 2^{2^{p(n)}}$), with the caveat that random + structural testing never closes soundness proof-theoretically — a community-level proof-refereeing pass on the calculus is still warranted. See the full section: [Independent review and assessment of the decidability proof and calculus](#independent-review-and-assessment-of-the-decidability-proof-and-calculus), the paper [`review_paper/review_cover_tree_tableau.pdf`](https://github.com/lambdamikel/alcircc5/blob/master/review_paper/review_cover_tree_tableau.pdf) (11 pages), and the test harness in [`review_paper/test/`](https://github.com/lambdamikel/alcircc5/blob/master/review_paper/test/).

> **Current assessment (May 2026, post Option 4, Claude).** The decidability argument for ALCI\_RCC5 rests on a **two-paper theoretical core** plus an empirically-validated operational layer. *Theoretical core (decidability):* (a) GPT-5.4's [v1 split-forest paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/sibling_interface_descriptors_ALCIRCC5_completed_eqsync_canonical_needpatched.pdf) supplies the proofs of Thms 1.17–1.19 (the quotient-to-model / soundness direction); Claude's [v2 sibling delta paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/sibling_interface_descriptors_ALCIRCC5_v2.pdf) (10 pages, May 2026) replaces v1 Def 1.5 / Def 1.7 / Thm 1.20-sketch and shows v1's proofs carry over verbatim. (b) GPT-5.5's [repaired split-forest decidability proof](https://github.com/lambdamikel/alcircc5/blob/master/papers/gpt5.5_round2/repaired_split_forest_no_automata_proof.pdf) (16 pages, May 2026, no-automata route) proves **both directions in full**, including the model-to-quotient (completeness) direction, via occurrence-level equality, mates with different parents, and request-closed cycles in place of $\omega$-acceptance. The repaired proof formulates ten finite-checkable validity conditions (V1)–(V10) and a coarse bound $B(C_0) = 2^{2^{p(n)}}$. Claude's earlier [v1](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5.pdf) and [v2.1](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5_v2.pdf) completeness-extraction papers are **superseded**. *Operational layer (algorithm):* Claude's [cover-tree tableau implementation](https://github.com/lambdamikel/alcircc5/blob/master/src/cover_tree_tableau.py) plus GPT-5.4's [cover-tree tableau paper](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/alcircc5_cover_tree_tableau_needall_patched.pdf). The **100% cover-tree model result** (775/775 SAT concepts, 12.2M models) provides strong empirical support, and six Python work packages (WP1–WP6) cross-check the repaired proof's combinatorial claims (all PASS). **What remains open:** the exact complexity — the PO-coherent fragment's quotient bound is doubly exponential (2-EXPTIME); the repaired proof's bound is doubly exponential in $|C_0|$. No EXPTIME upper bound for the full logic is asserted. Extension to ALCI\_RCC8 (which also has patchwork) is expected to work but has not been written up. **Practical validation:** the cover-tree tableau also solves a realistic [GIS taxonomy example](#a-gis-example-automated-taxonomy-computation) from Wessel's original report7.pdf (2002/2003) — computing the complete subsumption hierarchy for 18 spatio-thematic concepts (countries, cities, rivers, lakes) connected by RCC5 spatial relations, reproducing all **21/21 expected subsumptions** including non-trivial inferences like Hamburg ⊑ GermanCity that require RCC5 composition reasoning through spatial role chains.
...
> **Note (May 2026, post Option 4).** This subsection compares Claude's v2.1 completeness extraction against Wessel's whiteboard. v2.1 has since been **superseded** by GPT-5.5's repaired proof (see the [Round-2 review section](#round-2-review-and-repaired-proof-adoption-may-2026)). The "v2.1 (current)" column below reflects the v2.1 era; the actual current pillar is GPT-5.5's repaired proof, which is structurally close to v2.1 but uses **occurrence-level equality** (not port-equality), **multi-parent mate classes** (not single-parent), and **request-closed cycles** (not Mate-class reachability) — fixing v2.1's gaps G1, G3, G4. The four whiteboard ideas (PPI-tree, EQ-splitting, congruence quotient, DR rigidity) survive unchanged in the repaired proof.

Comparing v2.1 of the completeness extraction back to Wessel's original whiteboard sketch (top of this section), four passes of formalization (Wessel → GPT-5.4 → Claude v1 → GPT-5.5 review → Claude v2 → v2.1 obligation discharges) preserved the *core ideas* but rebuilt almost every *technical instantiation* of them.
...
> **Status note (post Option 4).** This section describes Claude's v2.1 completeness extraction, which was the current pillar in early May 2026 but has since been **superseded** by GPT-5.5's no-automata [repaired proof](https://github.com/lambdamikel/alcircc5/blob/master/papers/gpt5.5_round2/repaired_split_forest_no_automata_proof.pdf) after a Round-2 review found three load-bearing gaps in v2.1 (G1, G3, G4 — see the [Round-2 section below](#round-2-review-and-repaired-proof-adoption-may-2026)). The section is preserved for historical record.
...
### Net effect on the decidability argument

Combined with the v1 soundness chain (split-forest → unfolding → König → canonical refinements → strong-EQ model), the two v2 papers yield decidability of concept satisfiability in ALCI\_RCC5. The argument now stands on **three** files (v1 sibling, v2 sibling delta, v2 completeness extraction); v2 sibling is explicitly a **delta paper** and does *not* restate the v1 soundness proofs.

- [`papers/trees/sibling_interface_descriptors_ALCIRCC5_completed_eqsync_canonical_needpatched.pdf`](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/sibling_interface_descriptors_ALCIRCC5_completed_eqsync_canonical_needpatched.pdf) — **v1 sibling-interface paper** (GPT-5.4, April 2026). **Still part of the chain**: physically contains the proofs of Thms 1.17 (finite-prefix arc-consistency, §9), 1.18 (weak-EQ realisation, §11), and 1.19 (strong-EQ realisation, §11). These proofs are unchanged. **Superseded only in part**: v1 Def 1.5 (split-tree presentation, ambient-root), v1 Def 1.7 ($\Wit_k$ for $\PP/\PPI$), and v1 Thm 1.20 (proof sketch) are replaced by the v2 sibling paper.
- [`papers/trees/sibling_interface_descriptors_ALCIRCC5_v2.pdf`](https://github.com/lambdamikel/alcircc5/blob/master/papers/trees/sibling_interface_descriptors_ALCIRCC5_v2.pdf) — **v2 sibling-interface paper** (Claude, May 2026, 10 pages). A **delta paper**: it does not re-prove Thms 1.17–1.19; instead it (i) replaces v1 Def 1.5 (parent self-loops, no ambient root, generated witness-cover edges), (ii) replaces v1 Def 1.7 (PP/PPI existentials discharged by reachability over the parent graph, not witness-menu slots), (iii) shows via two short soundness-preservation lemmas that v1's proofs of Thms 1.17–1.19 carry over verbatim under the new definitions, and (iv) drops the v1 Thm 1.20 sketch and delegates the model→quotient direction to the v2 completeness extraction below.
- [`papers/completeness_extraction_ALCIRCC5_v2.pdf`](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5_v2.pdf) — **v2 completeness extraction** (Claude, May 2026, 31 pages, v2.1). The model→quotient direction, with the seven repairs above. **Wholly supersedes** the v1 completeness extraction.

So v1 sibling is **only superseded in the three places GPT-5.5 flagged** (Def 1.5, Def 1.7, Thm 1.20-sketch); the rest of v1 sibling — including the Thms 1.17–1.19 proofs — is still active. Only v1 **completeness extraction** is wholly replaced:

- v1 completeness extraction: [`papers/completeness_extraction_ALCIRCC5.pdf`](https://github.com/lambdamikel/alcircc5/blob/master/papers/completeness_extraction_ALCIRCC5.pdf) — by Claude (April 2026). Inherited the same defects from v1 sibling Def 1.5; replaced wholesale by v2 completeness extraction.

Other downstream artefacts (overview paper, DL2026 abstract appendix) have been updated to reference all three current files (v1 sibling for Thms 1.17–1.19, v2 sibling delta, v2 completeness extraction).
...
### Verification work (Claude's contribution)
...
**Current best statement:** the GPT-5.5 repaired proof. Claude's v2.1 is retained for the record but no longer claimed as a complete proof.
