...
\newcommand{\mtopmark}{\mathsf{Top}}
\newcommand{\evalmark}{\mathsf{Eval}}
...
\[
\mathcal T=(V,\preceq,\mathsf{par},\triangleleft, r_\ast, \varrho, L, \AllUp, \AllDown, \ReqUp, \ReqDown, \NeedDR, \NeedPO, \CrossDR, \CrossPO)
\]
where:
\begin{enumerate}[label=(\roman*)]
    ...
    \item $\AllUp(x),\AllDown(x),\ReqUp(x),\ReqDown(x),\NeedDR(x),\NeedPO(x),\CrossDR(x),\CrossPO(x)\subseteq \cl(C_0)$ are finite sets of obligations and requests at $x$.
\end{enumerate}
...
\begin{itemize}
    ...
    \item $C\in \NeedDR(x)$: every future $\DR$-related witness of $x$ must satisfy $C$;
    \item $C\in \NeedPO(x)$: every future $\PO$-related witness of $x$ must satisfy $C$;
...
\begin{itemize}
    ...
    \item $(\forall\DR\text{-init})$ If $\forall\DR.C\in L(x)$ and $C\notin \NeedDR(x)$, add $C$ to $\NeedDR(x)$.
    \item $(\forall\PO\text{-init})$ If $\forall\PO.C\in L(x)$ and $C\notin \NeedPO(x)$, add $C$ to $\NeedPO(x)$.
\end{itemize}

\begin{definition}[Local DR/PO type-safety slots]
For every tableau node $x$, the sets $\NeedDR(x)$ and $\NeedPO(x)$ collect the universal requirements that must be met by future open cross-witnesses of $x$:
\[
\NeedDR(x)=\{C\in\cl(C_0): \forall\DR.C\in L(x)\},
\qquad
\NeedPO(x)=\{C\in\cl(C_0): \forall\PO.C\in L(x)\}.
\]
Because both $\DR$ and $\PO$ are symmetric in RCC5, a candidate $\DR$-witness edge between tableau points $x,y$ is locally type-safe iff
\[
\NeedDR(x)\subseteq L(y)
\qquad\text{and}\qquad
\NeedDR(y)\subseteq L(x),
\]
and a candidate $\PO$-witness edge between $x,y$ is locally type-safe iff
\[
\NeedPO(x)\subseteq L(y)
\qquad\text{and}\qquad
\NeedPO(y)\subseteq L(x).
\]
These constraints are explicit signature components; they are separate from the existential request slots $\CrossDR$ and $\CrossPO$.
\end{definition}
...
No local rule creates a direct $\DR$- or $\PO$-witness as a tree edge. Instead, the sets $\CrossDR(x)$ and $\CrossPO(x)$ are discharged by the finite global side-condition checker of Section~\ref{sec:global-check}. The slots $\NeedDR(x)$ and $\NeedPO(x)$ are the dual universal side: they record what every future $\DR$- respectively $\PO$-witness of $x$ must satisfy. Intuitively, $\CrossDR/\CrossPO$ become existential witness-menu constraints on the finite quotient, while $\NeedDR/\NeedPO$ become the local type-safety side conditions that those menu entries must meet.
...
\[
\Loc(x):=\bigl(L(x),\AllUp(x),\AllDown(x),\ReqUp(x),\ReqDown(x),\NeedDR(x),\NeedPO(x),\CrossDR(x),\CrossPO(x),\mtopmark(x),\evalmark(x)\bigr),
\]
where $\mtopmark(x)$ records whether $x$ is the current ambient root and $\evalmark(x)$ records whether $x=r_\ast$.
\end{definition}
...
The components $\NeedDR,\NeedPO$ are universal \emph{expectation} slots: they summarize what every future $\DR$- or $\PO$-related witness must satisfy because of formulas already present in $L(x)$. By contrast, $\CrossDR,\CrossPO$ are existential demand slots: they say that some such witness still has to be found. The calculus keeps both kinds of information explicitly because both become part of the blocking signature.
\end{remark}
...
\begin{itemize}
    ...
    \item $\Wit_k(x)$ is a finite support-closed witness menu indicating how the obligations in $\CrossDR(x)$, $\CrossPO(x)$, $\ReqUp(x)$, and $\ReqDown(x)$ are intended to be discharged at signature level, subject to the type-safety side conditions encoded by $\NeedDR(x)$ and $\NeedPO(x)$.
\end{itemize}
...
\end{itemize}
For local packages $\Loc(x),\Loc(y)$, define the explicit cross-witness type-safety predicates
\[
\Safe_{\DR}(x,y)\iff \NeedDR(x)\subseteq L(y)\text{ and }\NeedDR(y)\subseteq L(x),
\]
\[
\Safe_{\PO}(x,y)\iff \NeedPO(x)\subseteq L(y)\text{ and }\NeedPO(y)\subseteq L(x).
\]
The conjunctions are symmetric because both $\DR$ and $\PO$ are symmetric RCC5 relations. Thus a menu entry proposing a $\DR$-witness between endpoint colors $p,q$ is accepted only if the represented local packages satisfy $\Safe_{\DR}$; similarly for $\PO$.
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item Every proposed $\DR$-menu edge between quotient nodes $q,q'$ satisfies
    \[
    \NeedDR(q)\subseteq L(q')
    \qquad\text{and}\qquad
    \NeedDR(q')\subseteq L(q).
    \]
    \item Every proposed $\PO$-menu edge between quotient nodes $q,q'$ satisfies
    \[
    \NeedPO(q)\subseteq L(q')
    \qquad\text{and}\qquad
    \NeedPO(q')\subseteq L(q).
    \]
    \item All ordered child-pair interface descriptors are locally coherent and type-safe.
    \item The quotient satisfies the extendibility conditions needed by the canonical refinement lemmas of the split-forest note, i.e. every finite prefix network built from the quotient has a canonical extendible $\DR/\PO$ refinement.
\end{enumerate}
A saturated blocked tableau is \emph{open} if it is locally clash-free and $\mathsf{Patch}(\mathcal T)$ accepts.

\begin{remark}
The tableau is therefore hybrid in the same sense that many global-caching calculi are hybrid: local tree expansion plus a finite global consistency check on the quotient. The side-checker is not extra semantic baggage; it is precisely the finite object required by the split-forest soundness proof.
\end{remark}
...
\begin{itemize}
    ...
    \item If $C\in \CrossDR(x)$ or $C\in \CrossPO(x)$, acceptance of $\mathsf{Patch}(\mathcal T)$ guarantees a designated witness menu entry compatible both with the sibling-interface descriptors and with the universal side conditions in $\NeedDR(x)$ or $\NeedPO(x)$; the split-forest theorem realizes it by the canonical $\DR/\PO$ refinement.
\end{itemize}
...
For $\exists\DR.C$ and $\exists\PO.C$, do not try to materialize a local tree edge. Instead, record the obligation in $\CrossDR$ or $\CrossPO$ and extract the appropriate support-closed witness-menu entry from the finite quotient of the presentation. Because the presentation is a genuine model, every extracted witness endpoint also satisfies the dual universal side conditions encoded by $\NeedDR$ and $\NeedPO$, and because the split-forest theorem provides canonical extendible refinements on all finite prefixes, the global side-checker will succeed on those extracted menus.
