...
\begin{definition}[Per-relation need families]\label{def:need-families}
For a Hintikka type $\tau\in\Tp(C_0)$ and a relation $R\in\RCC$, define
\[
\Need_R(\tau):=\{\,C\in \cl(C_0)\mid \forall R.C\in \tau\,\}.
\]
If $p$ is a rank-$k$ state for any $k$, write
\[
\Need_R(p):=\Need_R(\lambda(p)).
\]
A candidate RCC5 label $S\in\RCC$ on an edge from a state $p$ to a state $q$ is called \emph{relation-safe} if
\[
\Need_S(p)\subseteq \lambda(q)
\qquad\text{and}\qquad
\Need_{\inv(S)}(q)\subseteq \lambda(p).
\]
For $S\in\{\DR,\PO\}$ this agrees with Definition~\ref{def:type-safe-open}. The point of the full family $\Need_R$ is that witness menus may introduce explicit $\DR/\PO$ edges whose \emph{induced} pair-domains acquire candidate labels in $\{\PP,\PPI\}$ after composition with tree edges; those induced labels must also be filtered against the appropriate universal requirements.
\end{definition}
...
\begin{itemize}
    ...
    \item $\Wit_k(u)$ is a finite witness menu for the existential formulas in $\lambda(u)$, recording for each existential one designated target color and target relation domain together with the finite relation-sensitive side constraints saying that every explicit or induced candidate label $S\in\RCC$ appearing in that menu is relation-safe in the sense of Definition~\ref{def:need-families}.
\end{itemize}
...
\end{itemize}
These data must satisfy: every existential requirement in every state's local type is matched by the witness menu; every universal requirement is respected by the type-safe relation domains in the frontier descriptors; and, crucially, every explicit witness-menu edge and every induced pair-domain created from it by composition with tree edges is filtered relation-sensitively. Concretely, whenever a candidate label $S\in\RCC$ remains available between endpoint states $p$ and $q$, one must have
\[
\Need_S(p)\subseteq \lambda(q)
\qquad\text{and}\qquad
\Need_{\inv(S)}(q)\subseteq \lambda(p).
\]
Repeated state occurrences are quotiented by equality of rank-$d$ states.
\end{definition}

\begin{remark}[Why the full need family matters]\label{rem:full-need}
Although the only genuinely open frontier-frontier labels are $\DR$ and $\PO$, witness menus can still generate induced pair-domains whose candidate labels include $\PP$ or $\PPI$ after composition with tree edges. A simple example is
\[
(\forall \PO.\neg C)\sqcap(\forall \DR.\neg C)\sqcap(\forall \PP.\neg C)\sqcap(\forall \PPI.\neg C)\sqcap \exists \DR(\exists \PO.C),
\]
which is unsatisfiable: if $x\xrightarrow{\DR}y\xrightarrow{\PO}z$ with $z\models C$, then the induced domain on $(x,z)$ is obtained from $\comp(\DR,\PO)$ and every candidate label in that domain forces $z\models\neg C$. A quotient or tableau side-checker that only remembers $\forall\DR$- and $\forall\PO$-requirements would miss this contradiction. The relation-sensitive $\Need_R$ family is therefore part of the semantic validity condition.
\end{remark}
...
Every finite triple of nodes appears already in some finite prefix $P_n$, and $\Can_n$ is an atomic refinement of $N(P_n)$. Therefore every finite triple is composition-consistent under $\Can_\infty$, so the union is globally composition-consistent. Every designated existential requirement is satisfied because the unfolding was built from the witness menus. Every universal requirement is respected for two separate reasons: frontier-frontier table entries were constrained to be type-safe in Definition~\ref{def:coherence}, and all explicit or induced witness-menu domains were required in Definition~\ref{def:valid-quotient} to satisfy the full relation-sensitive conditions from Definition~\ref{def:need-families}. Hence $\Can_\infty$ turns the unfolding into a weak-$\EQ$ model.
...
\begin{remark}
The only compressed part in the final theorem is the extraction of the support-closed tables and relation-sensitive witness-menu filters from a model. That extraction is finite because the closure, the rank-$d$ color alphabet, and the open domain alphabet are all finite, and the $\Need_R$ families are read off directly from local types. The soundness direction proved above is the genuinely new global step.
\end{remark}
