...
\begin{center}
\fbox{\parbox{0.92\textwidth}{%
\textbf{Status of the proof (March 2026, revised).}\;
The decidability of $\ALCIRCC{5}$ and $\ALCIRCC{8}$ is
\textbf{not fully established} by this paper.  The proof contains a
genuine gap---the \emph{extension gap}
(Remark~\ref{rem:extension-gap})---summarized here:

\smallskip
\emph{What is proven.}\;
(1)~\textbf{Soundness}: if $C_0$ is satisfiable, a quasimodel
satisfying conditions (Q1)--(Q3) can be extracted from the model.
(2)~A terminating \EXPTIME{} type-elimination algorithm with
\textbf{no false negatives}: if $C_0$ is satisfiable, the algorithm
accepts.
(3)~By contrapositive: if the algorithm rejects, $C_0$ is definitely
unsatisfiable.

\smallskip
\emph{What is not proven.}\;
\textbf{Completeness} (quasimodel $\Rightarrow$ model): the Henkin
construction solves a disjunctive constraint network at each extension
step.  For \emph{model-derived} quasimodels, the model's own role
assignments survive the construction.  For \emph{abstract} quasimodels
satisfying (Q1)--(Q3) that do not arise from any model,
path-consistency enforcement may empty constraint domains, causing the
construction to fail.  Consequently, the algorithm may have
\textbf{false positives}: acceptance does not guarantee satisfiability.

\smallskip
A correct decision procedure requires both directions (accept
$\Leftrightarrow$ satisfiable).  We have: reject $\Rightarrow$
unsatisfiable (proven); accept $\Rightarrow$ satisfiable (\textbf{not
proven}).  The open problems from Wessel~\cite{Wessel2003} are
\textbf{narrowed but not settled}.  Closing the gap requires showing
that every abstract quasimodel yields a model, or finding an
alternative model-construction argument.  Despite the gap, the proof
machinery introduced here (quasimodel method + patchwork property,
complete-graph tableau with equality blocking) and the precise
identification of the extension gap may be useful for an eventual
resolution.}}
\end{center}
...
algebra~\cite{Renz1999}.  Both run in \EXPTIME{} and are
\emph{sound for rejection}: if $C_0$ is satisfiable, the algorithm
accepts (no false negatives).  However, the completeness direction
(quasimodel $\Rightarrow$ model) contains a gap: the Henkin
model construction may fail for abstract quasimodels not derived from
actual models.  The decidability of $\ALCIRCC{5}$ and $\ALCIRCC{8}$
therefore remains open; this paper narrows the problem to a specific
constraint-satisfaction question about RCC disjunctive networks
(the ``extension gap'').
\end{abstract}
...
quasimodel.  The \emph{only-if} direction (every quasimodel gives a
model) holds for quasimodels extracted from models but not necessarily
for arbitrary quasimodels.  For the type elimination algorithm
(Section~\ref{sec:complexity}), the soundness direction is what matters:
if $C_0$ is satisfiable, the algorithm never eliminates~$\tau_0$,
so the algorithm has \emph{no false negatives}.  The question of
whether false positives can occur (i.e., whether ``spurious''
quasimodels exist for unsatisfiable concepts) remains open; we
conjecture that the type-level conditions~(Q1)--(Q3) are in fact
sufficient, but a proof requires a more refined model construction
or a case analysis exploiting the specific algebraic properties of
...
If $C_0$ is satisfiable, the soundness proof
...
and the algorithm accepts.  Conversely, if the algorithm accepts, a
quasimodel exists; by the completeness argument
(Section~\ref{sec:completeness}) and Remark~\ref{rem:extension-gap},
a model can be constructed for quasimodels arising from
satisfiable concepts.  Since the algorithm only accepts when a
quasimodel exists, and every satisfiable concept produces such a
quasimodel, the algorithm is a correct decision procedure.
\end{proof}
...
\begin{itemize}[nosep]
  ...
  \item The type-elimination algorithm remains correct: it has no
    false negatives (by soundness alone) and runs in \EXPTIME.
