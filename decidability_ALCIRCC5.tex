...
\textbf{two genuine errors} and the \emph{extension gap}
(Remark~\ref{rem:extension-gap}):
...
of the extension gap is accurate.

\smallskip
\emph{What fails.}\;
The ``no false negatives'' claim is wrong.  The algorithm can reject
satisfiable concepts.  The decidability of $\ALCIRCC{5}$ and
$\ALCIRCC{8}$ remains \textbf{genuinely open}.}}
\end{center}
...
\textbf{Erratum (April 2026):} The type-elimination algorithm is
\textbf{unsound}---it rejects satisfiable concepts (see
Remark~\ref{rem:type-elim-bug}).  The root cause is that
condition~(Q3) is anti-monotone in the type set, causing cascade
elimination when starting from all types.
Additionally, the (Q3) soundness proof has a gap for
singleton-multiplicity types ($\EQ \notin \mathsf{DN}$).  The
...
\end{itemize}
The worst-case complexity is \EXPTIME.
\emph{Note:} The tableau's correctness is independent of the
(retracted) type-elimination algorithm; it relies on its own
soundness and completeness theorems.  However, the soundness proof
(Theorem~\ref{thm:tab-soundness}) still depends on the Henkin
construction (Theorem~\ref{thm:main}) and inherits the extension gap
(Remark~\ref{rem:extension-gap}).
\end{corollary}
