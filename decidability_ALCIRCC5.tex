...
\begin{itemize}[nosep]
  ...
  \item For $i \leq m$: $N(e_i, e_{m+1}) = S_i$ (to be determined),
    with $S_k = R$ fixed.
  \item The domain of $S_i$ is restricted to
    $\{S \in \NR \setminus \{\EQ\} \mid
    (\tau(e_i), S, \tau') \in P\}$
    --- that is, only roles that form a valid pair-type with $\tau'$.
\end{itemize}
...
\emph{pair} $(e_i, e_j)$, there \emph{exist} values $S_i^{(j)},
S_j^{(i)}$ satisfying these constraints.  Note the superscripts: the
value of $S_i$ may differ depending on which $e_j$ is the third node.
We need a \emph{single} $S_i$ for each~$i$.

The constraint network $N$ restricted to any triple $\{e_i, e_j,
e_{m+1}\}$ is consistent (by the pairwise argument above), so the
full network $N$ is path-consistent.  By the \textbf{patchwork
property} (Theorem~\ref{thm:patchwork}), a path-consistent atomic
RCC5 network is globally consistent.  Therefore, a simultaneous
assignment $S_1, \ldots, S_m$ satisfying all constraints exists.

Crucially, each $S_i$ in this global solution lies in the domain
$\{S \mid (\tau(e_i), S, \tau') \in P\}$, because this domain
restriction is part of the constraint network.  Hence condition~(a)
holds, and all pair-types are in~$P$.
\end{proof}
