...
\textbf{Status: Decidability proof (under verification).}
Concept satisfiability in $\ALCIRCC{5}$ is decidable
(\EXPTIME\ upper bound) via quadruple-type elimination.
The proof closes the ``extension gap'' from all previous approaches
by using 4-element consistency instead of 3-element consistency.
All algebraic properties are verified by exhaustive case analysis
over the RCC5 composition table (finite proof by cases).
}}
\end{center}
...
decidable with an \EXPTIME\ upper bound, settling the open
question from Wessel~\cite{Wessel2002,Wessel2003}.

The proof introduces \emph{quadruple-type elimination}, extending
the standard quasimodel framework with 4-element consistency
conditions.  The key technical contribution is the
\emph{one-point extension lemma} (Lemma~\ref{lem:extension}),
which shows that the quadruple-type condition ensures star
path-consistency, enabling the RCC5 tractability result to close
the Henkin construction's cross-edge gap.
