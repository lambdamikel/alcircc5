...
\textbf{Status: Strong computational evidence for decidability.}
The original Q4 condition has a soundness gap
(Section~\ref{sec:erratum}).  Replacing Q4 with three necessary
conditions --- \emph{disjunctive path-consistency},
\emph{sibling compatibility}, and \emph{role-path compatibility} ---
fixes soundness (all three are extractable from any model).
A stress test of 713 concepts (including adversarial patterns)
finds \textbf{zero correctness errors} and zero Henkin tree failures
across 12~hash seeds.  UNSAT answers are \textbf{provably sound};
the sibling check now correctly catches cross-role universal
contradictions like $\exists\PP.(\forall\PPI.A) \sqcap \exists\PPI.\neg A$.
...
Theorem~\ref{thm:decidability} & \textbf{Strong evidence} & See below \\
Properties 1--3 (Section~\ref{sec:computation}) & Correct & Pure algebraic facts \\
Three-check algorithm & \textbf{New} & 713 concepts, 0 errors \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{The three-check algorithm}

The working implementation replaces Q4 with three \emph{necessary}
conditions on the quasimodel type set~$T$.  Each condition is
extractable from any model of the concept, so UNSAT answers are
\textbf{provably sound}.

\medskip\noindent\textbf{Check 1: Disjunctive path-consistency.}\;
The $\Safe$ constraint network over~$T$ must be
path-consistent: for each triple of types
$(\tau_a, \tau_b, \tau_c)$, every relation
$r \in \Safe(\tau_a, \tau_b)$ must be supported through
$\tau_c$, i.e., $r \in \comp(r_1, r_2)$ for some
$r_1 \in \Safe(\tau_a, \tau_c)$, $r_2 \in \Safe(\tau_c, \tau_b)$.
By the patchwork property (Renz \& Nebel 1999), this is both
sound and complete at the type level.

\medskip\noindent\textbf{Check 2: Sibling compatibility.}\;
For each type~$\tau_i$ with demands $(R_1, D_1), \ldots, (R_k, D_k)$,
the type set must contain witness types
...
can be assigned a consistent mutual relation.  Crucially,
when two demand slots use different roles ($R_m \neq R_{m'}$),
the witnesses must be \emph{distinct elements} even if they
share the same type; the check must verify the pairwise
constraint in all such cases (same-type bypass is valid only
when $R_m = R_{m'}$, allowing a single element to serve both
demands).  The check uses arc-consistency pruning followed
by backtracking search.

\medskip\noindent\textbf{Check 3: Role-path compatibility.}\;
For each chain $g \xrightarrow{R} j \xrightarrow{S} w$ in the
quasimodel tree (grandparent--parent--child), the grandchild~$w$
must be connectable to the grandparent~$g$:
\[
  \comp(\inv(S), \inv(R)) \;\cap\; \Safe(\tau_w, \tau_g) \;\neq\; \emptyset.
\]
The check iteratively prunes invalid witnesses until a fixpoint.
This catches cross-level constraints such as
$\comp(\DR, \PPI) = \{\DR\}$ forcing a relation outside $\Safe$.

\subsection{Computational verification}

A \emph{witness plan} precomputes, for each type, the
sibling-compatible witness assignment via backtracking CSP.
The Henkin tree builder uses this plan with
\emph{future-flexibility ordering} (preferring $\PO$ over $\DR$
over $\PP/\PPI$ for cross-relations, since $\PO$ gives the
widest average composition) and backtracking over
alternative witnesses.

\medskip\noindent\textbf{Result.}\;
A comprehensive stress test of \textbf{713~concepts} across seven
categories (known SAT, known UNSAT, adversarial cross-role universals,
systematic role--atom combinations, and random concepts of depths
2--4) finds:
\begin{itemize}[nosep]
\item \textbf{Zero correctness errors}: all known-SAT and known-UNSAT
  concepts are classified correctly.
\item \textbf{Zero Henkin tree failures} for all 16~basic test concepts
  across \textbf{12~independent hash seeds}.
\item 7~remaining Henkin failures in the stress test occur at depths~2--3
  for concepts that \emph{are} satisfiable (verified by explicit
  cyclic models) but whose tree unfoldings have cross-level
  composition conflicts.  One additional failure is a builder
  over-branching issue (the concept has a tree model).
  None are reasoner soundness bugs.
\end{itemize}

\noindent\textbf{Concepts requiring cyclic models.}\;
Seven concepts are satisfiable only in cyclic (non-tree) models.
All share the structure $\exists R.\varphi \sqcap \exists\PP.\psi
\sqcap \forall S.\chi$ where the leaf type~$\tau_\ell$
(satisfying $\psi$) has $\Safe(\tau_\ell, \tau_\ell) = \{\DR\}$
due to accumulated universal constraints, while
$\comp(\PPI, \PP) = \{\PPI, \PP, \PO\}$ forces non-$\DR$
cross-level relations between leaf copies.
Since $\{\DR\} \cap \{\PPI, \PP, \PO\} = \emptyset$,
no tree model exists.  All seven have verified 3-element
cyclic models (e.g., $d \mathbin{\PO} e$, $d \mathbin{\PP} f$,
$e \mathbin{\PP} f$):
\begin{center}\small
\begin{tabular}{l}
$\exists\DR.A \sqcap \exists\PP.\neg A \sqcap \forall\PO.A$ \\
$\exists\PO.A \sqcap \exists\PP.\neg A \sqcap \forall\PO.A$ \\
$\exists\PO.A \sqcap \exists\PP.\neg B \sqcap \forall R.A$
  \quad ($R \in \{\DR, \PO, \PP, \PPI\}$; 4~concepts) \\
$\exists\PP.A \sqcap \exists\PP.\neg A \sqcap \forall\PO.A$ \\
\end{tabular}
\end{center}
...
\begin{itemize}[nosep]
\item All three checks are \emph{necessary} conditions
  (extractable from any model).
\item UNSAT answers are provably sound.
\item The one-point extension lemma is correct conditional
  on its star path-consistency hypothesis.
\item The algebraic properties are unconditionally correct.
\end{itemize}

\noindent\textbf{What is conjectured:}
\begin{itemize}[nosep]
\item The three checks are jointly \emph{sufficient}
  for the one-point extension at every tree depth.
\item Specifically: sibling compatibility ensures non-empty star
  domains for siblings; role-path compatibility ensures non-empty
  star domains for grandparents; and the patchwork property
  ensures the resulting disjunctive star network is globally
  satisfiable.
\end{itemize}

\noindent\textbf{What remains:}\; A formal proof that the
three checks imply star path-consistency for the full star
network at each extension step (including uncle and cousin
constraints).  Additionally, the stress test reveals that
some satisfiable concepts may require \emph{cyclic models}
rather than tree models: when $\Safe(\tau, \tau)$ is narrow
(e.g., $\{\DR\}$ only) and the tree-path composition forces
a different relation, tree unfolding fails even though a small
cyclic model exists.  The soundness proof should account for
this by using the patchwork property to construct finite
(possibly cyclic) models rather than relying on tree-shaped
Henkin constructions exclusively.
...
Replacing Q4 with three necessary conditions ---
\emph{disjunctive path-consistency}, \emph{sibling compatibility},
and \emph{role-path compatibility} --- fixes soundness
(provably: all three conditions are extractable from any model)
and provides strong computational evidence for completeness.

A working implementation correctly decides all test concepts in
a comprehensive stress test of 713~concepts.  The sibling
compatibility check correctly identifies cross-role universal
contradictions (e.g., $\exists\PP.(\forall\PPI.A) \sqcap \exists\PPI.\neg A$
is UNSAT because $\comp(\PPI,\PPI) = \{\PPI\}$ forces the universal
to fire).  The Henkin tree extension test passes with
\textbf{zero failures} for all basic tests across 12~hash seeds.

\medskip\noindent\textbf{Decidability status.}\;
The algorithm is a \emph{candidate decision procedure} for
$\ALCIRCC{5}$ with the following properties:
\begin{enumerate}[nosep]
\item UNSAT answers are \textbf{provably sound} (all checks
  are necessary conditions).
\item SAT answers are supported by \textbf{strong computational
  evidence} (zero counterexamples across extensive testing)
  but lack a formal sufficiency proof.
\item The algorithm runs in \EXPTIME{} (exponential in closure
  size, polynomial in the number of types).
\end{enumerate}
The decidability of $\ALCIRCC{5}$ is therefore strongly
\emph{conjectured} but not formally \emph{proved}.
The remaining gap is a formal proof that the three checks
jointly imply star path-consistency at every extension step
of the Henkin construction.
