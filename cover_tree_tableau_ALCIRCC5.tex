...
The implementation passes \textbf{902~test concepts} with zero
mismatches against an independent quasimodel-based
reasoner~\cite{ClaudeDecidability}.  Independent model verification
confirms \textbf{678/768}~SAT concepts with concrete models (zero
failures), and a decomposition test shows \textbf{89.3\%} of built
models have cover-tree structure.  The results are
...
compatibility), and report on a cross-validation of 902~test
...
Adversarial (incl.\ 7 cyclic) & 27 & 71.4 & 0 \\
Systematic $\exists R_1 \sqcap \exists R_2 \sqcap \forall R_3$
  & 512 & 3.8 & 0 \\
Random depth-2 & 200 & 0.4 & 0 \\
Random depth-3 & 100 & 3.2 & 0 \\
\midrule
\textbf{Total} & \textbf{902} & \textbf{78.9} & \textbf{0} \\
\bottomrule
\end{tabular}
\end{table}

All 902~tests produce identical results in both reasoners.
In particular:
\begin{itemize}[nosep]
...
\textbf{Result:} 678 of the 768~SAT concepts (88.3\%) have
independently verified concrete models, with \emph{zero}
verification failures.  The 90 build failures are concepts
...
Of the 675~models constructed by the model builder
(which imposes \emph{no} tree constraint), \textbf{603} (89.3\%)
...
at domain sizes 2--4, checking type assignment, composition
consistency (via backtracking), demand satisfaction, and cover-tree
structure.  Results:
\begin{itemize}[nosep]
  \item \textbf{664/768} SAT concepts (86.5\%) have at least one
    finite cover-tree model.
  \item \textbf{243,628} total models enumerated, of which
    \textbf{193,715} (79.5\%) have cover-tree structure.
  \item \textbf{8 concepts} require infinite cover-tree models
    (self-referencing $\PP$/$\PPI$ demands creating infinite
    ascending/descending chains) --- expected behavior in
    description logics, not counterexamples.
  \item \textbf{8 concepts} (4 unique patterns) have
    composition$+$safe conflicts \emph{specific to the tableau's
    type set}: $\Safe(\tau_{\text{child}}, \tau_{\text{cross}})
    = \{\PP,\PPI\}$ only, forcing multi-parenting.  These may
    have cover-tree models using alternative type sets not explored
    by the enumeration.
  \item \textbf{Zero genuine counterexamples} where cover-tree
    structure is provably impossible.
\end{itemize}

\paragraph{Three-layer evidence.}
The computational evidence for decidability is now three-layered:
(1)~logical agreement --- 902~concepts with zero mismatches between
reasoners; (2)~model construction --- 678 independently verified
concrete models; (3)~structural decomposition --- 89.3\% of those
...
constraint-checking strategies but agree on all 902~tests.
This is because:
\begin{itemize}[nosep]
...
some dimensions).  The fact that no such case arose in 902~tests
...
The implementation passes 902~test concepts with zero mismatches
against an independent quasimodel-based reasoner.  Independent model
verification confirms 678/768~SAT concepts with concrete models
(zero verification failures), and a cover-tree decomposition test
shows that 89.3\% of these models naturally exhibit cover-tree
structure (Section~\ref{sec:decomposition}).  Together, these three
