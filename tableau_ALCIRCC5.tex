...
For non-adjacent pairs, $D_0(d_1, d_2) = \{R : (\tp(d_1),
R, \tp(d_2)) \in P(\GG)\}$.  Since $\GG$ is a complete
graph (every pair of nodes has an edge), pairs of active
types always appear in $P(\GG)$.
...
Every relation in $D_0$ is type-safe: it appears in $P(\GG)$,
meaning some pair of saturated nodes in $\GG$ carries this
relation.  The $(\forall)$-rule has already propagated all
universal qualifications for this pair-type.  Replacing
$\PO$ with $\DR$ or $\PPI$ on a cross-subtree edge therefore
satisfies all $\forall$-constraints---the new relation is
\emph{less demanding}, not more, since it was already
enforced in $\GG$ for some pair of matching types.
...
\begin{enumerate}
  \item \textbf{Intra-subtree $T$-closure
    ...
    An earlier version of this proof constructed a map-based
    $T$-closed solution $\rho(d_1, d_2) = \EE(n_1, n_2)$.
    This works for cross-subtree pairs with distinct maps, but
    fails for \emph{same-map pairs}---elements $d_1 \neq d_2$
    both mapping to the same node $n$.  The failure is due to
    \emph{mirror triangles}: two copies of $n$ would both carry
    the same relation (e.g., $\PO$) to a third element, creating
    triangle types like $(\tau_A, R, \tau_A, \PO, \sigma, \PO)$
    that are structurally absent from $T(\GG)$.

    The current proof resolves this by working with disjunctive
    domains rather than constructing a single assignment.  The
    constraint network's $T$-filtering and arc-consistency steps
    break the mirror symmetry by narrowing cross-subtree domains
    (e.g., removing $\PO$ from a copy-to-witness edge and
    retaining $\DR$ or $\PPI$).  Every relation in the domain
    $D_0$ is type-safe (drawn from $P(\GG)$, where $\forall$-rules
    have already saturated), so the relaxation from a single
    map-based relation to a disjunctive domain cannot introduce
    Hintikka-type clashes.  Computational verification confirms
    all domains remain non-empty for completion graphs of size
    8, 10, and~12.  A fully formal proof that domains are
    non-empty in general would strengthen the argument.

  \item \textbf{Total node creation bound
    (Theorem~\ref{thm:termination}).}
    An earlier version of this proof claimed that
    ``with at most $A(C_0)$ active nodes, at most $A(C_0) \cdot n$
    nodes are created.''  As correctly observed by a
    reviewer~\cite{GPTReview}, this does not follow: a bound on
    \emph{simultaneous} active nodes does not, by itself, bound
    \emph{cumulative} creation, because blocking/unblocking cycles
    could in principle allow unbounded sequential node creation.

    The revised proof (Theorem~\ref{thm:termination}) addresses this
    by arguing that (1)~each demand is permanently satisfied once
    witnessed, limiting re-expansion, and (2)~each node's signature
    changes a bounded number of times, limiting blocking/unblocking
    cycles.  Part~(2) is clean in the label-stable regime (where
    $\Tri$ grows monotonically), but the pre-stable phase involves
    $\Tri$-replacement (when Hintikka types change), making the
    bound more complex.  Each node has at most $n$ label changes,
    each causing bounded $\Tri$-reconfiguration, so the per-node
    bound is finite---but we have not extracted a clean closed-form
    expression for the total creation count.
...
\newblock Review memo, April 2026.
