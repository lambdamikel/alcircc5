...
We prove soundness (the model is constructed
...
\emph{sound} (model construction via triangle-type-filtered
unraveling, with the $\Safe$ extension for same-map pairs
and global consistency from full RCC5 tractability) and
\emph{complete} (a model guides all nondeterministic choices).

However, \textbf{termination fails.}  The concept
$A \sqcap \exists\PP.A \sqcap
\forall\PP.(\exists\PP.A \sqcap \exists\DR.B)$
...
toward the decidability of $\ALCIRCC{5}$: the soundness and
completeness proofs are valid, but the calculus is not a
decision procedure.  The main contributions are:
(1)~the Tri-neighborhood blocking condition and the proof
that it ensures correct unraveling;
