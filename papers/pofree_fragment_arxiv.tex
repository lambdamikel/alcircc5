...
tree of fresh occurrences, whose part-of births generate a strict order,
whose disjointness births generate a downward-closed disjointness relation,
and in which every remaining pair overlaps. The construction rests on an
...
\begin{enumerate}
  ...
  \item A soundness proof by a \emph{fresh-occurrence} unfolding in which no
    element is reused, so that the frame conditions that defeated reuse-based
    constructions hold by construction (\S\ref{sec:soundness}).
...
\begin{itemize}
  \item \textbf{Michael Wessel} posed the problem in 2002/2003, supplied the
    split-forest picture of vertical nesting plus horizontal completion from
    which the shape of the unfolding descends, and the kernel idea from which
...
\texttt{formal/POFreeLift.lean} and \texttt{formal/RCC5NormalForm.lean}; the
Lean artifact is the one frozen at git tag
\texttt{certified-fragment-2026-08-29}, which differs from the current sources
only in the spelling of comments. The review and draft are in
