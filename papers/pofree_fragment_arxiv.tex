...
tree-shaped. What it reads off that model fits the same picture---a cone
signature is a point's type together with the types strictly below it, that
is, its vertical position, and the transition conditions push cones along $\PP$
and $\PPI$ and ask nothing structural of $\PO$---but that is a correspondence
rather than a documented design step: the cone-scheme plan does not refer to
split forests, and a type together with the types below it is a standard
invariant for logics over a transitive relation. What the earlier routes built in order to \emph{obtain} such a forest
