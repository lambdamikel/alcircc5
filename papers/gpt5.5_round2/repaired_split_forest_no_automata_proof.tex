...
Let a weak split occurrence structure satisfy the RCC5 composition table, converse, and typed relation-safety, and let $\eqc$ be an explicit typed equality congruence satisfying the analogues of (Ea)--(Ec) and containing no strict vertical comparable pair.  Then quotienting occurrences by $\eqc$ yields a strong abstract RCC5 frame.
\end{lemma}

\begin{proof}
Let $[u]$ be the $\eqc$-class of $u$.  Define
\[
  \rho([u],[v])=\lab(u,v).
\]
This is well-defined by equality congruence: replacing $u$ or $v$ by an equality mate preserves all external labels.  The quotient has $\rho([u],[v])=\EQ$ iff $[u]=[v]$, because explicit equality is exactly the relation being quotiented and no strict vertical comparable pair is equated.  Converse and the RCC5 composition table descend from the weak occurrence structure because they hold for representatives and the definition is representative-independent.
\end{proof}
...
\begin{proof}
Let $\Q$ be valid and unfold it to the occurrence forest $\U(\Q)$.  Each traversal of a blocking cycle yields fresh occurrences.  Assign to each occurrence the type of its state.

Define a weak pair labelling on occurrences.  Identical occurrences have label $\EQ$.  If two occurrences lie on one vertical branch, the lower-to-higher label is $\PP$ and the higher-to-lower label is $\PPI$.  If two occurrences are explicitly connected by equality ports, label them $\EQ$.  All other pair labels are supplied by the support-closed mosaic associated with their least generated interface.  Support closure and overlap agreement make this label independent of the chosen interface.  By Lemma~\ref{lem:patchwork}, every finite prefix satisfies converse and RCC5 composition.

Quotient by the explicit equality congruence.  Lemma~\ref{lem:eq-quotient} gives a strong abstract RCC5 frame.  Interpret every concept name $A$ by
\[
  A^\I=\{[u]:A\in\lambda(u)\}.
\]
This is well-defined because equality mates have identical types.

We prove by induction on $C\in\Cl(C_0)$ that
\[
  [u]\in C^\I \quad\Longleftrightarrow\quad C\in\lambda(u).
\]
Atomic and Boolean cases follow from the definition of $A^\I$ and closure type consistency.

For $R=\EQ$, the claim follows from strong equality and validity clause (V1).  For $R\in\{\DR,\PO\}$, existential formulas are witnessed by the concrete mosaic positions named in (V6), and universal formulas are enforced by (V7).  The induction hypothesis applies at the witness positions.

For $R\in\{\PP,\PPI\}$, suppose $\exists R.D\in\lambda(u)$.  Clause (V4) gives an equality mate state $\widehat s$ of the state of $u$ and a state $t$ reachable by a strict vertical $R$-path from $\widehat s$, with $D\in\lambda(t)$.  In the unfolding, the corresponding equality mate occurrence $\widehat u$ is in the same quotient class as $u$, and there is a strict vertical occurrence $v$ of state $t$ with relation $R$ from $\widehat u$ to $v$.  Hence $[u]=[\widehat u]$ has an $R$-successor $[v]$ satisfying $D$ by induction.

Conversely, if $[u]$ has an $R$-successor $[v]$ for $R\in\{\PP,\PPI\}$, then some equality mate of an occurrence representing $[u]$ is vertically $R$-related to an occurrence representing $[v]$.  Clause (V5) ensures that every such reachable target satisfies every required universal $D$; hence $\forall R.D$ is interpreted correctly.  If an existential is not in the type, its complement universal is in the type, so the closure-complement argument gives the reverse direction as usual.

Since $C_0\in\lambda(s_0)$, the class of the distinguished initial occurrence satisfies $C_0$.
\end{proof}
