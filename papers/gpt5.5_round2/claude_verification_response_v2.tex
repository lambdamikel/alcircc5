...
and the pair \((a, b)\) satisfies \(\rho(a, b) = \PO\).  In particular,
the pair is neither \(\PP\)-, \(\PPI\)-, \(\EQ\)-, nor \(\DR\)-related.
The exclusion of \(\DR\) is by common-root composition: since
\(d^* \PP a\) gives \(\PPI(a, d^*)\), the chain
\(a \to d^* \to b\) yields
\(\rho(a, b) \in \mathsf{comp}(\PPI, \PP) = \{\EQ, \PO, \PP, \PPI\}\),
so \(\DR\) is unavailable a priori;\@ the formula then rules out
\(\EQ\), \(\PP\), \(\PPI\), leaving \(\PO\) as the unique possibility.
...
contradiction.  Hence \(\rho(a, b) \in \{\DR, \PO\}\).  Moreover, by
the composition argument in the lemma statement,
\(\rho(a, b) \in \mathsf{comp}(\PPI, \PP) = \{\EQ, \PO, \PP, \PPI\}\),
which excludes \(\DR\).  Combining the two,
\(\rho(a, b) = \PO\).  \(\square\)
