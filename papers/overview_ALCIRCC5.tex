...
Every known undecidability proof for description logics ultimately
encodes a two-dimensional grid ($\naturals \times \naturals$ domino
tiling).  Grid encoding requires either functional roles, number
restrictions, role intersection, or role value maps.
$\ALCIRCC{5}$ has \emph{none} of these.
...
\begin{proof}
\begin{itemize}[nosep]
  \item $\EQ(x,y)$: impossible---$x$ and $y$ are distinct tree
...
\begin{tikzpicture}[
  every node/.style={draw, rounded corners=2pt, font=\scriptsize,
    minimum height=1.2em, inner sep=2pt},
...
% Row 1: Area
\node (area) at (0,-1) {Area};
\draw[arr] (top) -- (area);

% Row 2: Area's children (5 nodes, spread across ~12cm)
\node (city) at (-5,-2.2) {City};
\node (country) at (-2.5,-2.2) {Country};
\node (river) at (1,-2.2) {River};
\node (lake) at (4,-2.2) {Lake};
\node (mountain) [leaf] at (5.8,-2.2) {Mountain};
\draw[arr] (area) -- (city);
...
\node (germancity) at (-5.8,-3.5) {GermanCity};
\node (cityatriver) at (-4.0,-3.5) {CityAtRiver};
\node (germany) [leaf] at (-2.9,-3.5) {Germany};
\node (czech) [leaf] at (-1.7,-3.5) {Czech Rep.};
\node (localriver) at (-0.2,-3.5) {LocalRiver};
\node (nonlocal) at (1.5,-3.5) {NonLocalRiver};
\node (riverflowing) at (3.2,-3.5) {RiverIntoLake};
\node (alsterlake) [leaf] at (4.6,-3.5) {AlsterLake};
...
% Row 4: deeper nodes
\node (hamburg) [leaf] at (-4.9,-4.8) {Hamburg};
\node (germanriver) at (-0.2,-4.8) {GermanRiver};
\node (elbe) [leaf] at (1.5,-4.8) {Elbe};
...
\node (alster) [leaf] at (1.5,-6.1) {Alster};
