...
A week after v2.1, GPT-5.5 produced a \emph{second formal review}
of v2.1 itself and identified \textbf{eight structural gaps
(G1)--(G8)}, of which three are load-bearing: (G1)~profile-level
equality ports contradict vertical separation on self-loops via
the M1$+$M7 reflexivity chain; (G3)~the mate construction forces
all mates to share a single semantic parent, refuted by a
two-mate stress formula $C_{AB}$; (G4)~the rootless orientation
is incompatible with the depth-from-$u_*$ projection.  Alongside
the review, GPT-5.5 supplied a \emph{repaired
proof}~\cite{GPT55Repaired} that addresses all three via
(i)~\textbf{occurrence-level equality}
$u \equiv_{\EQ} v \Leftrightarrow \mathrm{orig}(u) = \mathrm{orig}(v)$
(not profile-port equality), (ii)~\textbf{mates with different
parents} via port-equivalence,
and (iii)~\textbf{request-closed cycles} in place of
$\omega$-acceptance.  The repaired proof formulates ten
finite-checkable validity conditions~(V1)--(V10) on rank-$d$
quotients and a coarse bound $B(C_0) = 2^{2^{p(n)}}$ on
certificate size.  v2.1 is therefore \textbf{marked superseded};
the GPT-5.5 repaired proof is the \textbf{current best
statement} of the decidability argument; Claude's contribution
shifts to verification of its combinatorial claims (WP1--WP6,
L1--L7).  WP1 (RCC5 composition table by exhaustive subset
enumeration) confirms the repaired proof's table matches
brute-force computation in all 25 cells.
