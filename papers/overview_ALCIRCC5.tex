...
\textit{round-7 split-forest decidability
proof}~\cite{GPT55Round7Sketch} (May~2026, 14~pages,
no-automata route), with detailed companion
\cite{GPT55Round7Expanded} (28~pages) and an independent
parity-tree-automata witness~\cite{GPT55AutomataRoute} (21~pages).
The round-7 architectural move addresses a defect identified in
the round-6 review~\cite{GPT55Round7Review}: indexing labels by
abstract position-symbol pairs $L_\Q:\Pos_\Q^2\to\Base$ with
diagonal $L_\Q(\pi,\pi)=\EQ$ collapses distinct laps of a blocking
cycle into semantic equality (the canonical one-state certificate
for $\exists\PP.\top\sqcap\forall\PP.\exists\PP.\top$ breaks
because $q(u_i)=q(u_j)=\pi$ for every $i,j$, forcing $\lab(u_i,
u_j)=\EQ$).  The defect is intrinsic to indexing by
position-symbol pairs and cannot be locally patched.

Round-7 replaces the position-symbol pair labelling by labels on
...
(tag $\eqtag$), vertical successor / predecessor pairs (tags $\up,
\down$), side-mosaic pairs, and residual sibling-frontier pairs.
...
$\ell_\Q(\alpha(u_i,u_j))\ne\EQ$ for any distinct laps; for the
one-state upward cycle the label is $\PP$, as intended.  Triple
composition is verified on a finite triple-shape set $\Tri_\Q$ via
\[
...
\]
the round-7 (V6) clause.  All of round 7 is twelve
finite-checkable validity conditions (V1)--(V12) on a finite
occurrence-sensitive certificate; both directions are proved
(soundness: a valid certificate unfolds to a strong abstract RCC5
model; completeness: every satisfiable $C_0$ admits a valid finite
certificate).  The certificate space is bounded by a coarse
$B(C_0)\le 2^{2^{p(n)}}$ for a polynomial $p$, sufficient for
decidability.

Three structural ingredients survive from earlier rounds and
remain load-bearing in round 7:
\begin{enumerate}[label=(\arabic*)]
  \item \textbf{Occurrence-sensitive equality} (round-7
    refinement of round-2's occurrence-level equality).
    Equality is generated only by explicit equality-port chains
    instantiated inside a context, never by reuse of an abstract
    position symbol in a blocking cycle.
  \item \textbf{Split copies for multiple selected proper
    superparts}.  Where an element has incomparable selected
    $\PP$-superparts (the $C_{AB}$ stress concept), the
    presentation carries one equality-mate occurrence per
    selected superpart, linked by explicit equality-port chains.
...
\medskip\noindent\textbf{Status of v1, v2.1, the round-2 31-page
proof, and the round-5 / round-6 deltas.}
Claude's v1 and v2.1 completeness-extraction papers, GPT-5.5's
round-2 31-page repaired proof~\cite{GPT55Repaired}, Claude's
round-5 (M6$'$) delta, items-4-to-8 delta, round-5 consolidation,
and round-6 internal-coherence pass are all retained in the
repository as the historical audit trail but are explicitly
\emph{marked superseded} by GPT-5.5's round-7
sketch~\cite{GPT55Round7Sketch}.  The round-2 through round-6
material is documented in
\texttt{OUTDATED.md}~\cite{ALCIRCC5Outdated}.  The round-7 sketch
is the single document that now carries the decidability claim on
the model-to-quotient side; its (V1)--(V12) validity conditions on
occurrence-sensitive pair shapes replace the
position-symbol-indexed (V1)--(V15) of the round-5 / round-6
consolidations.  The v1 sibling-interface
paper~\cite{SiblingCompleted} and its v2
delta~\cite{SiblingInterfaceV2} still supply the soundness chain
(Thms~1.17--1.19) and the cover-tree decomposition that the
round-7 sketch builds on.
...
\emph{cross-checking finite-combinatorial claims}.  Six Python work
packages (WP1--WP6) verify the round-2 31-page repaired
proof~\cite{GPT55Repaired} (now historical) and all
\textbf{PASS}: brute-force enumeration confirms GPT-5.5's RCC5
composition table in all 25 cells (WP1); a vertical-fragment
validity-condition checker accepts $C_\uparrow$ via parent
self-loops, accepts $C_{AB}$ via two mates with different parents,
rejects the v2.1-style single-parent $C_{AB}$ attempt, and rejects
two genuine UNSAT concepts (WP2); a small-model SAT/UNSAT oracle
exhibits a concrete 3-element model for $C_{AB}$ (WP3); $C_{AB}$
and request-closed cycle tests fold into WP2/WP3 (WP4, WP5); and a
mosaic enumeration confirms support-closure axioms on small
mosaics (WP6).

Three further self-contained pure-stdlib verification scripts
exercise the central round-7 stress concepts: WP7
(comparable side witnesses, $\exists\DR.(A\sqcap\exists\PP.B)\sqcap
\exists\DR.B$), WP8 (blocking-not-equality: builds a finite
truncation of the canonical $C_\uparrow$ chain and verifies that
the round-7 incidence-tagged label rule keeps distinct laps at
$\PP/\PPI$ rather than collapsing them to $\EQ$, while explicitly
demonstrating the round-6 collapsed rule), and WP9 (split copies
for $\exists\PP.A\sqcap\exists\PP.B$ with incomparable proper
superparts).  All three pass; no counterexample to a
combinatorial claim of the round-7 sketch was found.
...
\newblock A repaired split-forest decidability proof for
  ...
  tags; twelve validity clauses (V1)--(V12).  Current canonical
