...
\begin{remark}[Completeness bug in the quasimodel reasoner]
\label{rem:qm-bug}
The two examples above form a useful cross-validation pair.
During the analysis we observed that the implemented quasimodel
reasoner (\texttt{alcircc5\_reasoner.py}) \emph{incorrectly}
reports Example~\ref{ex:po-loop} ($Y$) as UNSAT, while the
cover-tree tableau correctly reports SAT. The root cause is a
role-path compatibility check that for every chain
$g \to_R j \to_S w$ requires
$\comp(\inv(S), \inv(R)) \cap \Safe(w, g) \ne \emptyset$,
treating $w$ as a fresh node in a tree unfolding. When $w$ may
equal $g$ itself (via symmetric roles), this check is too
strict---the quasimodel implementation implicitly assumes tree
models, whereas $\ALCIRCC{5}$ admits cyclic models that trees
cannot represent. The cover-tree tableau handles such cases
through back-edges in the cover tree. The concepts wrongly
rejected by the quasimodel implementation are exactly those
whose satisfiability requires a cycle through a symmetric
relation.
\end{remark}
