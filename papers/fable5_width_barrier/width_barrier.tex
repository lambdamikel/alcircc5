...
open. Closing F3 for real is a related, load-bearing construction: the
fixed non-oracular checker must decide the pipeline's hypotheses on
finite data, which needs a Boolean \code{Hintikka}-checker with an
adequacy proof (the structural \code{Hintikka} predicate quantifies over
the infinite \code{Nat}/\code{Concept} types, guarded by finite-list
membership, so it is decidable only after reformulation) --- deliberate
work, deferred to a reviewed round rather than bolted on here.
\textbf{F4} also remains. Accordingly the
