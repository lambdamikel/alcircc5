...
\title{A Fully Expanded Generated Split-Forest Decidability Proof for $\Logic$\\
       \large Round-8 repair with saturated side contexts and constructed catalogues}
...
side/tree exhaustion lemma verticalizes side/tree pairs only on the
\emph{finite, depth-bounded} boundary $B_k(u)$, but a $\DR/\PO$ side
witness can be RCC5-composition forced to be a proper part of an
...
For a selected $\DR$ or $\PO$ witness of a currently exposed occurrence, attach the saturated context $S_k(u)$ with the remaining modal-depth budget $k$.  The context contains the source's vertical boundary, the selected side witness, nested side witnesses, and all containment/equality relations forced between side positions and boundary positions.  Thus the situation
\[
   u\PP b,
   \qquad
   u\PO w,
\]
which allows $b$ and $w$ to be $\PP/\PPI$-comparable by the RCC5 composition table, is explicitly inspected inside $S_k(u)$; if $b$ and $w$ are comparable, the local vertical stratum is inserted.
...
\begin{proof}
For pairs, first test same occurrence and explicit equality.  If neither holds, test equality-aware vertical reachability.  If the pair lies in a saturated side neighbourhood or in an overlap of two such neighbourhoods, it is present in the neighbourhood or amalgam label matrix.  Lemma~\ref{lem:cross-side-tree} ensures that cross side/tree comparable pairs are handled here by a local vertical stratum, not by residual frontier.  If none of these cases applies, the pair is residual frontier after saturation, so Lemma~\ref{lem:residual-frontier} gives a $\front_R$ shape with $R\in\{\DR,\PO\}$.
...
\item \emph{Saturated side mosaics.}  For each selected $\DR/\PO$ witness, run the saturation construction of Definition~\ref{def:saturated-side-context} with the remaining modal-depth budget.  Record the resulting finite saturated neighbourhood as a mosaic.  This neighbourhood includes side/tree cross labels: if a side witness is $\PP$- or $\PPI$-comparable with a vertical boundary occurrence, the local vertical stratum is part of the mosaic.
