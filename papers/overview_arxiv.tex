...
\paragraph{Idea (the human's whiteboard).} Decompose a model into a
\emph{forest} of trees --- one per connected component --- by splitting
each join node into $\EQ$-mates, so the tree skeleton is explicit and the
(global) relational completion is handled separately by the patchwork
property. Tree-like presentations of infinite models are the classical
route to decidability (tree-model property, then automata or
type-elimination).
