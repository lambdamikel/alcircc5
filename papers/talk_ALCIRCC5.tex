...
\begin{itemize}
  ...
  \item The \alert{full} question is \alert{not settled} --- but it is
        \emph{reduced} to \alert{one well-posed lemma}, with an exact
        arithmetical address ($\Pi^0_1$) on the way.
...
\begin{itemize}
  ...
  \item \alert{20 adversarial reviews} --- a defect or overclaim found in
        \emph{all but two}
...
\textcolor{good}{\textbf{Abstract $=$ concrete}} &
  composition table $\Leftrightarrow$ real sets &
  \texttt{POFreeLift}\\
\bottomrule
\end{tabular}
\end{center}
...
\begin{block}{Theorem ($\forall\PO$-free fragment), machine-checked 2026-08-29}
  Concepts with \alert{no $\forall\PO$ subformula} are \alert{decidable}.
...
\begin{itemize}
  ...
  \item Reached \alert{three} times by three routes; the third
        (GPT-5.6~Sol's \emph{cone scheme}) is the one that certified.
  \item \alert{Decidable $\neq$ runnable} --- doubly exponential.
\end{itemize}
\end{frame}
...
\begin{block}{Lutz \& Wolter 2006 --- the one place anyone gestured at this}
  ...
  explicitly \alert{open}, and ask for ``a decidable and still useful
  variant''. Among the routes they float: ``giving up some of the
  relations\dots\ dropping $\PO$ still results in a useful formalism''.
\end{block}
\begin{itemize}
  \item But that sentence defends \alert{usefulness}, not decidability ---
        no proof, no sketch, no conjecture. The work they cite in support
        has a \emph{single} inclusion modality.
  \item Our restriction is \alert{weaker}: $\PO$ stays in the language.
        $\exists\PO$ is kept; only $\forall\PO$ goes.
  \item Wessel's own 2003 report did canvass restricted fragments --- but
        strictly stronger ones, all working by restoring the \alert{finite
        model property}, which this fragment provably lacks.
\end{itemize}
\vfill
\begin{center}
...
\begin{enumerate}
  ...
  \item \textbf{Don't trust --- verify.} Every load-bearing claim was
        cross-checked mechanically: 94 probes that re-derive the algebra
...
\textcolor{good}{\textbf{Lean-certified}} &
  \textbf{$\forall\PO$-free decidability} (raw input, NNF, concrete sets);
  abstract $=$ concrete sat.; soundness pipeline; abstraction faithfulness;
  RCC5 normal form (both dir.); F6\,$\Rightarrow$\,decidable; $\Pi^0_1$
  dovetail &
  kernel-checked, with a stated axiom footprint\\
\addlinespace
\textbf{Theorem-level} &
  the \emph{two arguments} for the fragment ($K(C_0)$ bound, extraction);
  identity-selector characterization; W2$'$ folds into F6 &
...
\end{center}
\begin{itemize}
  \item NO $\Rightarrow$ decidable, YES $\Rightarrow$ the undecidability
        route opens --- certified machinery waits on \alert{both} sides.
  \item One \alert{sub-problem off the pile}: drop $\forall\PO$ and it is
        decided, in the kernel --- three cold reviews, no counterexample.
