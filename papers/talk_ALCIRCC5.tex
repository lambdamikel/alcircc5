...
\author{Michael Wessel\\[2pt]
  {\small with AI assistants: Claude (Anthropic) and GPT-5.4/5.5 (OpenAI)}}
\date{July 2026}
...
%% ==================================================================== 5
\begin{frame}{Description logic in one slide}
Concepts = definitions built from properties and \alert{quantified relations}:
\begin{align*}
  \textsf{GermanCity} &\equiv \textsf{City} \sqcap \exists\PP.\,\textsf{Germany}
      && \text{``a city \emph{properly inside} Germany''}\\
  \textsf{CityAtRiver} &\equiv \textsf{City} \sqcap \exists\PO.\,\textsf{River}
      && \text{``a city \emph{overlapping} some river''}
\end{align*}
\vfill
A \alert{reasoner} then derives the taxonomy \emph{automatically}:
\[
  \textsf{Hamburg} \;\sqsubseteq\; \textsf{GermanCity}
  \qquad\text{and}\qquad
  \textsf{Hamburg} \;\sqsubseteq\; \textsf{CityAtRiver}.
\]
\vfill
\begin{itemize}
  \item The decision problem underneath everything:
        \alert{concept satisfiability} --- ``can this definition be realized at all?''
  \item Subsumption, classification, consistency all reduce to it.
\end{itemize}
\end{frame}
...
\begin{tikzpicture}[scale=.95]
  \draw[-{Latex},very thick,petrol] (0,-1.6) -- (0,1.6)
     node[above] {\textbf{vertical} $\PP,\PPI$ (nesting)};
  \draw[-{Latex},very thick,warm] (-3.4,0) -- (3.4,0)
     node[right] {\textbf{horizontal} $\DR,\PO$ (side by side)};
  \node[petrol,align=left,anchor=west] at (.25,1.1)
     {\small\emph{tame}: composition can force it,\\ \small infinite chains fold into loops};
  \node[warm,align=left,anchor=west] at (-3.3,-.85)
     {\small\emph{wild}: no \emph{single} step forces a value\\
      \small (joint constraints can); forces only \emph{apart}};
\end{tikzpicture}
\end{center}
\vfill
\begin{itemize}
...
\begin{center}
\resizebox{\textwidth}{!}{$\comp(\DR,\PPI)=\comp(\PP,\DR)=\{\DR\},\quad
...
is $\DR$ from every part of $y$\\[3pt]
...
\begin{tikzpicture}[
  ...
  \node[box, fill=good!15] (dec) at (-4.4,0) {\textbf{NO}\\ $\Rightarrow$ \textbf{decidable}};
  \node[box, fill=openc!20] (und) at (4.4,0) {\textbf{YES}\\ $\Rightarrow$ grids $\Rightarrow$ dominoes\\ $\Rightarrow$ \textbf{undecidable}};
  \draw[-{Latex}, thick] (f6) -- (dec) node[midway, above, lab] {bound the\\ looseness};
  \draw[-{Latex}, thick] (f6) -- (und) node[midway, above, lab] {circumvent the\\ looseness};
\end{tikzpicture}
\end{center}
\vfill
...
\end{center}
\vspace{-4pt}
{\small An RCC8 network isomorphic to an $n\times n$ grid --- constructed by
hand in my 2002/2003 report (report7, Fig.~10). The models are
\emph{out there}.}
\end{frame}
...
\begin{center}
\includegraphics[width=.66\textwidth]{report7_figure11_coincidence.png}
\end{center}
\vspace{-4pt}
{\small report7, Fig.~11: the two successors $z$ (east-north) and $z'$
(north-east) \emph{should} be the same corner --- but the table allows
\alert{six} relations between them. Leave it loose: the branches never join.
Force a merge with isolation markers: \emph{every} corner collapses to one
point (verified again in 2026, at depth 2, for RCC5 \emph{and} RCC8).}
\vfill
\begin{quote}\small
...
\begin{itemize}
  ...
  \item \alert{15 adversarial reviews} --- a defect or overclaim found in
        \emph{all but two}
  \item \alert{84} self-contained verification probes (each re-derives the
...
\column{.44\textwidth}
  \begin{block}{Why we moved to Lean}
    Thirteen of fifteen reviews broke a \emph{prose} proof --- typically at a
...
\textcolor{good}{\textbf{Abstraction faithful}} &
  ...
  \texttt{Round19Transport}\\
\textcolor{good}{\textbf{RCC5 normal form}} (fwd) &
  every network $\Rightarrow$ order + disjointness &
...
\begin{block}{Theorem (forward direction kernel-checked, arbitrary domains;
converse tested to size 4)}
  A strong-$\EQ$ RCC5 network is composition-closed \emph{iff} it is an
  \alert{ordered-disjoint structure} (the ``$\Rightarrow$'' is the certified
  half):
  \begin{itemize}
...
The clean structural fact both major proof routes silently relied on ---
now a theorem with a 150-line proof the kernel accepts.
\end{frame}
...
\begin{tikzpicture}[scale=.82]
  \draw[thick,black!40] (-1.1,0) ellipse (2.1 and 1.25);
  \draw[thick,black!40] (1.1,0) ellipse (2.1 and 1.25);
  \node at (-2.55,1.15) {\small $\Sigma^0_1$};
  \node at (2.55,1.15) {\small $\Pi^0_1$};
  \node[align=center] at (0,.35) {\small $\Delta^0_1$\\ \scriptsize decidable};
  \fill[black!50] (-2.15,-.35) circle (2pt) node[below,font=\scriptsize] {halting};
  \fill[black!50] (2.3,.45) circle (2pt) node[above,font=\scriptsize] {domino};
  \fill[petrol] (2.05,-.55) circle (2.6pt)
     node[below,font=\scriptsize,petrol] {\textbf{SAT}$(\ALCIRCC{5})$};
  \draw[-{Latex}, dashed, petrol, thick] (1.85,-.45) to[bend left=12] (.45,-.15);
  \node[petrol,font=\scriptsize] at (1.5,.05) {quantit.-free F6?};
\end{tikzpicture}
\column{.46\textwidth}
\begin{itemize}
...
%% ==================================================================== 30
\begin{frame}{2003, by hand}
\begin{center}
\includegraphics[height=.72\textheight]{report7_figure6_taxonomy.png}
\end{center}
\vspace{-4pt}
{\small The GIS concept taxonomy I drew by hand in report7 (Fig.~6), 2003.}
\end{frame}

%% ==================================================================== 31
\begin{frame}{2026, by the reasoner: all 21/21 subsumptions --- edge for edge}
\begin{center}
\includegraphics[width=.9\textwidth]{gis_taxonomy_reasoner_2026.png}
\end{center}
\vspace{-2pt}
{\small Recomputed automatically 23 years later (green = individuals; note
\emph{Hamburg} and \emph{Alster} each have \alert{two} parents --- the DAG
the reasoner must get right).}
\end{frame}
...
\begin{enumerate}
  ...
  \item \textbf{Don't trust --- verify.} Every load-bearing claim was
        cross-checked mechanically: 84 probes that re-derive the algebra
...
\begin{itemize}
  ...
  \item Everything is frozen, tagged, reproducible: paper (37pp), 4 Lean
        files, 84 probes, 2 reasoners, full conversation log.
\end{itemize}
\vfill
\begin{center}
