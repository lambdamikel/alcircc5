...
%% Based on papers/overview_arxiv.tex.  Revised 2026-08-29 for the
%% forall-PO-free fragment's certification and the three cold reviews;
%% revised 2026-09-14 per GPT-5.6 Sol's cold review (claim calibration,
%% Lutz-Wolter credits, the companion fragment paper).
...
\newcommand{\picDR}{\begin{tikzpicture}[scale=.5,baseline=-2pt]
  \draw[thick,petrol] (0,0) circle (.30); \draw[thick,warm] (.85,0) circle (.30);
\end{tikzpicture}}
\newcommand{\picPO}{\begin{tikzpicture}[scale=.5,baseline=-2pt]
  \draw[thick,petrol] (0,0) circle (.30); \draw[thick,warm] (.38,0) circle (.30);
\end{tikzpicture}}
\newcommand{\picEQ}{\begin{tikzpicture}[scale=.5,baseline=-2pt]
  \draw[thick,petrol] (0,0) circle (.30); \draw[thick,warm,dashed] (0,0) circle (.24);
\end{tikzpicture}}
\newcommand{\picPP}{\begin{tikzpicture}[scale=.5,baseline=-2pt]
  \draw[thick,warm] (0,0) circle (.32); \draw[thick,petrol] (-.09,-.05) circle (.16);
\end{tikzpicture}}
\newcommand{\picPPI}{\begin{tikzpicture}[scale=.5,baseline=-2pt]
...
\author{Michael Wessel\\[2pt]
  {\small with AI assistants: Claude Opus~4.6--4.8, Fable~5, Opus~5 (Anthropic);\\
...
\begin{itemize}
  ...
  \item In 2026 I directed a \alert{five-month AI campaign} on it:
        frontier models, adversarial review, machine verification.
  \item The \alert{full} question is \alert{not settled}; we know exactly
        where our main route is stuck (a width property, F6).
  \item A \alert{sub-question is settled}: drop $\forall\PO$ and the logic is
        \alert{decidable}, by a procedure the \alert{Lean kernel checks}.
\end{itemize}
\vfill
\begin{block}{How to read everything in this talk}
  The mathematics is AI-produced and \emph{unverified by human experts}.
  Standing label for the \alert{full logic}: \statuslabel{}\,---\,%
  but the $\forall\PO$-free fragment is
  \textcolor{good}{\textbf{machine-certified}}, reviewed three times.
\end{block}
\end{frame}
...
\begin{itemize}\small
  ...
  \item every country Alster lies in is a \textsf{Germany}
        ($\forall\PP.(\neg\textsf{Country}\sqcup\textsf{Germany})$)
        $\;\Rightarrow\; c$ is one
\end{itemize}
...
%% ==================================================================== 11
\begin{frame}{Shadows, live width, and the property our route turns on}
\begin{itemize}
...
\end{itemize}
\vfill
\begin{block}{Property F6 (the keystone of the certificate route)}
  Is the live width of models bounded by a computable function of the
  input concept?
\end{block}
\vfill
\begin{center}
  \textbf{If yes} (and two argued steps are proved) $\Rightarrow$ finite
  certificate space $\Rightarrow$ \alert{satisfiability is decidable}.
\end{center}
\end{frame}
...
\begin{tikzpicture}[
  ...
  \node[box, fill=good!15] (dec) at (-4.6,0) {\textbf{NO}\\ $\Rightarrow$ \textbf{decidable}\\ \scriptsize (by our route)};
  \node[box, fill=openc!20] (und) at (4.5,0) {\textbf{YES}\\ $\Rightarrow$ our route fails;\\ \scriptsize undecidable \emph{if} it\\ \scriptsize yields a grid};
  \draw[-{Latex}, thick] (f6) -- (dec);
  \draw[-{Latex}, thick] (f6) -- (und);
  \node[lab, good] at (-2.65,1.6) {bound the\\ looseness};
  \node[lab, warm] at (2.7,1.6) {circumvent\\ the looseness};
\end{tikzpicture}
\end{center}
\vfill
The \emph{same} looseness of the composition table blocks \alert{both}
proofs --- a \emph{shared} obstruction, not a theorem that either answer
settles the problem. That is why neither direction has moved in twenty years.
\end{frame}
...
%% ==================================================================== 15
\begin{frame}{\dots but no known concept \emph{forces} them: the coincidence obstruction}
\begin{center}
...
\begin{itemize}
  ...
  \item \alert{21 adversarial reviews} --- a defect or overclaim found in
        \emph{all but two}
  \item \alert{94} self-contained verification probes (each re-derives the
        algebra from set semantics)
  \item 5 Lean files, $\sim$\alert{51{,}000 lines}, \alert{zero \texttt{sorry}}
  \item 2 working reasoners, cross-validated on a \alert{911-case} suite
\end{itemize}
\column{.48\textwidth}
\begin{block}{The most telling pattern}
  Unsatisfiable concepts \emph{were} accepted --- by manuscripts as
  literally written. Every such case traced to a
  \alert{missing clause in an argument}, never to a counterexample to the
  conjecture itself.
\end{block}
\medskip
...
\begin{itemize}
  ...
  \item Dormant $\forall$'s reawaken blocked nodes; no fixed-arity local
        summary we tried (types, triangles, quadruples, profiles, \dots) was enough.
\end{itemize}
\vfill
\begin{center}
\alert{Lesson: in a total-relation logic, ``local'' does not exist.}\\
Whether a node may be reused depends on the whole graph --- a \emph{global} condition no local test sees.
\end{center}
\end{frame}
...
\column{.46\textwidth}
  \begin{itemize}
    ...
    \item All remaining cross-edges are \alert{horizontal}; complete them by
          the RCC5 \emph{patchwork property}.
    \item The one component \alert{every} cold review found sound ---
          though the certified route keeps only its \emph{shape}, not this
          machinery: no patchwork appeal, no quotient.
  \end{itemize}
\end{columns}
\vfill
...
\column{.44\textwidth}
  \begin{block}{Why we moved to Lean}
    ...
    found \emph{interface} gaps and overclaims instead --- each closed or
    withdrawn.
  \end{block}
\end{columns}
\end{frame}
...
\textcolor{openc}{\textbf{Forcing reduction}} (cond.) &
  F6 fails $\Leftrightarrow$ forced crowds, \emph{if} adequacy &
  \texttt{ForcingReduction}\\
\textcolor{openc}{\textbf{Dovetail schema}} (cond.) &
  qual.\ F6 $\Rightarrow$ decidable, \emph{if} searches complete &
  \texttt{SemiDecidability}\\
\midrule
\textcolor{good}{\textbf{$\forall\PO$-free decided}} \tiny(2026-08-29) &
  \normalsize a \emph{decision procedure}, raw input &
  \texttt{POFreeLift}\\
\textcolor{good}{\textbf{Abstract $=$ concrete}} &
  composition table $\Leftrightarrow$ real sets (cf.\ L\&W Thm~23) &
  \texttt{POFreeLift}\\
\bottomrule
\end{tabular}
\end{center}
\vfill
\alert{Not} in the kernel: F6 itself, and the steps from it to a complete
certificate search. They appear only as \emph{hypotheses} of conditional
theorems --- never axiomatized, never proved.
\end{frame}

%% ==================================================================== 23
\begin{frame}{The full-logic theorem (conditional; its premise is open)}
\begin{block}{Conditional decidability --- what the kernel actually checks}
  \textbf{If} a verified finite checker is complete for some computable list
  of candidate certificates per concept, \textbf{then} $\ALCIRCC{5}$ concept
  satisfiability is \alert{decidable}.
\end{block}
\vfill
\begin{itemize}
  ...
  \item Open: the \alert{premise}. Our route to it has two \emph{argued, unproved}
        steps: F6 $\overset{?}{\Rightarrow}$ refined W2$'$
        $\overset{?}{\Rightarrow}$ complete lists.
\end{itemize}
\end{frame}

%% ==================================================================== 24
\begin{frame}{Unconditional win \#1: a decidable fragment --- now \emph{certified}}
\begin{block}{Theorem ($\forall\PO$-free fragment), machine-checked 2026-08-29}
  Concepts whose NNF has \alert{no $\forall\PO$} are \alert{decidable}.
  In Lean, with fragment membership the \emph{only} hypothesis:
  \smallskip
  {\scriptsize\texttt{decidableFSat} (raw input) \quad
   \texttt{decidableSat\_cone} (NNF) \quad
   \texttt{decidableSetSat} (real sets)}
\end{block}
\vfill
\begin{itemize}
  \item Still allowed: $\exists\PO$, $\forall\DR$, $\exists\DR$ and
        \alert{all} part-of modalities --- an expressive spatial fragment.
  \item On \alert{raw} input the condition is \emph{polarity-sensitive}: no
        $\forall\PO$ positively, no $\exists\PO$ \emph{negatively}.
  \item Argued by two routes, \alert{certified} by a third ---
        GPT-5.6~Sol's \emph{cone scheme}. Now its own paper.
  \item Concept satisfiability only (not closed under negation);
        \alert{decidable $\neq$ runnable} --- doubly exponential.
\end{itemize}
\end{frame}

%% ==================================================================== 24b
\begin{frame}{Why \emph{that} fragment --- the one-sentence reason}
\begin{block}{The residual default}
  When composition leaves a pair's relation open, you can always settle it
  as \alert{$\PO$} --- \emph{unless some formula can object to a $\PO$
  edge}. The only formula that can is $\forall\PO.D$.
\end{block}
\vfill
\begin{itemize}
  \item So in the fragment the \alert{lazy arrangement} --- declare only what
        a birth forces, default every remaining pair to $\PO$ --- is
        \alert{always} a model.
  \item Hence \alert{nothing ever has to be coordinated}: every relation
        between two different children is the residual, and the residual is
        free. That is why the construction needs no joint realizability.
  \item Put $\forall\PO$ back and you do not lose one case of one induction
        --- you lose \alert{the default itself}. That is the whole boundary.
  \item The arrangement it builds is a \alert{split forest} --- part-of trees,
        horizontal relations read off: Route~2's whiteboard idea,
        \emph{built} rather than extracted.
\end{itemize}
\end{frame}

%% ==================================================================== 24c
\begin{frame}{Is the fragment new? What the literature actually says}
\begin{block}{Lutz \& Wolter 2006 --- the one place anyone gestured at this}
  They leave $L_{\mathrm{RCC5}}(\mathit{RS})$ --- \emph{exactly} this logic ---
  \alert{open}, and float: ``giving up some of the relations\dots\ dropping
  $\PO$ still results in a useful formalism''.
\end{block}
\begin{itemize}
  \item That sentence defends \alert{usefulness}, not decidability ---
        no proof, no sketch, no conjecture.
  \item Our restriction is \alert{weaker}: $\PO$ stays in the language.
        $\exists\PO$ is kept; only $\forall\PO$ goes.
  \item What they \emph{did} prove about it --- r.e., representable by
        regions (Thms~23, 24, 28) --- says nothing about decidability.
  \item Wessel's 2003 report canvassed \emph{stronger} restrictions, all
        restoring the \alert{finite model property} --- which this fragment lacks.
\end{itemize}
\vfill
\begin{center}
New \alert{so far as we can determine} --- deliberately weaker than a
priority claim.
\end{center}
\end{frame}
...
Both directions are kernel-checked for arbitrary domains, the
``$\Leftarrow$'' with GPT-5.6~Pro's \alert{canonical set representation}.
That RCC5 structures are representable by regions at all is
\alert{Lutz \& Wolter's} Thm~23 (2006); what is new is this form, the
explicit construction, and the kernel proofs.
\end{frame}

%% ==================================================================== 26
\begin{frame}{A rediscovery: the problem's address is $\Pi^0_1$}
\begin{columns}[T]
\column{.54\textwidth}
\begin{center}
...
\begin{itemize}
  ...
  \item \alert{Not new}: Lutz \& Wolter, 2006 (Thm~28). We rederived it
        and at first claimed it --- since corrected.
\end{itemize}
\end{columns}
\end{frame}
...
\end{center}
\begin{itemize}
  \item For this route, needed: only \alert{qualitative} F6 --- \emph{some}
        finite certificate per satisfiable concept, \alert{no computable bound}.
...
FO$^2$ + 1 transitive rel. & \textbf{?} & decidable; a 2-copy encoding \emph{nearly} fits\\
\bottomrule
\end{tabular}
\end{center}
\vfill
The failure that matters traces to one fact: \alert{every pair is related},
so models are maximally non-local --- the obstruction F6 names.
\end{frame}
...
\begin{itemize}
  ...
  \item A \alert{911-case suite} (99 labelled, 812 oracle comparisons) ---
        \alert{zero mismatches}; $775/775$ decomposition tests; \alert{no verdict
        ever shown wrong}. One latent gap, found by code review (April), fixed
...
\vfill
\begin{center}
A running algorithm and a kernel-checked surround, \alert{pointing the same
way} --- evidence, not a proof.
\end{center}
\end{frame}
...
\begin{itemize}
  ...
  \item a $\sim$51{,}000-line kernel-checked Lean development
...
%% ==================================================================== 32b
\begin{frame}{The failure mode worth naming: productive-looking thrashing}
\begin{center}
\alert{Three weeks. Four} successive repairs to one architecture. Each
locally reasonable, each probed, each refuted by an exact finite
countermodel --- and it proposed the next one.
\end{center}
\begin{itemize}
  \item It never concluded the \alert{class} was wrong --- though its own
        design document already recorded this as the \emph{fifth}
        occurrence of the same shape.
  \item Not hallucination, not local error: \alert{every step was correct
        and checked}. The context makes the framing feel settled because
        it was never questioned there.
  \item What broke it came from outside: a halt, and a \alert{different
        model} --- no history in the design --- asked for a different
        architecture. That answer became the certified route.
\end{itemize}
\vfill
\begin{center}\small
Cold review, aimed at \emph{design} rather than proof. Knowing \emph{when}
to call it was the human's --- \alert{we cannot automate that}.
\end{center}
\end{frame}
...
\begin{enumerate}
  ...
  \item \textbf{Don't trust --- verify.} We cross-checked load-bearing claims
        mechanically: 94 probes that re-derive the algebra from set
        semantics, and ultimately the Lean kernel. The machine checks are
        the ground truth, \alert{not the model's confidence}.
  \item \textbf{Label honestly.} \emph{Strongly supported} $\neq$
        \emph{certified}. Overclaims kept slipping in --- most recently in our
        own status tables --- and cold reviews kept catching them; each was
        withdrawn \alert{on the record}.
\end{enumerate}
\vfill
...
\begin{itemize}
  ...
  \item Saying exactly this was a stated purpose of the work. Its first
        ...
        mathematics or the message --- so it goes out
...
\textcolor{good}{\textbf{Lean-certified}} &
  \textbf{$\forall\PO$-free decidability} (raw, NNF, sets); abstract $=$
  concrete; soundness pipeline; faithfulness; normal form; conditional
  reductions \emph{as conditionals} &
  kernel-checked, stated axioms\\
\addlinespace
\textbf{Published} &
  $\Pi^0_1$ membership; representation by regions (Lutz \& Wolter 2006,
  Thms~23, 24, 28) &
  an external theorem, cited\\
\addlinespace
\textbf{Theorem-level} &
  the fragment's two \emph{arguments} ($K(C_0)$, extraction); W2$'$ refinement &
  proof in a work package\\
\addlinespace
\textbf{Empirical} &
  cover-tree tableau; 911-case suite; model verifier &
  test evidence, not a decision theorem\\
\addlinespace
\textcolor{openc}{\textbf{Open}} &
  F6; whether identity-selector minors are its only obstruction;
  completeness of the certificate scheme; full decidability &
  no proof claimed\\
\bottomrule
\end{tabular}
\end{center}
...
\begin{tikzpicture}[font=\small]
  \node[draw, rounded corners=3pt, fill=petrol!12, inner sep=8pt, align=center] (q)
    {\textbf{Full decidability: open}\\[2pt]
     our route's crux: can a concept force\\ unbounded \emph{identity-selector}
     structure? \scriptsize(conjectured)};
\end{tikzpicture}
\end{center}
\begin{itemize}
  \item NO $\Rightarrow$ our route can close; YES $\Rightarrow$ it fails, and
        undecidability would still need a grid.
  \item One \alert{sub-problem off the pile}: drop $\forall\PO$ and it is
        decided, in the kernel --- three cold reviews; now its own paper.
  \item Reproducible: 5 Lean files ($\sim$51k lines, 0 \texttt{sorry}), 94
        probes, 2 reasoners, full log.
\end{itemize}
\vfill
\begin{center}
  \alert{\textbf{Open since 2003. Still standing --- but one wall has come
  down.}}\\[3pt]
...
\begin{itemize}
  \item The naive ways to force width (shell half-graphs, prefix pumps,
        $\PO$-gaps) all \alert{collapse} to finite catalogues --- probe-checked.
  ...
  \item A finite catalogue can hold only boundedly many private selectors.
        \textbf{Conjecture:} F6 fails iff some concept forces such minors for
        every $n$.
  \item Bare RCC5 networks with unbounded such structure \emph{exist}
        ...
        The uniformization side-condition (W2$'$) is argued to fold into F6.
\end{itemize}
\end{frame}
