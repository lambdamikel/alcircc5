...
\begin{itemize}
  ...
  \item And one \alert{sub-problem removed from the pile}: drop $\forall\PO$
        and it is decided, in the kernel, on raw input, under real set
        semantics --- three cold reviews, no counterexample.
  \item Everything reproducible: papers, 4 Lean files ($>$45k lines,
        zero \texttt{sorry}), 100+ probes, 2 reasoners, full audit log.
\end{itemize}
\vfill
\begin{center}
  \alert{\textbf{Open since 2003. Smaller than ever. Still standing ---}}\\
  \alert{\textbf{but one wall of it has come down.}}\\[4pt]
