...
\begin{enumerate}[label=(\alph*)]
  ...
  \item \textbf{Completeness extraction.}
    ...
    and is written out in our companion
    paper~\cite{CompletenessExtraction}: starting from a pointed
    strong-$\EQ$ model, we split join nodes into $\EQ$-mates, read
    off rank-$d$ states by induction, extract sibling-interface
    descriptors from realized atomic relations, and extract
    relation-safe witness menus.  The key lemma is that in any
    extracted quotient, Need-filtered pair domains equal unfiltered
    ones, because the source model satisfies all universals and
    local types are preserved under the rank-$d$ quotient.
