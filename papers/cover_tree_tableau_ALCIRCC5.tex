...
(composition propagation and restricted sibling
...
\begin{itemize}[nosep]
  \item For all $\forall R.D \in \tau$: $D \in \sigma$; and
    if $R \in \{\PP, \PPI\}$, additionally $\forall R.D \in \sigma$.
  \item For all $\forall \inv(R).D \in \sigma$: $D \in \tau$; and
    if $\inv(R) \in \{\PP, \PPI\}$, additionally
    $\forall \inv(R).D \in \tau$.
\end{itemize}
...
\begin{remark}[Why universals propagate for $\PP$ and $\PPI$]
\label{rem:trans-propagation}
The second clause in each bullet is the standard transitive-role
propagation rule (as in $\mathcal{ALCH}_{\mathrm{tr}}$). Both $\PP$
and $\PPI$ are transitive in RCC5 ($\comp(\PP, \PP) = \{\PP\}$,
$\comp(\PPI, \PPI) = \{\PPI\}$), so a $\PP$-successor of a
$\PP$-successor of $x$ is itself a $\PP$-successor of $x$. If
$\forall \PP. D$ holds at $x$ and $\PP(x, y)$, then every
$\PP$-successor of $y$ must satisfy $D$; hence
$\forall \PP. D$ holds at $y$. Failing to propagate the universal
itself (and only its body $D$) would admit type pairs
$(\tau, \sigma)$ with $\forall \PP. D \in \tau$ but
$\forall \PP. D \notin \sigma$, in which case a $\PP$-grandchild
could falsify $D$ while each immediate step individually satisfies
the weak safe condition. This was an implicit assumption in earlier
formulations of this calculus; making it explicit closes an
unsoundness reported by Opus~4.7 on the concept
$\forall \PP. \neg C \sqcap \exists \PP. \exists \PP. C$. The
other three base relations are not transitive
($\comp(\DR, \DR) = \RCCrels$, $\comp(\PO, \PO)$ is all of
$\RCCrels$; $\EQ$ is reflexive-identity under strong-$\EQ$
semantics), so no analogous propagation is needed for them.
\end{remark}
...
\begin{enumerate}[label=(CT\arabic*)]
  ...
  \item \textbf{Tree-cross interaction}: for each type with multiple
    demands, every pair of witnesses must be jointly satisfiable
    under composition constraints
    (Section~\ref{sec:singleton}).
  \item \textbf{Role-path compatibility}: for every three-type chain
    $\tau_g \xrightarrow{R} \tau_j \xrightarrow{S} \tau_w$ (with
    $\exists R.D \in \tau_g$ witnessed by $\tau_j$ and
    $\exists S.E \in \tau_j$ witnessed by $\tau_w$), the grandchild
    $\tau_w$ must be connectable back to $\tau_g$ by some relation
    in $\comp(\inv(S), \inv(R)) \cap \Safe(\tau_w, \tau_g)$, with a
    cycle-close via $\tau_w = \tau_g$ admitted exactly when
    $(\inv(S), \inv(R))$ admits $\EQ$ (Section~\ref{sec:rolepath}).
\end{enumerate}
...
\subsection{Phase 4: Tree-cross interaction (composition propagation)}
\label{sec:singleton}

When a type $\tau_i$ has two demands $\exists R_1.D_1$
and $\exists R_2.D_2$,
...
For this to be satisfiable, some relation in the composition
set must be safe between the witnesses.  The check is:
for every pair of demands at type~$\tau_i$,
verify that there exist witnesses
$\tau_{j_1}, \tau_{j_2} \in T$ such that
\[
  \comp(\inv(R_1), R_2) \;\cap\; \Safe(\tau_{j_1}, \tau_{j_2})
  \;\neq\; \emptyset
\]
\emph{and} conversely
$\comp(\inv(R_2), R_1) \cap \Safe(\tau_{j_2}, \tau_{j_1})
\neq \emptyset$.

When the composition is a \emph{singleton} $\{S\}$,
this forces $S \in \Safe(\tau_{j_1}, \tau_{j_2})$.
But the check is equally important for non-singleton
compositions: if every relation in
$\comp(\inv(R_1), R_2)$ is unsafe between
all candidate witness pairs, the type set is still
inconsistent.

\begin{proposition}[Key compositions in RCC5]
\label{prop:singletons}
Some compositions involving tree roles and cross-edge roles:
\begin{center}
...
$\PPI$ (tree child) & $\DR$ (cross) &
  $\comp(\PP, \DR) = \{\DR\}$ \textit{(singleton -- forced)} \\
$\PP$ (tree parent) & $\PPI$ (tree child) &
  $\comp(\PPI, \PPI) = \{\PPI\}$ \textit{(singleton -- forced)} \\
$\PPI$ (tree child) & $\PP$ (tree parent) &
  $\comp(\PP, \PP) = \{\PP\}$ \textit{(singleton -- forced)} \\
$\PO$ (cross) & $\PP$ (tree parent) &
  $\comp(\PO, \PP) = \{\PP,\PO\}$ \textit{(both may be unsafe)} \\
$\PP$ (tree parent) & $\DR$ (cross) &
  $\comp(\PPI, \DR) = \{\DR,\PO,\PPI\}$ \textit{(usually satisfiable)} \\
\bottomrule
\end{tabular}
\end{center}
\end{proposition}
...
\subsection{Phase 5: Role-path compatibility (three-type chains)}
\label{sec:rolepath}

The tree-cross check of Section~\ref{sec:singleton} covers chains of
length~$2$ emanating from a single source type. It misses
\emph{three-type chains} $\tau_g \to \tau_j \to \tau_w$ in which
$\tau_g$ and $\tau_j$ each have a single demand, because the early
exit ``$|\,\text{dems}(\tau_i)\,| \le 1$'' skips them. A
systematic family of counterexamples of the form
\[
  C_{R_1, R_2} \;\equiv\;
  \exists R_1.\,\exists R_2.\,A
  \;\sqcap\;
  \bigwedge_{R \,\in\, \comp(R_1, R_2)} \forall R.\,\neg A
  \qquad (R_2 \ne \inv(R_1))
\]
was identified by Opus~4.7 as an adversarial review of an earlier
draft of this calculus. These concepts are unsatisfiable: the chain
$g \xrightarrow{R_1} j \xrightarrow{R_2} w$ with $A \in \tau_w$
forces $g$ to relate to $w$ by some relation in $\comp(R_1, R_2)$, and
every such relation is barred by a universal in $\tau_g$.
Twelve of the sixteen $(R_1, R_2)$ cells of the $4 \times 4$ grid
yield genuinely unsatisfiable concepts --- only the four
$\EQ$-admitting cells $(\DR,\DR)$, $(\PO,\PO)$, $(\PP,\PPI)$,
$(\PPI,\PP)$ are satisfiable (via an $\EQ$-identification of $w$ with
$g$).

Phase~5 enforces the check explicitly. For each demand
$\exists R.D$ at a type $\tau_g$ and each candidate witness
$\tau_j$ (with $D \in \tau_j$, $R \in \Safe(\tau_g, \tau_j)$), and
for each further demand $\exists S.E$ at $\tau_j$, require that
\emph{some} grandchild $\tau_w$ with $E \in \tau_w$,
$S \in \Safe(\tau_j, \tau_w)$ satisfies one of:
\begin{itemize}[nosep]
  \item $\comp(\inv(S), \inv(R)) \cap \Safe(\tau_w, \tau_g)
    \neq \varnothing$, or
  \item $\tau_w = \tau_g$ and
    $(\inv(S), \inv(R)) \in \{(\DR,\DR), (\PO,\PO),
    (\PP,\PPI), (\PPI,\PP)\}$ \emph{(strong-$\EQ$ cycle-close)}.
\end{itemize}
Witness candidates are pruned iteratively until a fixpoint is
reached; the type set is rejected if any demand's candidate set
becomes empty.

\begin{remark}[Why the cycle-close clause is needed]
\label{rem:cycleclose}
The split-forest semantics represents $\EQ$-mates by distinct cover-tree
nodes with the same extracted type. The four $\EQ$-admitting
composition pairs are exactly those for which the chain
$g \xrightarrow{R_1} j \xrightarrow{R_2} g$ closes back through
strong-$\EQ$ identification. Omitting the cycle-close clause
would wrongly reject the satisfiable cyclic-model concepts
such as $C \sqcap \exists\PO.\exists\PO.C \sqcap
\bigwedge_{R \ne \EQ} \forall R.\neg C$, which admits a
two-element model $\{d, a\}$ with $\PO(d, a)$.
\end{remark}
...
but the composition propagation check (Section~\ref{sec:singleton})
captures its essential consequence: the composition of any two
demand roles must intersect the safe set of their witnesses.

\subsection{Implicit PP via composition forcing}
\label{sec:pp-forcing}

The following example shows that $\PP$ edges can be forced
without any explicit $\exists\PP$ demand, purely through
composition and universal filtering.

\begin{example}[PP forcing without $\exists\PP$]
\label{ex:pp-forcing}
Consider
\[
  X \;=\; \exists\DR.\exists\PO.C
    ...
    \;\sqcap\; \forall\PP.X.
\]
The concept has no $\exists\PP$ demand, yet requires an
infinite model with $\PP$ edges.  Here is how the cover-tree
tableau handles it.

A root type~$\tau_0$ satisfying~$X$ has the demand
$\exists\DR.D$ (where $D = \exists\PO.C$), witnessed by
a type~$\tau_1$ containing $\{D, \neg C\}$.
Type~$\tau_1$ in turn has the demand $\exists\PO.C$,
witnessed by a type~$\tau_2$ containing~$C$.

Cross-edge consistency (CT2) requires
$\Safe(\tau_0, \tau_2) \neq \varnothing$.
Since $\forall\DR.\neg C \in \tau_0$ and $C \in \tau_2$,
relation $\DR$ is unsafe.  Similarly, $\forall\PO.\neg C$
blocks $\PO$.  Of the remaining relations, $\PP$ is safe
provided $\tau_2$ satisfies $X$ (the only $\forall\PP$
formula in~$\tau_0$ is $\forall\PP.X$).

In the cover tree, $\tau_0 \;\PP\; \tau_2$ means
$\tau_2$~is an \emph{ancestor} of~$\tau_0$ (since $\PPI$
= parent direction).  Since $X \in \tau_2$, the ancestor
also has the $\exists\DR.D$ demand, regenerating the
pattern: $\tau_2$ creates its own $\DR \to \PO$ chain
forcing yet another $\PP$-ancestor above it, giving an
infinite ascending chain:
\[
  \tau_0 \;\PP\; \tau_2 \;\PP\; \tau_2' \;\PP\;
  \tau_2'' \;\PP\; \cdots
\]
The type set is finite: $\tau_2$ is \emph{reused} as its
own $\PP$-ancestor (different domain elements, same abstract
type).  The valid type set
$\{\tau_0, \tau_1, \tau_2\}$ passes all four checks
(CT1--CT4), and the concept is correctly reported as SAT.
\qed
\end{example}

This pattern---$\PP$ edges forced by $\comp(\DR,\PO) =
\{\DR,\PO,\PP\}$ combined with universals blocking $\DR$
and~$\PO$---is a distinctive feature of the complete-graph
semantics.  It demonstrates why the cover-tree tableau's
separation of tree and cross layers is essential: the forced
$\PP$ edge places the $C$-element in the \emph{tree layer}
(as an ancestor), not the cross layer, even though the
original demands involved only $\DR$ and~$\PO$.
...
\begin{enumerate}[nosep]
  ...
  \item The composition propagation check
    (Section~\ref{sec:singleton}).
...
\begin{itemize}[nosep]
  ...
  \item All 6 cross-role composition-propagation UNSAT patterns
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{Descriptor language and local coherence axioms
    (C1--C5).}
    The composition propagation check captures the most critical
    coherence constraint: for every demand pair, the composition
    set must intersect the safe set of their witnesses.
...
\begin{itemize}[nosep]
  ...
  \item The quasimodel reasoner's sibling check on all demand
    ...
    cross-only check.
  \item The cover-tree's composition propagation check catches
    exactly the cases where the quasimodel's stronger checks
    would also reject (all composition-propagation UNSAT patterns
    are caught by both).
\end{itemize}
...
\subsection{Proof status: theory vs.~implementation}
\label{sec:proof-status}

It is worth being precise about which statements are formally
proven and which are supported by empirical agreement only, because
the two are easily conflated in a repository that mixes theory
papers with an executable reasoner.

\medskip\noindent\textbf{What is formally established (modulo peer
review).}
\begin{enumerate}[label=(\alph*)]
  \item \textbf{Soundness of the split-forest semantics.}
    Theorems~1.17--1.19 of~\cite{SiblingCompleted} prove by LCA
    induction that every valid finite rank-$d$ quotient unfolds to
    an arc-consistent disjunctive network, and that K\"onig's
    lemma plus canonical refinements yield a strong-$\EQ$ model.
    This direction is fully written out.

  \item \textbf{Completeness extraction.}
    The reverse direction---every satisfiable
    $C_0$ admits a valid finite rank-$d$
    quotient---was labelled ``condensed'' in~\cite{SiblingCompleted}.
    Claude's earlier completeness-extraction
    attempts (\cite{CompletenessExtractionV1}, v1; v2.1, May~2026) are
    \emph{superseded} after a GPT-5.5 round-2 review found
    two structural defects in v1 and a residual
    $\omega$-acceptance dependency in v2.1.  The current best
    statement is GPT-5.5's repaired
    proof~\cite{GPT55Repaired} (May~2026, 16~pages, no-automata
    route), which formulates rank-$d$ split-forest certificates
    and ten finite-checkable validity conditions (V1)--(V10), and
    proves both directions in their entirety.  The three structural
    ingredients that make the proof go through are (i)~occurrence-level
    equality, (ii)~mates with different parents, and
    (iii)~request-closed cycles in place of $\omega$-acceptance.

  \item \textbf{Decidability of $\ALCIRCC{5}$ concept satisfiability.}
    The conjunction of~(a) and~(b) establishes decidability:
    $C_0$ is satisfiable iff it admits a valid finite rank-$d$
    quotient, and the space of such quotients is finite and
    effectively searchable (rank-$d$ states are bounded by a
    function of $|\mathrm{cl}(C_0)|$).  An explicit elementary
    complexity bound is not extracted.
\end{enumerate}

\medskip\noindent\textbf{What the cover-tree tableau is and is
\emph{not}.}
The cover-tree tableau (\S\ref{sec:algorithm}) is \emph{inspired
by}~\cite{SiblingCompleted,CoverTree} but is \emph{not} a formal
derivation of those semantics.  It is a set of five concrete
checks (CT1--CT5) over type sets, arrived at by algorithmic
simplification and---in the case of CT5---by empirical
counterexample analysis from Opus~4.7's adversarial review
(\S\ref{sec:rolepath}), rather than by translation from the
split-forest definition of a valid finite quotient.  Concretely:
\begin{itemize}[nosep]
  \item The path-consistency check is restricted to the $\{\DR,\PO\}$
    sub-network.  The split-forest semantics works with the full
    4-relation network under disjunctive arc-consistency.
  \item The sibling-interface descriptor conditions~(C1)--(C5)
    of~\cite{SiblingCompleted} are not explicitly checked:~(C1)
    and~(C2) are implicit in the $\{\DR,\PO\}$ sibling check, but
    the downward-support axioms~(C3)--(C4) that track how
    cross-edge domains evolve through ancestor-descendant chains
    are not represented.
  \item Witness menus, rank-$d$ state identification, and the
    relation-sensitive filtering of pair domains are not
    constructed.
  \item CT5 (role-path compatibility) was ported from the
    quasimodel reasoner to patch the twelve-counterexample family
    reported in Opus~4.7's adversarial review
    (\texttt{review\_paper/review\_cover\_tree\_tableau.tex},
    Round~1).  The cycle-close clause for $\EQ$-admitting pairs
    preserves soundness on the three genuine cyclic-$\mathsf{SAT}$
    concepts.  While CT5 is motivated by the underlying semantics
    (it prevents compositions that could not arise in a valid
    quotient), its specific form was not derived from~(C3)--(C4).
\end{itemize}

\medskip\noindent\textbf{What we claim, in one sentence.}
$\ALCIRCC{5}$ concept satisfiability is decidable by~(a)--(c) above;
the cover-tree tableau is a \emph{plausible and empirically
validated} algorithmic realization (911 original cross-validation
tests plus 400+ Round-2 adversarial tests, zero mismatches against
the cycle-aware quasimodel reasoner), but its formal equivalence
to the split-forest semantics---in particular, that CT1--CT5
exactly capture the valid-quotient conditions---remains to be
established.  The empirical agreement between two independently
designed reasoners is strong evidence that such an equivalence
should be provable, but evidence is not proof.
...
\begin{enumerate}[label=(\arabic*)]
  \item \textbf{Formal correctness.}
    ...
    The Round-2 adversarial audit (400+ additional targeted and
    random tests, zero mismatches against the cycle-aware
    quasimodel reasoner; see~\S\ref{sec:proof-status}) sharpens the
    empirical case but does not close the formal gap.
...
\begin{enumerate}[nosep]
  ...
  \item A composition propagation check that catches
    cross-role UNSAT patterns where the composition set between
    any two witnesses has empty intersection with the safe set.
\end{enumerate}
...
\newblock \url{https://github.com/lambdamikel/alcircc5/blob/master/papers/decidability_ALCIRCC5.pdf}
...
\newblock \url{https://github.com/lambdamikel/alcircc5/blob/master/papers/ALCI_RCC5_contextual_tableau_draft.pdf}
...
Claude Opus (Anthropic), prompted by Michael Wessel.
\newblock Closing the completeness gap for $\mathcal{ALCI}_{RCC5}$:
  extraction of valid finite rank-$d$ quotients from strong-$\EQ$
  models (v1; \emph{superseded}).
\newblock Manuscript, April 2026.
...
\newblock A repaired split-forest decidability proof for
  $\mathcal{ALCI}_{RCC5}$ (no-automata route).
\newblock Manuscript, May 2026.
