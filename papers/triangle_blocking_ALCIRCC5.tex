...
\medskip\noindent\textbf{Note on companion paper status.}\;
The companion paper~\cite{CompanionPaper} has itself undergone
significant revision: its type-elimination algorithm is
\textbf{retracted} (unsound due to Q3 anti-monotonicity failure),
and the tableau's own soundness proof is \textbf{unproven}
(depends on the same Henkin construction with the extension gap).
The conditional result in this paper depends only on the
tableau's \emph{termination and completeness}, which remain
established.}}
\end{center}
...
%% ============================================================
\begin{abstract}
The companion paper~\cite{CompanionPaper} presented two candidate
decision procedures for concept satisfiability in $\ALCIRCC{5}$.
The type-elimination algorithm has since been retracted (unsound),
and the tableau's soundness direction (open branch $\Rightarrow$
model) remains incomplete due to the extension gap.
...
calculus with equality anywhere-blocking.  The type-elimination
algorithm has since been \textbf{retracted}: the anti-monotonicity
property (Q3) on which its soundness depended does not hold, leading
to cascade elimination of satisfiable types.  For the tableau
calculus, termination and completeness (satisfiable $\Rightarrow$
open branch) are established, but the \emph{soundness} direction
(open branch $\Rightarrow$ satisfiable) contained a gap: it relied
on a Henkin construction whose CSP step may fail because the
disjunctive RCC5 network is not necessarily path-consistent.  This
is the \emph{extension gap}.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Conditional decidability}
    ...
    tractability yields a model.  Combined with the tableau's
    termination and completeness from~\cite{CompanionPaper} (which
    remain established despite that paper's other retractions),
    this gives decidability conditional on the \emph{Extension
    Solvability Conjecture}.
...
\begin{theorem}[\cite{CompanionPaper}, Section~7 tableau]
...
\noindent\textit{Note.}\; This theorem concerns the Section~7 tableau
of~\cite{CompanionPaper}.  That paper's type-elimination algorithm
has been retracted (unsound), but the tableau's termination and
completeness remain established independently.
...
Combine:
\begin{itemize}[nosep]
  \item Termination and completeness of the Section~7 tableau
    (Theorem~\ref{thm:term-complete},
    from~\cite{CompanionPaper}---these properties are independent
    of that paper's retracted type-elimination algorithm and its
    unproven tableau soundness).
  \item Conditional soundness (Theorem~\ref{thm:soundness}),
    which provides an \emph{alternative} soundness argument
    replacing the companion paper's Henkin-based proof.
...
\newblock \emph{Note}: type-elimination algorithm retracted (unsound);
  tableau soundness unproven (extension gap).  Tableau termination and
  completeness remain established.
