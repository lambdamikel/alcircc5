...
\title{Description Logic Research in the Era of Agentic AI:\\
  How a Human-and-AI Team Delivered the World's First\\
  DL Decidability Proof}
...
(DL) theory. The mathematical content has been developed and revised
across \emph{four rounds of adversarial review} between OpenAI's
GPT-5.4~Pro / GPT-5.5 and Anthropic's Claude Opus~4.7, with Wessel
guiding the search and correcting high-level errors.
...
agentic AI frontier models (OpenAI's GPT 5.4 Pro / GPT 5.5,
Anthropic's Claude Opus 4.7) for DL research. We hence feel an
obligation to share our observations with the DL community. A
complete survey of the project, a proof sketch, implementation report,
and the multi-round adversarial review log are in the Appendix.

\textbf{We are fully transparent: The \emph{Appendix} was written by
the AI agents, prompted and guided by Michael Wessel.  The main
\emph{Extended Abstract} contribution was written by Michael Wessel
(by hand). The results have not been peer-reviewed by human experts
and should be treated as a \emph{candidate}, not an established
theorem.}
\end{abstract}
...
containing a Lake''. Cohn's paper did not address decidability or
complexity of the new logic.

In 2000, I started a systematic investigation of the Description
Logic (DL) equivalent of Cohn's logics and called it the $\ALCIRCC{}$
DL family, aiming for decidability results and practical prover
...
Cognitive Systems Group at the University of Hamburg, continuing the
group's tradition of spatioterminological reasoning with DLs
...
results for general (non-spatial) DLs with complex role inclusion
axioms (``role boxes'') \cite{Wessel2000RA,
  Wessel2000SG,Wessel2001,Wessel2001RA}, and some decidability results
for the smaller -- but practically irrelevant -- $\ALCIRCC{}$ family
members \cite{Wessel2002, Wessel2003}. Unfortunately, I was unable to
settle decidability or undecidability of $\ALCIRCC{5}$ or
...
OpenAI's GPT 5.4 Pro \cite{GPT54Pro} (and later GPT 5.5
\cite{GPT55Repaired}), and Anthropic's Claude Opus 4.7
...
correctness of my original contributions; no major errors were found,
and the established results still stand unrefuted in April~2026.  The
agents then confirmed that $\ALCIRCC{5}$ / $\ALCIRCC{8}$
(un)decidability is still open, and a three-week after-work side
project\footnote{Costs: $\approx$\,500\,USD all-in (Claude Code Pro
subscription plus extra token usage).} explored 11 decidability
approaches and 7 potential undecidability reductions.

Of those 11 attempts, one attempt to prove \ALCIRCC{5} decidability
...
\ALCIRCC{5} models} \cite{SiblingCompleted, GPT55Repaired}, and has
survived \emph{four rounds of adversarial review} between GPT-5.5 and
Claude. If found correct by human experts, then this proof might
represent \emph{the world's first AI-generated DL decidability proof}.

We \emph{propose for the community's consideration:} \textbf{$\ALCIRCC{5}$
is decidable.}  We invite scrutiny and constructive feedback from the
DL community to challenge this claim. For the sake of scientific
progress and full transparency, we (the human-and-AI team) will
evaluate and respond to all \emph{constructive feedback and reviews}
that we receive on our project Github repository \cite{ALCIRCC5Repo}.
...
relations of the region connection calculus $\mathit{RCC}n$:
$\{\DR,\PO,\EQ,\PP,\PPI\}$ for $n=5$, and the eight
$\{\DC,\EC,\PO,\EQ,\TPP,\NTPP,\TPPI,\NTPPI\}$ for $n=8$.  Concepts are
ordinary \ALCI\ concepts where the role names $R$ range over the RCC
base relations (with $\exists \PP^{-}.C \equiv \exists \PPI.C$, etc.).
An interpretation assigns \emph{exactly one} base relation to every
pair $(x,y) \in \Delta^{\mathcal{I}} \times \Delta^{\mathcal{I}}$,
subject to JEPD, $\PP^{\mathcal{I}} = (\PPI^{\mathcal{I}})^{-1}$ (with
the $\mathit{RCC}8$ $\TPP/\TPPI$, $\NTPP/\NTPPI$ analogues), and the
$\mathit{RCC}n$ composition table $\comp$: for $(x,y) \in
R_1^{\mathcal{I}}$ and $(y,z) \in R_2^{\mathcal{I}}$, the label of
$(x,z)$ must lie in $\comp(R_1, R_2)$.  Under \emph{strong-EQ
semantics} \cite{Wessel2003}, $\EQ^{\mathcal{I}}$ is identity on
$\Delta^{\mathcal{I}}$. Each model is a \emph{complete directed graph}
on $\Delta^{\mathcal{I}}$ colour-coded by RCC$n$. The fundamental
...
$\ALCIRCC{5}$ and $\ALCIRCC{8}$ have neither the tree- nor the
finite-model property. From 2002 to 2003, I established that
$\ALCIRCC{1}, \ALCIRCC{2}, \ALCIRCC{3}$ are decidable
\cite{Wessel2002, Wessel2003} (deterministic or near-deterministic
tables); $\ALCIRCC{5}$ is $\PSPACE$-hard, and $\ALCIRCC{8}$ is
$\EXPTIME$-hard (via a $\mathit{TPPI}/\mathit{NTPPI}$ binary counter).
The decidability of $\ALCIRCC{5}$ and $\ALCIRCC{8}$ was left
open. I did identify the so-called \emph{co-incidence construction}
\cite{Wessel2003}, Def. 6, Figs. 10 and 11, that was later also used
by Lutz and Wolter to prove undecidability of $\LRCC{8}$
\cite{LutzWolter2006}; and adding a hybrid binder
(``$\downarrow$'') to $\ALCIRCC{8}$ makes the logic immediately
undecidable \cite{Wessel2003}.
...
\cite{Wessel2003}, Def. 6, Figs.\ 10 and 11: two $\TPP/\NTPP$-chains
...
construction} computationally with an RCC8 network enumerator written
in Lisp in 2002/2003. The reduction relies on two features unavailable
in \ALCIRCC{5}: (a) the $\mathit{RCC}8$ refinement $\TPP/\NTPP$
(\ALCIRCC{5}'s coarser $\PP$ collapses the two), and (b) the
topological interpretation in $\mathbb{R}^n$ -- it is the additional
structure in topological models that enforces co-incidence, not the
TBox axioms, and this is not available in the \emph{abstract} standard
DL models of the $\ALCIRCC{}$ family.

Nevertheless, we continued evaluating potential avenues for
undecidability of $\ALCIRCC{5}$ / $\ALCIRCC{8}$ -- but failed.
Instead, evidence indicating $\ALCIRCC{5}$ decidability started to
pile up! We thus attempted a series of standard tableau blocking
(e.g., double blocking \cite{DLHandbook2007}) and extended tableau
blocking ideas.  Of the 11 investigated decidability approaches, 3
were partially successful, 2 suggested novel reductions and decidable
fragments \cite{TwoTierQuotient}, and 6 (or 7) approaches failed
\cite{ALCIRCC5Outdated}, whereas the standard undecidability
approaches all failed \cite{ALCIRCC5Repo}.
...
\cite{DLHandbook2007}, terminates but is unsound for \ALCIRCC{5}: the
usual unravelling copies a blocked node's subtree under its blocker,
and composition of the copied RCC5 edges with the ambient network
frequently produces atomic relations that violate the composition
table or contradict locally satisfied universals.  Richer blocking
conditions -- triangle
blocking~\cite{TriangleBlocking,ResponseGPTBlocking},
sibling-interface blocking~\cite{SiblingCompleted}, rank-$d$
blocking~\cite{CompletenessExtraction} -- recover soundness by forcing
blocker and blockee to agree on enough surrounding context
(triples of RCC relations, sibling descriptors, or full rank-$d$
states), but strong enough variants either fail to terminate or
require an a priori bound on the context depth -- which is what the
decidability proof is trying to
establish~\cite{FWProof,PatchworkAugmentation,
  TypedPatchworkCounterexample,ALCIRCC5Outdated,StatusAfterFW}.
...
\cite{SiblingCompleted, GPT55Repaired}. A split-tree is a rooted tree
$T$ with a map $\mathit{orig}: T \to \Delta^{\mathcal{I}}$ into a DAG
of $\mathit{PPI}/\mathit{PP}$-edges on the original domain: each tree
node carries a Hintikka type, and whenever an element has several
...
descriptors for non-ancestor pairs.}  Weak-EQ semantics turns split
copies into ``EQ-mates'' that strong-EQ later collapses.  Because
$\mathit{PPI}/\mathit{PP}$ is a DAG (joins allowed, cycles forbidden),
the split-tree is well-defined and finite-quotientable; unravelling
replaces ``two nodes have the same type'' with ``two subtrees present
the same finite sibling-interface profile''.  A tableau-like calculus
was implemented -- the cover-tree tableau/reasoner~\cite{CoverTree,
  CoverTreeImpl}.

\textbf{The current decidability argument} rests on two papers.
(1)~GPT-5.4's \emph{v1 split-forest paper} \cite{SiblingCompleted}
proves the quotient-to-model (soundness) direction (Thms.~1.17--1.19).
(2)~GPT-5.5's \emph{repaired split-forest decidability proof}
\cite{GPT55Repaired} (31 pages after \emph{four rounds of adversarial
review}, May~2026, no-automata route) proves both directions in full
on rank-$d$ split-forest certificates against ten finite-checkable
validity conditions (V1)--(V10), with a verticalization discipline
(mosaic axiom~(M6)), port-indexed mate clusters, an abstract triple
graph $T_Q$, and a polynomial-width side interface $\mathrm{Side}(u)$;
the certificate space is bounded by $B(C_0) = 2^{2^{p(n)}}$, derived
grammar-first.  Together, (1)+(2) reduce $\ALCIRCC{5}$ concept
satisfiability to a finite enumeration.
...
\textbf{911 randomly generated \ALCIRCC{5} concepts}, a cover-tree
decomposition test confirming finite cover-tree models on the
\textbf{775 satisfiable concepts (100\%)} from that suite (12.2M
models enumerated), and a Round-2 adversarial audit of 400+ additional
concepts constructed by Opus 4.7. Moreover, the reasoner reproduced
\textbf{all 21 subsumptions} of the 18-concept GIS taxonomy from
\cite{Wessel2003}, Fig.~6 (and Appendix
Fig.~\ref{fig:gis-taxonomy}). Six Python work packages (WP1--WP6)
cross-check the finite combinatorial content of the repaired proof;
all PASS, no counterexample found.
...
loop. The agents~\cite{ClaudeOpus47,GPT54Pro,GPT55Repaired} are highly
technically capable by now: \emph{every technical artifact and aspect
of this project was created and managed by them,} including proofs
and their typesetting~\cite{SiblingCompleted,CoverTree,
  CompletenessExtraction,CoverTreeImpl,TwoTierQuotient,FWProof,
  TriangleBlocking,PatchworkAugmentation,
  TypedPatchworkCounterexample,ResponseGPTBlocking,StatusAfterFW,
  GPT55Repaired}, Python reasoners and test
suites~\cite{CoverTreeReasoner,QuasimodelReasoner,CyclicReasoner,
  ModelVerifier,DecompositionTest,StressTestCoverTree,
  StressTestCyclic,TestCyclicReasoner,GISTaxonomy}, and the repo
itself; see {\tt README.md} and {\tt CONVERSATION.md} on
\cite{ALCIRCC5Repo,ALCIRCC5Outdated}.
...
work, and GPT-5.5 reviewed the resulting proof four times, each round
identifying further gaps that were then closed in the next
revision~\cite{GPT55Repaired}.

Exploring 11 decidability approaches and 7 blocked undecidability
reductions in three weeks would have been impossible without agents:
they \emph{traverse a large search space far faster than a human can,}
descend more deeply into rabbit holes, and reorient quickly if
required.
...
self-sceptical working style.  They are also a \emph{treasure trove of
deep knowledge} in math and logic -- they explored a broader range of
techniques and approaches than I could have familiarized myself with
in many weeks of study.
...
assess their own output, and \emph{can self-correct.}

Standards will be required to \emph{ensure continued scientific
integrity}: a full, authentic {\tt CONVERSATION.md} log documenting
who -- human or agent -- contributed what might be a first step.
\emph{Transparency is non-negotiable; we hope the DL community will
decide to benefit from these emerging capabilities -- for the sake of
science.}
...
valid type set):
Given a model~$\mathcal{I} \models C_0$, construct a
\emph{generated split-forest certificate} for it.  This direction
was treated through three successive attempts.
\textit{Completeness extraction~v1}~\cite{CompletenessExtractionV1}
(April~2026) and \textit{v2.1}~\cite{CompletenessExtraction}
(May~2026) by Claude each closed obstacles that the prior version
had failed to handle, but each in turn was found defective under
adversarial review by GPT-5.5: v1 used a Hasse construction that
failed on infinite $\PP$-chains (e.g.\ $C_\uparrow \equiv
\exists\PP.\top \sqcap \forall\PP.\exists\PP.\top$) and contained a
witness conflation in the $C5$ triple-coherence proof; v2.1 fixed
both of those but then a second-round review identified
\textbf{eight structural gaps in v2.1} of which three were
load-bearing: profile-port equality contradicted vertical
separation on self-loops via the $M1{+}M7$ reflexivity chain; the
mate construction forced all mates to share a single semantic
parent, refuted by a two-mate stress formula
$C_{AB} \equiv A \sqcap B \sqcap \exists\PP.A \sqcap \exists\PP.B
\sqcap \forall\PP.(A\to\lnot B) \sqcap \forall\PP.(B\to\lnot A)$;
and the rootless orientation was incompatible with the
depth-from-$u_*$ projection used in the proof.

The \textbf{current best statement} of the
model-to-quotient direction is GPT-5.5's
\textit{repaired split-forest decidability
proof}~\cite{GPT55Repaired} (May~2026, 31~pages after a fourth
round of review, no-automata route), which removes all three
load-bearing defects.  The three structural fixes are:
\begin{enumerate}[label=(\arabic*)]
  \item \textbf{Occurrence-level equality:}
    $u \equiv_{\EQ} v \Leftrightarrow \mathrm{orig}(u) =
    \mathrm{orig}(v)$ on the split presentation, not a
    profile-port equivalence on the rank-$d$ quotient.  Self-loops
    on the parent map become trivially reflexive and the
    vertical-separation chain that defeats~(G1) vanishes.
  \item \textbf{Mates with different parents} via
    explicit port-equivalence on $\mathrm{Mate}_Q$, so a single
    rank-$d$ state can carry incomparable upward $\PP$
    witnesses---the $C_{AB}$ stress pattern is accommodated rather
    than ruled out.
  \item \textbf{Request-closed cycles} in place of
    $\omega$-acceptance.  Transitive $\PP/\PPI$ eventualities are
    discharged by finite-graph reachability plus a request-closure
    condition on the rank-$d$ quotient: no parity or B\"uchi
    automaton, no $\omega$-language machinery.  Equality of
    profile names along a blocking cycle never generates
    $\equiv_{\EQ}$; each lap of the cycle unfolds to fresh
    occurrences.
\end{enumerate}
The repaired proof formulates the model-to-quotient direction as
\textbf{ten finite-checkable validity conditions
(V1)--(V10)} on rank-$d$ quotient certificates and proves both
directions: every valid certificate unfolds to a strong abstract
RCC5 frame satisfying $C_0$ (soundness), and every satisfiable
$C_0$ admits a finite valid certificate (completeness).  After
four rounds of adversarial review by GPT-5.5, validity conditions
(V3), (V4), (V5), and (V9.Eb) are restated in port-indexed form
using context- and port-attached mate clusters $\mathrm{Mate}_Q(C,
s, p) \subseteq S \times \mathrm{Port}(s')$; mosaics carry a
verticalization stratum $\mathrm{Vrt}(M)$ (axiom (M6)) that
separates vertical positions (source $+$ EQ mates $+$ selected
$\PP/\PPI$ witnesses) from the sibling frontier (DR/PO witnesses
$+$ boundary nodes); finite support closure is checked on an
explicit abstract triple graph $T_Q$ with cardinality bounds
$|\mathrm{Pos}_Q| \leq 2|S|\cdot(n{+}1)\cdot|\mathrm{Ctx}_Q|$ and
$|\mathrm{Trip}_Q| \leq |\mathrm{Pos}_Q|^3 \cdot 125$; and the
side interface $\mathrm{Side}(u)$ has an explicit five-item
composition with polynomial width bound $m(n) = O(n^3)$.  The
certificate space is bounded by a coarse
$B(C_0) = 2^{2^{p(n)}}$ for a polynomial $p$, derived
\emph{grammar-first} from these component bounds, and validity is
finite-graph checkable, so $\ALCIRCC{5}$ concept satisfiability
reduces to a finite enumeration.

\medskip\noindent\textbf{Status of v1, v2.1, and the repaired
proof.}  v1 and v2.1 are retained in the repository for the audit
trail but are explicitly \emph{marked superseded}: v1 by the
sibling-interface v2 delta~\cite{SiblingInterfaceV2} that already
fixed the Def~1.5 / Def~1.7 / Thm~1.20-sketch defects, and v2.1 by
the GPT-5.5 round-2 review.  The repaired proof~\cite{GPT55Repaired}
is the single document that now carries the decidability claim on
the model-to-quotient side; its V1--V10 validity conditions are
designed to be sufficient inputs for the soundness chain in the v1
sibling-interface paper~\cite{SiblingCompleted}.

\medskip\noindent\textbf{Computational verification of the
combinatorial claims.}
With the proof side stable, Claude's contribution shifted to
\emph{cross-checking the repaired proof's finite-combinatorial
claims} per GPT-5.5's verification-recommendations document.  Six
Python work packages (WP1--WP6) are all \textbf{PASS}: brute-force
enumeration confirms GPT-5.5's RCC5 composition table in all 25
cells (WP1); a vertical-fragment validity-condition checker accepts
$C_\uparrow$ via parent self-loops, accepts $C_{AB}$ via two mates
with different parents, rejects the v2.1-style single-parent
$C_{AB}$ attempt, and rejects two genuine UNSAT concepts on the
soundness side (WP2, covering V1/V3/V4/V5/V9/V10); a small-model
SAT/UNSAT oracle exhibits a concrete 3-element model for $C_{AB}$
with two incomparable $\PP$-superparts (WP3); $C_{AB}$ and
request-closed cycle tests fold into WP2/WP3 (WP4, WP5); and a
mosaic enumeration confirms (M1)--(M5) and the support-closure
axioms on small mosaics, including the patchwork-on-triangles
property for RCC5 (WP6).  No counterexample to any combinatorial
claim of the repaired proof was found.
...
\begin{enumerate}
  \item The \emph{v1 split-forest paper}~\cite{SiblingCompleted}
    ...
    carry over verbatim.
  \item The \emph{GPT-5.5 repaired
    proof}~\cite{GPT55Repaired} (May~2026, 31~pages after four
    rounds of review, no-automata route): formulates rank-$d$
    split-forest certificates and ten finite-checkable validity
    conditions (V1)--(V10), and proves both directions in their
    entirety, including \textbf{completeness} (every satisfiable
    $C_0$ admits a valid finite certificate).  The three
    structural ingredients that make the proof go
    through---occurrence-level equality, mates with different
    parents, and request-closed cycles---resolve the gaps that
    defeated Claude's earlier completeness-extraction attempts
    (v1, v2.1; both now marked superseded).  The fourth-review
    pass added the verticalization discipline (M6), port-indexed
    mate clusters, the abstract triple graph $T_Q$, and the
    explicit side-interface construction; all six load-bearing
    gaps (G1)--(G6) and five smaller issues from the fourth
    review are closed in the main proof.
\end{enumerate}
Together, (1) and (2) establish the biconditional displayed
above.  The certificate space is bounded by
$B(C_0) = 2^{2^{p(n)}}$, validity is finite-graph checkable, and
...
combinatorial content of the repaired proof:  RCC5 composition
...
search (WP6).  All six \textbf{PASS}; no counterexample to a
combinatorial claim of the repaired proof was found.
...
valid-certificate conditions (V1)--(V10) has not been written.
...
\newblock A repaired split-forest decidability proof for
  $\ALCIRCC{5}$ (no-automata route): occurrence-level equality,
  mates with different parents, request-closed cycles.
\newblock Technical report, May~2026.
