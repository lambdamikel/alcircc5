...
\begin{abstract}
% TODO(Michael): extended-abstract abstract, to be written by hand.
...
$\ALCIRCC{5}$---an open problem since Wessel~(2002/2003). The full
survey, proof sketch, implementation report, and the two-round
adversarial review log are reproduced verbatim in the appendix. The
body of this note focuses on the \emph{process}: what the human and
the AI agents contributed, which strategies worked, which failed, and
what the episode suggests about the role of agentic AI in description
logic research.
\end{abstract}
...
% TODO(Michael): Strategies --- the methodology of the
% collaboration: multiple parallel reasoning approaches, iterative
% formalisation via LaTeX, executable implementations as independent
% oracles, adversarial review by a second AI agent, the role of
% cross-validation with deliberate blind-spot awareness, and the
% discipline of withdrawing claims when counterexamples are found.
...
% TODO(Michael): Summary and implications --- what the episode
% suggests about human-and-AI collaboration in formal-methods
% research more broadly, residual caveats (testing vs.\ proof,
% peer review still warranted), and an outlook for the DL
% community.
...
both of those but then a second-round review identified \textbf{eight
structural gaps (G1)--(G8)} of which three were load-bearing:
(G1)~profile-port equality contradicted vertical separation on
self-loops via the $M1{+}M7$ reflexivity chain; (G3)~the mate
construction forced all mates to share a single semantic parent,
refuted by a two-mate stress formula
$C_{AB} \equiv A \sqcap B \sqcap \exists\PP.A \sqcap \exists\PP.B
\sqcap \forall\PP.(A\to\lnot B) \sqcap \forall\PP.(B\to\lnot A)$;
(G4)~the rootless orientation was incompatible with the
...
proof}~\cite{GPT55Repaired} (May~2026, 16~pages, no-automata
route), which removes all three load-bearing defects.  The three
...
$C_0$ admits a finite valid certificate (completeness).  The
certificate space is bounded by a coarse
$B(C_0) = 2^{2^{p(n)}}$ for a polynomial $p$, and validity is
...
\begin{enumerate}
  ...
  \item The \emph{GPT-5.5 repaired
    proof}~\cite{GPT55Repaired} (May~2026, 16~pages, no-automata
    route): formulates rank-$d$ split-forest certificates and ten
    finite-checkable validity conditions (V1)--(V10), and proves
    both directions in their entirety, including
    \textbf{completeness} (every satisfiable $C_0$ admits a
    valid finite certificate).  The three structural ingredients
    that make the proof go through---occurrence-level equality,
    mates with different parents, and request-closed
    cycles---resolve the gaps that defeated Claude's earlier
    completeness-extraction attempts (v1, v2.1; both now marked
    superseded).
\end{enumerate}
