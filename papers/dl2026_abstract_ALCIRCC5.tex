...
(May~2026, 31~pages, v2.1) repairs both v1 defects via seven concrete
...
\medskip\noindent\textbf{Round-2 update (May~12,~2026).}
GPT-5.5 produced a second formal review of v2.1 and identified
eight structural gaps, three of which are load-bearing
(profile-port equality contradicts vertical separation on
self-loops; the mate construction forces all mates to share a
parent; the rootless orientation is incompatible with the
depth-from-$u_*$ projection).  GPT-5.5's repaired
proof~\cite{GPT55Repaired} fixes all three via occurrence-level
equality ($u \equiv_{\EQ} v \Leftrightarrow \mathrm{orig}(u) =
\mathrm{orig}(v)$), mates with different parents, and
request-closed cycles in place of $\omega$-acceptance.  It states
ten finite-checkable validity conditions (V1)--(V10) on rank-$d$
quotients with coarse bound $B(C_0) = 2^{2^{p(n)}}$.  v2.1 is
\emph{marked superseded}; the GPT-5.5 repaired proof is the
\emph{current best statement}; Claude's contribution shifts to
computational verification (WP1--WP6, L1--L7).  WP1 (RCC5
composition table by exhaustive subset enumeration) confirms the
repaired proof's table in all 25 cells.
