...
experts}; they are published as a discussion piece, and the
completeness direction of the decidability argument has a known
open gap (\S\ref{ssec:round8-review}).  The claims should not be
taken as established results unless independently verified.
...
The decidability argument is most developed in the GPT-5.5
\emph{round-8 saturated-side-context split-forest
...
assertions.  The history of how the argument reached its current
form is collected in~\S\ref{sec:history-of-attempts} below.

\paragraph{Status: an open completeness gap.}
A subsequent cold review of round-8 (Claude Opus~4.8,
\S\ref{sec:history-of-attempts}) found that the cross side/tree
exhaustion lemma closes the side/tree gap only on the
\emph{finite, depth-bounded} boundary $B_k(u)$, whereas a
$\DR/\PO$ side witness can be RCC5-composition forced to be a
proper part of an \emph{infinite} pumped containment tower.  The
concrete satisfiable witness is
...
to be a proper part of \emph{every} tower node.  The bounded
saturated side context cannot carry the forced $\PP/\PPI$ labels on
the tail, so the round-8 completeness construction produces an
invalid certificate for a satisfiable concept (machine-checked).
This is a genuine gap in the completeness proof of medium--high
severity.  The decidability \emph{theorem} is most likely still
true --- an alternative presentation, placing $w$ as a true
vertical descendant of the tower, appears to certify $C_0'$ --- and
a proposed repair (\emph{composition-forced verticalization}:\@
splice the forced witness into the cover relation $\Eup$ at a
bounded threshold height so equality-aware reachability carries the
label up the tail) is recorded in~\S\ref{sec:history-of-attempts},
with machine-checked linchpins but open bookkeeping obligations and
no external ratification.  We therefore state the result honestly as
\textbf{strongly supported but not, as written, proven}:\@ the
soundness direction is sound, the architecture is sound, but the
completeness construction has a known open gap.  The remainder of
this subsection states the construction at the level the manuscript
develops it.
...
finite certificate) --- \emph{this is the direction with the open
gap above}.  A satisfying interpretation is thinned to a
...
presentation, not merely from abstract position symbols.  The
known gap (D-1) is that the split-cover construction files every
$\DR/\PO$ witness in a bounded side context, which cannot record
the labels forced by composition with an unbounded ancestor tower;
the proposed repair re-routes such a witness into the vertical
backbone (\S\ref{sec:history-of-attempts}).
...
the most developed statement of the architecture; round 7 is its
...
are inherited rather than re-derived.  The repair has open
bookkeeping obligations (excision coherence under replacement,
simultaneous splices, overlap-amalgam closure, finiteness
constants) and has not been ratified by GPT-5.5 or by human
experts; we record it as a candidate, not a settled fix.  Until it
(or a GPT-5.5 round-9 fix) is ratified, the completeness direction
has a known open gap and the theorem's status is
\emph{strongly supported, not proven}.
...
this writing the argument is \emph{strongly supported but not, as
written, proven}:\@ the completeness direction has a known open
gap (D-1, \S\ref{ssec:round8-review}) with a proposed-but-unratified
repair.
...
\begin{enumerate}
  \item The \emph{v1 split-forest paper}~\cite{SiblingCompleted}
    ...
    carry over verbatim.
  \item The \emph{GPT-5.5 round-8 split-forest
    proof}~\cite{GPT55Round8Sketch} (May~2026, 15-page sketch)
    with detailed companion~\cite{GPT55Round8Expanded} (30~pages)
    and parity-tree-automata witness~\cite{GPT55AutomataRoute}
    (21~pages): formulates finite occurrence-sensitive
    split-forest certificates with twelve finite-checkable
    validity conditions (V1)--(V12), establishes \textbf{soundness}
    in full, and \emph{aims} at \textbf{completeness} (every
    satisfiable $C_0$ admits a valid finite certificate).  The
    central architectural move (introduced in round 7, preserved
    in round 8) is that finite labels live on occurrence-sensitive
    \emph{pair shapes}
    ...
    $\PP/\PPI$ eventualities by request-closed cycles.
    \emph{However}, the completeness construction has a known open
    gap (D-1, \S\ref{ssec:round8-review}):\@ a $\DR/\PO$ witness
    composition-forced into an infinite pumped tower is not
    correctly routed, so the construction as written can emit an
    invalid certificate for a satisfiable concept; a proposed
    repair (composition-forced verticalization,
    \cite{Opus48Round8Repair}) is unratified.  Round-8 supersedes
    Claude's earlier round-5/round-6 manuscripts and the round-2
    31-page repaired proof~\cite{GPT55Repaired}, all retained as
    the historical audit trail
    (see~\texttt{OUTDATED.md}~\cite{ALCIRCC5Outdated}).
\end{enumerate}
Subject to the open D-1 gap, (1) and (2) aim to establish the
biconditional displayed above; the soundness direction
($\Leftarrow$) is established unconditionally.  The certificate
...
the long context can find.  This was confirmed three times on this
...
instance also supplied a candidate repair).  The reviewer-was-cold
...
intuition.  But the round-8 proof is qualitatively different from
...
was confirmed three times (April~2026, the cover-tree
...
completeness gap and proposed a repair).  The cost is one
session; the pay-off is a list of concrete defects --- and, in
the third instance, a candidate repair --- that the long-context
author can then ratify or replace.  We recommend cold review as a
...
sobering corollary, visible across all three instances, is that
\emph{each} cold review found a genuine defect in a manuscript its
authors believed was finished, including two successive rounds
each advertised as ``canonical'':\@ the moment to trust a
``finished'' claim is after a cold review finds \emph{nothing},
which has not yet happened here.  The pattern is operationalisable
...
not been peer-reviewed by human experts, and as of this writing
the round-8 completeness construction has a known open gap (D-1,
\S\ref{ssec:round8-review}) with a proposed-but-unratified repair.
The decidability theorem is strongly supported but not, as
written, proven.  The meta-research point---\emph{that this kind
...
\item formalise the mathematical proofs (split-forest semantics,
  completeness extraction across eight review rounds, the round-7
  pair-shape / incidence-tag architecture, and the round-8
  saturated-side-context construction);
...
\item conduct computational verification campaigns (911-concept
  ...
  stress-concept verifications, the round-8 composition-arithmetic
  checks);
...
\newblock Repairing the round-8 completeness construction for
  ...
  threshold height; machine-checked threshold lemmas; open
  bookkeeping obligations O1--O4; unratified.
\newblock Technical report, May~2026.
