...
\subsection{What remains open, and a final assessment}
\label{ssec:final}

\paragraph{Open.}
\begin{itemize}
  \item \textbf{Decidability itself (F6).}  Whether some concept forces
    unbounded live width---equivalently, an unbounded rigid horizontal
    crowd, equivalently unbounded identity-selector minors.  This is the
    24-year-old open problem, now cleanly stated.  W2$'$ folds into it.
  ...
  \item \textbf{Human verification.}  \emph{None of these proofs have
    been verified by human domain experts.}  The AI-generated results
    should be treated as conjectures supported by machine checking and
    computational evidence, not as established theorems---with the
    kernel-certified soundness pipeline and the certified normal form the
    exceptions, and those still unreviewed by humans.
\end{itemize}

\paragraph{A final honest assessment.}
