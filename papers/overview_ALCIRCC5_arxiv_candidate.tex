...
\emph{round-9 forced-verticalization split-forest
proof}~\cite{GPT55Round9Sketch} (May~2026, 16-page sketch,
no-automata route), with detailed
companion~\cite{GPT55Round9Expanded} (33~pages) and an independent
parity-tree-automata witness~\cite{GPT55AutomataRoute} (21~pages).
Round-9 preserves the round-7 architectural move (incidence-tagged
pair shapes) and the round-8 saturated side context $S_k(u)$, and
adds \emph{composition-forced verticalization} to close the one
completeness gap (D-1) found by a cold Claude Opus~4.8 review of
round-8:\@ a $\DR/\PO$ side witness whose ancestor trace has a
$\PPI$-tail is spliced into the generated cover relation $\Eup$ at a
bounded threshold, so equality-aware vertical reachability --- not a
bounded side context --- carries the forced containment label up the
infinite tail.  The history of how the argument reached its current
form is collected in~\S\ref{sec:history-of-attempts} below.

\paragraph{Status: a repaired gap, strongly supported, not
mechanically certified.}
The completeness gap (D-1) that a cold Claude Opus~4.8 review found
in round-8 is closed at the manuscript level in round-9.  The gap
...
is repaired at the manuscript level, but the repair (like the rest
of the manuscript) has not itself been independently cold-reviewed
or machine-certified.  GPT-5.5 names the bounded-threshold
...
finite-index replacement argument.  GPT-5.5 explicitly characterises
...
Round-9 has not itself been independently cold-reviewed in turn; we
therefore state the result as \emph{strongly supported, not
mechanically certified}.  Round-9 is the most developed statement of
the architecture; round-8 is its immediate predecessor.
...
mechanically certified}:\@ the one known completeness gap (D-1,
\S\ref{ssec:round8-review}) is repaired at the manuscript level in
round-9 (\S\ref{ssec:round9}), but neither that repair nor the
surrounding manuscript has been independently cold-reviewed in turn
or machine-certified.
...
\begin{enumerate}
  \item The \emph{v1 split-forest paper}~\cite{SiblingCompleted}
    ...
    carry over verbatim.
  \item The \emph{GPT-5.5 round-9 split-forest
    proof}~\cite{GPT55Round9Sketch} (May~2026, 16-page sketch)
    with detailed companion~\cite{GPT55Round9Expanded} (33~pages)
...
and integrated as round-9).  The cost is one session; the pay-off
is a list of concrete defects --- and, in the third instance, a
candidate repair --- that the long-context author can then ratify or
replace (round-9 is exactly such a ratification).  We recommend cold review as a
...
not been peer-reviewed by human experts; the one completeness gap a
cold review found (D-1, \S\ref{ssec:round8-review}) is repaired at
the manuscript level in round-9 (\S\ref{ssec:round9}), but that
repair has not itself been independently cold-reviewed or
machine-certified.  The decidability theorem is strongly supported
but not mechanically certified.  The meta-research point---\emph{that this kind
