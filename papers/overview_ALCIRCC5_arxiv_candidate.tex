...
%% ====================================================================
\section{History of Attempts (April--July 2026)}
...
%% ====================================================================
\subsection{From automata to Lean: how the argument settled}
\label{ssec:settled}

By the end of May~2026 the decidability argument had reached its
automata form (Theorems~A/B/C, \S\ref{ssec:soundness-completeness}), and
we regarded that as a natural consolidation point.  It did not remain
one.  A materially stronger reviewer became available (Claude
Fable~5, a capability tier above the round-12 author), and the
adversarial-review series resumed: across July~2026 the work went through
a further sequence of reviews and repairs, migrated its load-bearing
soundness argument into the Lean~4 proof assistant, and---most
importantly---localized the entire remaining difficulty into a single,
well-posed lemma.  Rather than recount that as a round-by-round
chronology (the blow-by-blow ledger lives in the repository's
\texttt{CONVERSATION.md}, and the formalization history in
\texttt{LEAN.md}), we describe the \emph{routes} that were investigated
and where each one led.  This is the honest shape of the project as it
now stands.

\subsection{The routes investigated}
\label{ssec:routes}

Every route shares one foundation and differs only in how it tries to
make a \emph{decision procedure} rigorous on top of it.

\paragraph{The shared foundation: the split-forest normal form (Theorem~A).}
Every satisfiable concept admits a witness-generated, tree-like
\emph{split-forest} model---obtained by witness thinning, generated
containment covers, split copies for incomparable superparts,
side-interface mosaics, and a typed-$\EQ$ quotient.  This semantic normal
form is the one component that has \emph{never} broken: it was found
sound by every cold review across the project.  All of the routes below
consume it rather than replace it; they differ in how they turn ``a
model of this shape exists'' into ``an algorithm can decide whether one
exists.''

\paragraph{Route 1 --- the cover-tree tableau (operational).}
Claude's cover-tree tableau (\S\ref{sec:cover-tree}) is a concrete,
implemented decision procedure with exact-occurrence blocking.  It is
trustworthy on satisfiable inputs and has a known blind spot on certain
unsatisfiable inputs (the strong-$\EQ$ \PO-loop pattern).  It is the
project's operational oracle and cross-validation tool---not the
decidability proof, but the thing every proposed proof was checked
against.

\paragraph{Route 2 --- the no-automata split-forest certificate (rounds~2--10).}
A hand-built finite certificate for satisfiability, validated by local
checks over the split forest.  Across rounds~2--10 it repeatedly reduced
to automaton-shaped machinery it could not fully discharge:
request-closed cycles behave like B\"uchi/parity acceptance, and ``finite
product-state exhaustion'' behaves like product-automaton reachability.
Successive cold reviews found successive defects---an ancestor-tower gap,
then the dual descendant tower, then that finite-checkability was
asserted rather than proved.  The thread converged, honestly, onto an
automaton.

\paragraph{Route 3 --- the automata split-forest proof (rounds~11--12).}
This made the convergence explicit and became the canonical statement for
a time: Theorem~A (normal form) $\to$ Theorem~B (a \emph{finite
abstraction} captures saturated split forests up to satisfiability) $\to$
Theorem~C (a two-way alternating parity tree automaton accepts exactly
the valid abstractions, and its emptiness is decidable).  Theorem~B is
the single load-bearing keystone.  Cold reviews (the 5th and 6th) found
its global-coherence step \emph{assumed, not proved}; round~12 rebuilt
the abstraction on \emph{patchwork bags} so that global $\mathrm{RCC5}$
consistency follows from the citable patchwork property of $\mathrm{RCC5}$
(Renz--Nebel), with eventualities decided by two-way automaton emptiness
(Vardi).  A 7th (hot) review then found a residual Shadow-Amalgamation
gap in the truth lemma.  The keystone had become smaller and rested on
standard external theorems, but it was still the keystone.

\paragraph{Route 4 --- the cluster-quasimodel decision layer (rounds~15--18).}
Following a cold review's recommendation, the tree automaton was dropped
in favour of a mosaic / type-elimination procedure on the infinitary
abstraction directly: a finite \emph{catalogue} of interface patterns
carrying \emph{steering functions}, with one-step demand fulfilment (no
promissory witnesses, hence no parity/eventuality condition).  This is
the route on which the whole difficulty was finally \emph{localized}.
Once the construction was written explicitly, everything reduced to two
named lemmas: \textbf{F6}, a computable bound on the \emph{live width} of
the certificate (the genuine, non-shadow branching an interface must
record), and \textbf{W2$'$}, uniformization of the extraction (that
locally realizable glue steps factor through the finite catalogue's
states).  Cold reviews sharpened the steering condition from pointwise to
joint and pinned down the transport rule; no new mathematical obstacle
beyond F6/W2$'$ appeared.

\paragraph{Route 5 --- the Lean formalization (rounds~19--30).}
The certificate transport, S-layer, catalogue, and logic layers were
ported into Lean~4 (core, no \texttt{mathlib}).  The result is a
kernel-checked development (\texttt{formal/Round19Transport.lean}, zero
\texttt{sorry}) in which the \emph{soundness} pipeline---certificate
syntax $\to$ a genuine $\mathrm{RCC5}$ model of the logic---and the
\emph{faithfulness} of the Hintikka abstraction (the type-level
certificate loses nothing relative to models) are proved by the kernel.
On top of it sits a \emph{decision-grade, non-oracular} reduction: a
fixed Boolean checker of first-order finite certificate data, proved
sound, from which \texttt{Decidable} satisfiability follows by finite
search---leaving exactly one open premise, a computable complete
enumeration of bounded certificates, which is precisely F6 $\wedge$
W2$'$.  Two cold reviews of the Lean confirmed there is no
\emph{soundness} defect and found only completeness-interface gaps, which
subsequent rounds closed.  The concrete history is in \texttt{LEAN.md}.

\paragraph{The reduction to one keystone, and the recent attacks.}
All five routes bottom out at the same place: \emph{does some concept
force unbounded live width?}  Two independent cold research attacks then
sharpened this question.  Every naive counterexample---shell half-graphs,
prefix pumps, single-chain \PO-gaps---collapses to a finite catalogue, so
raw pairwise non-mergeability is the wrong invariant; the sharp
obstruction is a forced, unbounded family of \emph{identity-selector
minors} (private matching of unbounded hitting number).  W2$'$ was shown
to \emph{fold into} F6 (its coarse form is false, but repairable by a
state refinement whose size F6 itself bounds).  A regular-cover
reframing---finite generators whose infinite unfoldings are the
models---gave a complete \emph{local} algebra, of which one durable
piece was harvested and machine-\emph{certified}: the $\mathrm{RCC5}$
\emph{normal form} (every strong-$\EQ$ $\mathrm{RCC5}$ network is an
ordered-disjoint structure---a strict partial order for $\PP$ plus a
downward-closed disjointness for $\DR$, with $\PO$ residual).  But the
regular-cover route's own keystone is F6 relabelled, and its authors
concede the remainder ``cannot be a purely local $\mathrm{RCC5}$
composition problem.''

\paragraph{The knife's edge.}
F6 is not merely the decidability keystone; it is \emph{the same fact} as
the obstruction to proving the problem \emph{undecidable}.  To force
undecidability one would force a rigid coordinate grid, and the abstract
composition table cannot force the coincidence condition
(``east-of-north $=$ north-of-east'')---exactly because it cannot pin a
horizontal relation to a single value.  Turn that looseness into a bound
and one obtains decidability; circumvent it and force a grid and one
obtains undecidability.  The two open directions are one question seen
from two sides, which is why neither has been settled in over twenty
years.
...
Decidability of $\ALCIRCC{5}$ remains \emph{open}.  What the project has
produced, and what is genuinely certified, is the following.

\begin{itemize}
  \item \textbf{The sound semantic foundation.}  The split-forest normal
    form (Theorem~A)~\cite{SiblingCompleted,SiblingInterfaceV2}: every
    satisfiable concept has a tree-like split-forest model.  Found sound
    by every cold review; never broken.
  \item \textbf{A machine-checked soundness pipeline and
    abstraction-completeness.}  In Lean~4 (zero \texttt{sorry}): a valid
    finite certificate unfolds to a genuine $\mathrm{RCC5}$ model of the
    concept, and the Hintikka abstraction is faithful (satisfiability and
    Hintikka-realizability coincide).  The soundness half of the
    decidability equivalence is thus kernel-certified.
  \item \textbf{A machine-checked, non-oracular decision-grade
    reduction.}  A fixed Boolean checker of first-order finite
    certificate data, proved sound, yielding \texttt{Decidable}
    satisfiability by finite search modulo a single open premise---which
    is exactly F6 $\wedge$ W2$'$.
  \item \textbf{The problem reduced to one well-posed keystone.}  The
    entire remaining difficulty is F6 (bounded live width), sharpened to
    the identity-selector-minor dichotomy, with W2$'$ folded in.  This is
    a single lemma a future solver can aim at, not a diffuse open
    question.
  \item \textbf{A certified local theory and an unconditionally decidable
    fragment.}  The $\mathrm{RCC5}$ normal form is machine-certified
    (\texttt{formal/RCC5NormalForm.lean}).  The
    \emph{$\forall\PO$-free} fragment---concepts with no $\forall\PO$
    subformula, but \emph{retaining} $\exists\PO$, $\forall\DR$,
    $\exists\DR$, and all vertical part-of modalities, hence still a
    genuinely expressive spatial fragment---is unconditionally
    decidable~\cite{TwoTierQuotient}: it is the syntactic case of the
    two-tier PO-coherent fragment in which $\PO$---the only relation the
    composition table cannot pin to a single value---is the sole source
    of difficulty, and is therefore harmless.
  \item \textbf{The undecidability routes shown blocked}
    (\S\ref{sec:undecidability}), with the coincidence obstruction identified
    as the fact shared with the decidability keystone.
  \item \textbf{A methodology.}  Adversarial cold review as a standard
    step, and a division of labour between models, both discussed below.
\end{itemize}

The formalization details (the round-19--30 progression, the two cold
reviews of the Lean, the exact theorem names and axiom footprints) are
recorded in \texttt{LEAN.md}; \texttt{README.md} gives the current-status
summary.

\subsection{Dead ends}
\label{ssec:deadends}

Several routes and proof strategies were pursued and abandoned.  Each is
a genuine negative result about the problem, not merely a failed attempt.

\begin{itemize}
  \item \textbf{A hand certificate for satisfiability.}  The no-automata
    split-forest thread (rounds~2--10) repeatedly reduced to
    automaton-shaped machinery it could not discharge by hand
    (request-closed cycles behave like B\"uchi/parity acceptance; ``finite
    product-state exhaustion'' like product-automaton reachability); it
    converged, honestly, onto an automaton.  The lesson---the keystones a
    hand proof kept rediscovering (finite checkability, simultaneous
    eventuality fulfilment) are exactly what an automaton discharges by
    construction---is why the automata route exists.
  \item \textbf{Route-dependent global coherence.}  The round-11
    interface-transformer abstraction derived a globally consistent
    labelling of an infinite tree from purely local checks, assuming a
    confluence law it never proved; a cold review broke it.  It was
    replaced by resting global consistency on the citable $\mathrm{RCC5}$
    patchwork property.
  \item \textbf{Forcing a counterexample by raw crowds.}  Every naive
    attempt to force unbounded width---shell half-graphs, prefix pumps,
    single-chain $\PO$-gaps---collapses to a finite catalogue.  Raw
    pairwise non-mergeability is the wrong invariant; the
    certificate-relevant one is unbounded row/profile inequivalence
    surviving bounded refinement (identity-selector minors).
  \item \textbf{Proving F6 by local compression.}  An algebra-only
    ``compress the support tree'' proof of F6 is refuted: arbitrary
    jointly-realizable $\mathrm{RCC5}$ glue steps can have unbounded
    separator cover (machine-checked), so any F6 proof must use that the
    bad steps are \emph{not $\ALCIRCC{5}$-forceable}, not merely
    $\mathrm{RCC5}$-consistent.
  \item \textbf{Ramsey-type combinatorics.}  Wrong quantifier order:
    Ramsey theory gives unavoidable structure in a large fixed object,
    whereas F6 concerns structure a formula can \emph{force}.  The
    substrate is Ramsey-friendly (large irreducible $\{\DR,\PO\}$ crowds
    exist freely), which is exactly why Ramsey does not fit.
  \item \textbf{Coarse uniformization (W2$'$).}  Its coarse form is false:
    two occurrences of identical coarse state can be forced apart by their
    relation to a third occurrence outside the interface.  It is repaired
    by a bounded state refinement---and that refinement is bounded by F6,
    so W2$'$ folds into F6 rather than standing beside it.
  \item \textbf{The regular-cover reframing as a shortcut.}  A finer,
    cleaner vocabulary (finite generators, infinite unfoldings) that
    subsumes the width view and exhausts the local algebra---but its own
    keystone is F6 relabelled, and it concedes the remainder ``cannot be
    a purely local $\mathrm{RCC5}$ composition problem.''  A better map,
    not a shorter path.
\end{itemize}

\subsection{Missing hard pieces}
\label{ssec:missing}

\begin{itemize}
  \item \textbf{Decidability itself (F6)}---the keystone.  Whether some
    concept forces unbounded live width---equivalently, an unbounded rigid
    horizontal crowd, equivalently unbounded identity-selector minors.
    This is the 24-year-old open problem, now cleanly stated; W2$'$ folds
    into it.  It is provably the \emph{same} fact that blocks a proof of
    undecidability.
  \item \textbf{The converse of the $\mathrm{RCC5}$ normal form.}  The
    forward direction (every $\mathrm{RCC5}$ network is ordered-disjoint)
    is certified; the converse (ordered-disjoint structures realize
    $\mathrm{RCC5}$ closure) is exhaustively machine-checked at small size
    but not yet formalized.
  \item \textbf{A fully wired, human-reviewed decision procedure.}  The
    Lean decision-grade reduction is non-oracular and sound, but its
    completeness premise is exactly F6; and no part of the development has
    been reviewed by human domain experts.
  \item \textbf{Exact complexity.}  The PO-coherent fragment has a
    doubly exponential quotient bound (2-$\EXPTIME$)~\cite{TwoTierQuotient};
    a matching $\EXPTIME$ upper bound for it is not established, and no
    complexity bound is claimed for the full logic.
  \item \textbf{$\ALCIRCC{8}$.}  RCC8 also has the patchwork property, so
    the approach should extend; the $\mathrm{TPP}/\mathrm{NTPP}$
    distinction may require a finer status partition.  Not worked out.
\end{itemize}

\subsection{Final assessment}
\label{ssec:final}

After roughly thirty repair rounds and fifteen adversarial reviews, the
project has reached a genuine \emph{local optimum}.  The local algebra of
$\mathrm{RCC5}$ is exhausted and, in one case, certified; the soundness
side of the decidability argument is machine-checked; and the entire
remaining difficulty has been compressed into a single well-posed lemma
(F6) which is provably the \emph{same} fact that blocks a proof of
undecidability.  Each further round now buys a smaller delta---a cleaner
restatement of the same keystone, or a more specific local lemma---rather
than movement on the summit.  The honest one-line status is unchanged
from the project's standing label: \emph{strongly supported, not
certified}, with the soundness pipeline, the abstraction faithfulness,
and the $\mathrm{RCC5}$ normal form the certified exceptions.

We therefore offer the work in the shape it has actually taken: not a
resolution of a two-decade open problem, but that problem
\emph{reduced}---honestly, checkably, and with a machine-checked
surround---to its irreducible core, together with a map of why that core
is hard and a named toolbox for attacking it.  If the question is ever
settled, we expect the resolution to begin from a map of this shape.
