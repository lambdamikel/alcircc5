...
\title{A Round-9 Repaired Split-Forest Decidability Proof for \texorpdfstring{$\mathcal{ALCI}_{\mathrm{RCC5}}$}{ALCI-RCC5} Without Automata: Forced Verticalization}
...
\begin{abstract}
This note gives a single coherent version of the repaired split-forest proof for decidability of satisfiability of \(\ALCIRCC\) concepts over abstract complete atomic RCC5 frames.  The proof keeps the original architecture: witness thinning, generated containment covers, split copies for several selected containment parents, finite side interfaces, support-closed mosaics, typed equality quotienting, and finite regular certificates.  It does not use Büchi or parity automata.  Instead it uses direct finite-index blocking with explicit request-closed cycles.  The earlier blocking/equality repair is retained: finite labels are indexed by occurrence-sensitive pair shapes, so two occurrences in different laps of a blocking cycle may have the same profile and local position symbol without being \(\EQ\)-identified.  The new round-9 repair addresses the unbounded side/tree defect: a nominal \(\DR/\PO\) side witness that RCC5 composition and ancestor universals force to become a proper part of an infinite pumped tower is spliced into the generated containment backbone at a bounded threshold.  Hence the forced tail relation is carried by equality-aware vertical reachability rather than by an impossible bounded side context.
\end{abstract}
...
For selected \(\DR\) or \(\PO\) witnesses, first test for composition-forced verticalization.  If the witness is forced to be a proper part of an unbounded ancestor tower, splice an occurrence of the witness into the generated cover relation at the bounded threshold and keep the source--witness edge as its inherited residual \(\DR/\PO\) relation.  Otherwise attach a saturated side context containing the source occurrence, its exposed vertical boundary, the witness occurrence, nested side witnesses of smaller modal depth, and all boundary/side labels.  If a side witness is \(\PP\)- or \(\PPI\)-comparable with another side witness or with a finite vertical boundary occurrence of the source, include the necessary local vertical mini-context in the side interface rather than forcing the pair into the residual sibling frontier.  Thus residual frontier pairs are only \(\DR\) or \(\PO\) after equality closure, finite containment saturation, and forced verticalization of unbounded tower interactions.
...
\begin{lemma}[Composition-forced verticalization]\label{lem:forced-verticalization-compact}
Let \(u\) have a selected \(\DR/\PO\) witness \(w\), and let
\[
   u<a_1<a_2<\cdots
\]
be a generated proper-superpart tower above \(u\).  Put \(r_m=L(a_m,w)\).  Then each \(r_m\in\{\DR,\PO,\PPI\}\) and the sequence has the form \(\DR^*\PO^*\PPI^*\).  If the \(\PPI\) tail is nonempty, the completeness construction does not leave \(w\) merely in the bounded side context of \(u\).  Instead, after finite-index shortening of constant trace regimes, it inserts a genuine generated cover edge from an occurrence of \(w\) to the first retained representative \(a^*\) of the \(\PPI\) tail.  All later pairs \((a_m,w)\) are then ordinary equality-aware vertical pairs.  If the trace has no \(\PPI\) tail, the deep pairs remain residual \(\DR/\PO\) pairs and are checked by the universal-safety clauses.
\end{lemma}

\begin{proof}
The labels \(\EQ\) and \(\PP\) are impossible: \(\EQ\) would identify the side witness with an ancestor of \(u\), and \(\PP\) would imply \(u\PP w\), contradicting that \(w\) is a \(\DR/\PO\) witness of \(u\).  Since \(L(a_{m+1},a_m)=\PPI\), the RCC5 table gives
\[
\comp(\PPI,\DR)=\{\DR,\PO,\PPI\},\quad
\comp(\PPI,\PO)=\{\PO,\PPI\},\quad
\comp(\PPI,\PPI)=\{\PPI\}.
\]
Hence the trace is monotone in the order \(\DR\succ\PO\succ\PPI\), and \(\PPI\) is absorbing.  The tower descriptor is augmented with the current trace value.  Since there are finitely many augmented descriptors and at most three constant trace regimes, the ordinary blocking/replacement argument shortens each regime to bounded length.  The splice edge is inherited from the satisfying model, so it preserves acyclicity and RCC5 composition; it changes only whether the forced tail relation is presented by side data or by vertical reachability.
\end{proof}

\begin{lemma}[Global residual frontier]\label{lem:global-residual-frontier-compact}
After saturated side closure and composition-forced verticalization, every residual frontier pair has label \(\DR\) or \(\PO\).
\end{lemma}

\begin{proof}
Equality pairs are handled by equality ports.  Finite containment pairs inside exposed side/tree boundaries are handled by saturated local vertical strata.  Containment pairs that persist along an unbounded ancestor tower are handled by a bounded generated-cover splice and then by equality-aware vertical reachability.  Therefore a pair that remains residual cannot carry \(\EQ,\PP\), or \(\PPI\); the only possible RCC5 atoms left are \(\DR\) and \(\PO\).
\end{proof}
