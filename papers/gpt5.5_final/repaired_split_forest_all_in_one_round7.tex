...
\title{A Repaired Split-Forest Decidability Proof for \texorpdfstring{$\mathcal{ALCI}_{\mathrm{RCC5}}$}{ALCI-RCC5} Without Automata}
...
\begin{remark}[The central round-7 repair]
...
For selected \(\DR\) or \(\PO\) witnesses, attach finite side contexts containing the source occurrence, the witness occurrence, and the boundary/equality-mate occurrences needed to synchronize labels with the surrounding containment context.  If a side witness is itself \(\PP\)- or \(\PPI\)-comparable with another side witness, include the necessary local vertical mini-context in the side interface rather than forcing the pair into the residual sibling frontier.  Thus residual frontier pairs are only \(\DR\) or \(\PO\); containment labels inside side interfaces are explicitly verticalized.
...
\section{Mosaics and side interfaces}

Mosaics are finite snapshots of side and boundary interaction.  They are not required to be geometric regions.  They are finite abstract RCC5 networks satisfying the same atomic table as the global frame.
...
\[
  M=(P,\tau,L,\mathcal V)
\]
where \(P\) is a finite position set, \(\tau:P\to\mathrm{Type}(C_0)\), \(L:P^2\to\Base\), and \(\mathcal V\) is a finite family of declared vertical strata.  It satisfies:
\begin{enumerate}[label=(M\arabic*)]
\item \(L(p,p)=\EQ\), and \(L(p,q)=\EQ\) iff \(p=q\) inside the mosaic, except for explicitly declared equality ports that are identified by the certificate equivalence;
\item \(L(q,p)=\Inv(L(p,q))\);
\item for all \(p,q,r\in P\), \(L(p,r)\in\comp(L(p,q),L(q,r))\);
\item if \(\exists R.D\in\tau(p)\) and the witness is local to this mosaic, then some \(q\in P\) has \(L(p,q)=R\) and \(D\in\tau(q)\);
\item if \(\forall R.D\in\tau(p)\), then every \(q\in P\) with \(L(p,q)=R\) satisfies \(D\in\tau(q)\);
\item if \(L(p,q)\in\{\PP,\PPI\}\), then \(p,q\) lie in a common declared vertical stratum, unless the pair is a boundary pair that is explicitly supplied by equality-aware vertical reachability in the surrounding generated tree;
\item if \(p,q\) are residual sibling-frontier positions, then \(L(p,q)\in\{\DR,\PO\}\).
\end{enumerate}
\end{definition}

\begin{definition}[Support closure]
A finite mosaic family \(\Mos_Q\) is support-closed when:
...
\item every side-interface position that carries a \(\PP\) or \(\PPI\) relation is contained in an explicit local vertical stratum;
\item equality-port positions have the same type and the same labels to every boundary and mosaic position;
\item whenever two mosaics overlap on a common boundary, the certificate contains an amalgamating mosaic or an explicit cross-label table for their union satisfying (M1)--(M7).
\end{enumerate}
All clauses are finite because every mosaic has size bounded by the certificate grammar.
\end{definition}

\begin{lemma}[Finite patching of support-closed mosaics]\label{lem:patch}
Let \(F\) be any finite set of occurrences produced by unfolding a valid certificate.  The labels on pairs in \(F\) induced by vertical reachability, equality ports, and named side mosaics satisfy strong equality, converse, and RCC5 composition.
\end{lemma}

\begin{proof}
Strong equality and converse are immediate from the priority definition of labels and from mosaic axioms (M1)--(M2).  For composition, take \(u,v,w\in F\).  There are finitely many possible occurrence-sensitive triple shapes for \((u,v,w)\): equality collapse, strict vertical chain, mixed vertical-side configuration, common side mosaic, or overlap of two side mosaics over a named boundary.  Validity of the certificate includes a finite table of all coherent occurrence-sensitive triple shapes and requires the corresponding labels \((R_{uv},R_{vw},R_{uw})\) to satisfy \(R_{uw}\in\comp(R_{uv},R_{vw})\).  Because the triple shape records whether two endpoints are the same concrete occurrence, equality mates, strict vertical copies, or merely repetitions of the same finite profile in different laps, no blocking copy is misclassified as \(\EQ\).  Therefore the required composition inclusion holds.
\end{proof}

\section{Finite occurrence-sensitive certificates}

This section is the main formal repair.  The certificate has finite abstract profiles, but labels are not functions of profile or abstract position alone.  They are functions of \emph{pair shapes}.  Pair shapes include an incidence tag that distinguishes same occurrence, explicit equality mate, strict vertical ancestor, strict vertical descendant, and side/frontier interaction.
...
\]
All \(u_i\) may have the same finite profile and port symbol, but \(u_i\ne u_j\) for \(i\ne j\), and \(u_i\not\eqc u_j\).  The incidence tag for \((u_i,u_j)\) with \(i<j\) is \(\up\), not \(\self\).  Therefore the label is \(\PP\), not \(\EQ\).  This is exactly the round-7 correction to the abstract-position labelling problem.
