...
it in August after the fragment was machine-certified. The routes it describes
...
decidability (F6) stays \textbf{open} --- forced by $\forall\PO$, which the
fragment removes. One caveat is complexity, not correctness: the procedure
is decidable but \textbf{not runnable}, the signature space being doubly
exponential. This supersedes the 2026-08-06 ``three of four quadrants''
...
independent cold reviews of it found no counterexample. The fragment is
exactly the region where the open keystone F6 --- bound the live width --- is
a theorem instead of a conjecture, and its boundary, one constructor wide, is
drawn by the composition table itself.
