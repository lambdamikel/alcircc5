...
position}. The certified route carries the normalisation and the
...
\item \textbf{August 2026 --- the ConeScheme route, and the one that is
  ...
  model, and no joint realisability across a signature's several demands is
  ever needed. This route does \emph{not} use the $K(C_0)$ bound, so
  items~1--3 above remain independent, unformalised arguments for the same
...
kernel-checked). Not formalised on \emph{this} route: the assembly construction gluing these
...
remain \textbf{strongly supported, not formalised end-to-end}; and the
