...
\author{Michael Wessel --- with Claude Opus~4.8 and Claude Opus~5
(Anthropic)\thanks{Opus~4.8 wrote this explainer in July 2026; Opus~5 revised
it in August after the fragment was machine-certified, and in September after
a further cold review. The routes it describes
are due to several models and are attributed individually in
\S\ref{sec:ledger}.}}
\date{2026-08-29, revised 2026-09-14}
...
\smallskip
\textbf{Honesty note (updated 2026-08-29):} the fragment is now
\textbf{machine-certified decidable across all four quadrants}, including
the mixing one ($\exists\PO+\exists\PP$) that was open when this explainer
was first written. The capstones are in Lean~4
(\code{formal/POFreeLift.lean}, ${\sim}45{,}400$ lines, \textbf{zero
sorries}, axioms \code{propext}/\code{Classical.choice}/\code{Quot.sound}):
\code{decidableFSat} on \emph{raw} input with negation,
\code{decidableSat\_cone} on NNF concepts, and \code{decidableSetSat} under
\emph{concrete set} semantics --- with fragment membership the only
hypothesis in each. They were reached by a \emph{third} route, distinct from
the two argued in this explainer: a finite control graph of signatures, a
monotone elimination to a greatest fixed point, and a fresh-occurrence
unfolding in which no node is ever reused. It does not use the $K(C_0)$
bound, so the two arguments here remain theorem-level and independently
worth reading. Three independent cold reviews (2026-08-29) attacked the
trusted base, completeness, and soundness respectively, and \textbf{none
found a counterexample or a defect in the fragment result}. \emph{Full-logic}
decidability stays \textbf{open}. One caveat is complexity, not correctness:
the procedure is decidable but \textbf{not runnable}, the signature space being
doubly exponential. This supersedes the 2026-08-06 ``three of four quadrants''
status; see \code{ASSEMBLY\_DESIGN.md} \S\S267--297.

\smallskip
\textbf{Update (2026-09-14):} the certified result now has its own focused
paper, \code{papers/pofree\_fragment\_arxiv.pdf}. Two published theorems of Lutz
and Wolter (2006) are credited there and here: every RCC5 structure is
representable by regions (their Theorem~23, of which the certified
abstract-versus-sets equivalence is a special case), and the logic coincides
with its substructure logic over regular closed regions of $\mathbb{R}^n$
(Theorem~24), so the fragment result holds verbatim in that topological
reading too.}}
\end{center}
...
\smallskip
\emph{On raw input.} Stating the fragment over NNF concepts is not a
restriction, but the correct condition on \emph{raw} concepts is
polarity-sensitive and is easy to get wrong: pushing a negation through
$\exists\PO.D$ produces $\forall\PO$, so the condition is \textbf{no
$\forall\PO$ in positive position and no $\exists\PO$ in negative
position}. The certified route carries the normalization and the
preservation theorem, so this is a theorem rather than a convention
(\code{nnfP\_correct}, \code{decidableFSat}).
...
this document is why the difficulty that has defeated every full-logic
attempt is concentrated in exactly that one constructor.
...
is the same obstruction the full logic's open width question (F6) runs
into.
...
open keystone of the project's principal full-logic certificate route. It
is \emph{not} itself a Lean hypothesis: the certified conditional theorem
assumes a complete enumeration of certificates, and F6 is the argued but
unformalized route to that premise. \textbf{The $\forall\PO$-free fragment
is the place where a live-width bound is proved} (at theorem level, by
both routes below) rather than conjectured --- though the route that
finally certified the fragment needs no width bound at all.
...
\paragraph{The connection to the open problem is close.} F6, the keystone
of the full-logic certificate route, asks for a computable live-width bound.
Both routes above \emph{prove} one for the fragment, at theorem level
(Route~1 explicitly: $K(C_0)$). And
...
shapes} and \textbf{three arrivals} in this explainer, plus a \textbf{fourth
shape} that is the one actually certified (item~5 below), and the
certification pass itself.
...
\item \textbf{August 2026 --- the ConeScheme route, and the one that is
  certified} (design: GPT-5.6~Sol, prompted by Wessel after two rounds of
  cold attack had refuted the previous certification architecture; Lean
  development: Claude). A genuinely \emph{third} shape, and the reason the
  fragment is now machine-certified end to end. The move is to stop reusing
  nodes: a finite \emph{cone scheme} serves only as a \emph{control graph} of
  signatures (a support type together with an upper bound on the types of
  its strict $\PP$-predecessors); a monotone, reductive elimination runs to
  a greatest fixed point; and every non-$\EQ$ existential unfolds to a
  \emph{fresh occurrence}, so that nothing is ever identified or borrowed.
  \emph{Why the previous architecture failed}, since it is the transferable
  part: reusing a node imposes a \emph{global} condition --- the order the
  reused edges induce must stay acyclic, and the relations they force must be
  \emph{jointly} realizable --- while every discipline proposed for enforcing it
  was a \emph{local} test, per edge or per node or per type. A local test cannot
  see what it is not looking at, and the four countermodels say so one axis at a
  time: per-edge safety removes self-loops and nothing longer (two individually
  safe fallbacks forming a four-cycle); a key read off an occurrence's downward
  cone cannot separate occurrences whose obstruction is \emph{horizontal}; a
  class top defined by type alone drags one member's disjointness witness onto
  another's cone. That is the pointwise-versus-joint shape that had already
  broken three earlier architectures in this project, so the fault lay in
  proposing another local discipline, not in the particular ones proposed.
  Standard \emph{blocking} is the same story in its most familiar form --- it
  is exactly the local test, ``same type, so let one occurrence stand for the
  other'' --- and it fails here for the same reason, since type equality cannot
  see whether the reuse closes a cycle in the order or puts the blocker's
  disjointness witness inside the blocked node's cone. The
  cone scheme does not test better --- it removes the obligation, and the global
  conditions then hold structurally rather than by enforcement.

  The frame conditions that had defeated the previous architecture ---
  irreflexivity of the order, and its disjointness from the disjointness
  relation --- then hold \emph{structurally} rather than by construction.
  The \emph{completeness} half inverts what every earlier route attempted.
  Those read completeness as \emph{extraction} --- given a model, build a finite
  certificate out of it --- and that construction is where the $K(C_0)$ budgets
  live. Here nothing is extracted. The signature space is fixed in advance from
  $C_0$; a model point's signature is \emph{read off} it (its type, plus the
  types of its strict $\PP$-predecessors); and the signatures a model realises
  survive their own elimination, because for each demand the model hands over a
  witness. That is a \emph{post-fixed point}, and a monotone reductive
  elimination puts every post-fixed point inside its greatest fixed point.
  Finiteness is free, the space having been finite before any model appeared.
  The monotonicity is not a technicality: this project once retracted a
  type-elimination procedure whose rule was \emph{anti-monotone} --- discarding
  one type could invalidate the witness justifying another --- so exactly this
  argument was unavailable to it and its fixpoint over-eliminated. That is why
  the operator here asks only that each demand \emph{have} a target, and why
  that was checked before anything was built on it.

  What makes the route work is specific to this fragment, and is worth
  stating because it explains the boundary: a composition cell containing
  $\PO$ can always be discharged by putting the pair at $\PO$ unless some
  formula can object to a $\PO$ edge, and the only formula that can is
  $\forall\PO.D$; so here the maximally-$\PO$ unfolding is \emph{always} a
  model, and no joint realizability across a signature's several demands is
  ever needed. This route does \emph{not} use the $K(C_0)$ bound, so
  items~1--3 above remain independent, unformalized arguments for the same
  theorem. Capstones: \code{decidableFSat}, \code{decidableSat\_cone},
  \code{decidableSetSat}. The design document is
  \code{papers/cone\_scheme\_plan/}; the Lean development and its
  section-by-section provenance are \code{formal/POFreeLift.lean} and
  \code{ASSEMBLY\_DESIGN.md} \S\S267--297.
\item \textbf{2026-08-29 --- three cold reviews.} The certified route was
  attacked three times independently: at its trusted base and execution, at
  its completeness direction, and at its soundness direction. None found a
  counterexample. The second found a genuine defect --- a dropped
  $\forall\PO$ propagation in the transition relation, vacuous on the
  fragment but not on the full-logic \textsc{unsat} test built from the same
  machinery --- and the third found no defect in the development at all, its
  five findings all lying in the project's own probes and prose.
\end{enumerate}

\paragraph{What stayed constant across all three routes.} Worth one
paragraph, because it is the honest answer to ``what was the human's idea worth
in the end.'' Every route here presents a model the same way: a vertical
skeleton of ${<}$-edges, one tree per component, with $\DR$ and $\PO$ as
non-tree edges completed afterwards --- the split-forest discipline of
\S\ref{ssec:tableau}. The certified route (item~5) is that picture in an
unusually literal form: its elements are demand \emph{words}, so occurrences of
the same type stay distinct and are never re-identified; its order is generated
by exactly two moves, ``prepend a $\PP$ birth'' and ``strip a $\PPI$ head''; and
stripping the leading vertical births returns each occurrence's component root,
which order edges provably preserve and $\DR$ births provably change.

One clarification, because it is easy to draw the wrong conclusion: the
certified route does not \emph{assume} split-forest models. Its completeness
half takes an arbitrary model and reads a signature off it. But the shape is
still what the soundness half \emph{constructs}, and it is the content of the
normal form that makes composition closure free. That a signature records
vertical position fits the same picture, though as a correspondence rather than
a documented design step: the cone-scheme plan does not refer to split
forests. The idea's job changed --- from a
property to be established of an arbitrary model, which is the extraction
problem the earlier routes were refuted on, to a shape to be built, which is
free --- and that change is why it went through.

What the routes did \emph{not} share is the machinery. The certified one needs
no patchwork appeal (composition closure comes from the ordered-disjoint
axioms), no quotient (nothing is re-identified, because no particular model has
to be recovered), and no width bound. Three successive architectures were each
refuted at the point where they tried to \emph{build} a finite presentation of
such a forest and keep it consistent; what finally worked was to stop building
one and let the syntax's own demand words be the forest. The shape was the
durable contribution; the machinery was not.
...
(\code{RCC5NormalForm.lean}, \code{odNet\_frame} in \code{POFreeLift.lean},
canonical representation)\\[0.3em]
Abstract $=$ set satisfiability, \emph{full} logic & \textbf{Certified}
(\code{satisfiable\_iff\_set}); a special case of Lutz--Wolter Thm.~23. By
their Thm.~24 the fragment result also holds for substructures of regular
closed regions of $\mathbb{R}^n$ (cited, not formalized)\\[0.3em]
...
sorries). Cold-reviewed three times without a counterexample, and audited
at source level by a fourth review (2026-09-14) that found no defect.
Decidable, \emph{not} runnable: the signature space is doubly
exponential\\[0.3em]
...
\textbf{How far the first route's own certification campaign got.} This
paragraph is a record of what \emph{that} route certified, not a statement of
what remains: the theorem was carried by a different route in August 2026
(ledger \S\ref{sec:ledger}, item~5), and none of the machinery named here is
used by it. Rounds A--D1 (2026-07-22/23) landed the entire \textbf{soundness
side plus the decision architecture}: a valid two-tier certificate, with any
...
kernel-checked). Not formalized on \emph{this} route: the assembly construction gluing these
into an accepted code, and the $K(C_0)$ counting/enumeration.

That is no longer the last word on the theorem, though it remains the last
word on \emph{this} argument. The fragment was carried to a machine-certified
decision procedure in August 2026 by a third route (ledger
\S\ref{sec:ledger}, item~5) which does not use $K(C_0)$ at all, and which has
since been through three independent cold reviews without a counterexample.
So the honest labels differ: the \textbf{theorem} is
\textbf{machine-certified}; the \textbf{two arguments in this explainer}
remain \textbf{strongly supported, not formalized end-to-end}; and the
\textbf{full logic} remains open.
...
concrete satisfiable concept with one $\forall\PO$ provably defeats that
route's constant-interface certificate via ``wandering witnesses'' that
...
always-safe free amalgamation. Two argument shapes and three independent
arrivals, and then a fourth shape --- a finite control graph of signatures
with a fresh-occurrence unfolding --- which is the one that carried the
theorem all the way into the kernel: since 2026-08-29 the fragment has a
machine-certified \emph{decision procedure}, on raw input and under concrete
set semantics, with fragment membership its only hypothesis, and three
independent cold reviews of it found no counterexample. On the fragment the
live-width bound that the full logic's certificate route needs (F6) is a
theorem rather than a conjecture --- though the certified procedure turns out
not to need it --- and the fragment's boundary, one constructor wide, is drawn
by the composition table itself.

\bigskip
\begin{center}
\fbox{\parbox{0.92\textwidth}{\small\itshape
Companions: \code{papers/pofree\_fragment\_arxiv.pdf} (the certified result,
as a focused paper), \code{WHY\_ITS\_HARD.md} (the full problem, non-technical),
