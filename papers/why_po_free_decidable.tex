...
A and~B (2026-07-22) landed the model-of-$C_0$ layer, so a valid
two-tier certificate, now with \textbf{any family of ascending or
descending kernels}, yields a model of the concept, kernel-checked end
to end (witnesses include the ascending-plus-descending two-tower
concept). Still open: an executable Boolean checker (round~C) and the
completeness extraction carrying the $K(C_0)$ bound (round~D, the bulk
