...
\newcommand{\NRe}{\ensuremath{\mathcal{N}_{\mathcal{R}_8}}}% the 8 RCC8 base relations
...
found no concept term that forces the coincidence at the level of abstract
models.

The analysis reveals that Wessel's 2002/2003 technical report independently
developed the RCC8 description-logic analogues of these ingredients---the
even-odd chain for functional successors, an explicit two-dimensional
TPP/NTPP grid, and the identification of the coincidence gap as the decisive
obstacle---three to four years before the Lutz--Wolter LMCS publication, and
was the first to identify that obstruction in this setting.  Lutz and Wolter
close the gap by a \emph{different}, topologically grounded construction---a
linearized enumeration of tile positions in the Marx--Reynolds lineage they
cite (their~\S5)---rather than by the two-dimensional grid Wessel analyzed.
The decidability of $\ALCIRCC{8}$ remains open.
\end{abstract}
...
Lutz and Wolter~\cite{LutzWolter2006} later studied \emph{modal
...
built as an RCC8 network, while finding no concept term that \emph{enforces} it
...
\[
\NRe = \{DC,\, EC,\, PO,\, EQ,\, TPP,\, NTPP,\, TPPI,\, NTPPI\}.
\]
...
The composition table $\comp(R,S) \subseteq \NRe$ is defined for all pairs
$R,S \in \NRe$ and captures which base relations are possible for a pair
...
\begin{itemize}
  ...
  \item every pair $(d,e)$ with $d \neq e$ is assigned exactly one base
    relation $R \in \NRe \setminus \{EQ\}$, and $EQ^{\mathcal{I}} =
    \mathrm{Id}(\Delta^{\mathcal{I}})$
    (strong EQ semantics);
  \item the role box (composition-table constraints) is satisfied:
    for all $R,S \in \NRe$, $R^{\mathcal{I}} \circ S^{\mathcal{I}} \subseteq
...
In~\cite{LutzWolter2006}, the same idea appears within their Section~5
undecidability reduction, as follows.  An infinite
...
and Reynolds and Zakharyaschev~\cite{ReynoldsZakharyaschev2001}, where it
originates.  They do not mention Wessel's even-odd chain in that connection,
although they cite his 2001 workshop paper~\cite{Wessel2001}, in which it first
appears, elsewhere in their paper.
...
\begin{remark}
  ...
  Marx--Reynolds~\cite{MarxReynolds1999} for temporal logics, and the binary
  counter and $\EXPTIME$-hardness constructions themselves follow a PDL encoding
  that Wessel adapts (\cite{Wessel2003}, \S4.4.2).  Wessel's contribution is the
  \emph{application to RCC8 description logics}: exploiting the $TPP/NTPP$
  role-versus-transitive-orbit separation to carry those PDL-style constructions
  into a spatial DL, three to four years before Lutz--Wolter.
\end{remark}
...
\subsection{Lutz--Wolter's construction: linearizing the plane (2006)}

In~\cite{LutzWolter2006}, Section~5 (Theorem~5, Corollary~6), Lutz and Wolter
\emph{sidestep} the coincidence problem rather than enforcing it on a
two-dimensional grid.  Following Marx and Reynolds~\cite{MarxReynolds1999},
they fix a \emph{linear enumeration} $\lambda$ of the plane positions
($\lambda(1)=(0,0)$, $\lambda(2)=(1,0)$, $\lambda(3)=(1,1)$, \ldots; their
Figure~4) and enforce a single $ntpp$-ordered chain $r_1, r_2, \ldots$ with
$r_i$ representing position $\lambda(i)$.  Additional ``go right'' regions
connect each $r_i$ to the region representing the position to its right,
and---crucially---``reaching the position above the current one is simply a
matter of going to the right and then advancing by one in the
enumeration''~\cite{LutzWolter2006}.  There is thus no two-path coincidence
$R_X \circ R_Y = R_Y \circ R_X$ to force on a $2$-D grid: the linearization
dissolves it.  What still requires the \emph{topological} setting is the
rigidity that keeps this linear encoding faithful:
\begin{itemize}
  \item In a topological space, $TPP(a,b)$ means $a \subsetneq b$ with
    $a \cap \partial b \neq \emptyset$.  The containment structure of regions
    in~$\reals^n$ ($n \geq 1$) is rigid enough to realize the $ntpp$-chain and
    the ``go right'' regions as genuinely nested regions, pinning the
    enumeration down consistently; the second dimension is then recovered
    \emph{from} the enumeration, never as an independent vertical successor
    that would have to coincide with a horizontal one.
...
\begin{theorem}[Lutz--Wolter~\cite{LutzWolter2006}, Theorem~5 and Corollary~6]
...
\begin{proof}
  ...
  topological, and topological rigidity makes Lutz and Wolter's linearized
  enumeration \emph{faithful}: the $ntpp$-chain genuinely enumerates the plane
  positions and the ``go right''\,/\,``advance one'' regions land coherently,
  so every $\LRCC{8}$-model of $C_\mathcal{D}$ encodes a valid tiling and
  $\LRCC{8}$-unsatisfiability follows.

  For $\ALCIRCC{8}$, that rigidity is gone.  The composition-table semantics
  does not pin the enumeration down to a genuine grid: nothing forces the
  ``go right then advance one'' regions to realize the intended
  two-dimensional adjacency, so an abstract model can satisfy $C_\mathcal{D}$
  while the encoded positions do \emph{not} form a tiling---a ``spurious''
  model.  The underlying obstruction is the one Wessel identified: any faithful
  grid encoding ultimately needs the coincidence
  $R_X \circ R_Y = R_Y \circ R_X$ (condition~\ref{g:coinc}), and abstract RCC8
  does not force it---the composition table alone allows too many relations
  $\{PO, EQ, TPP, NTPP, TPPI, NTPPI\}$ at the coincidence point (Figure~11).
...
\begin{remark}\label{rem:concrete-attempt-conclusion}
  ...
  Lutz--Wolter's $\LRCC{8}$ reduction \emph{avoids} the gap rather than
  closing it on a 2-D grid: they linearize the plane into a single
  $ntpp$-chain (the Marx--Reynolds enumeration) and recover the vertical
  direction from the enumeration, so no ``north-of-east $=$ east-of-north''
  coincidence has to be enforced at all; topological rigidity is what keeps
  that linear encoding faithful~\cite{LutzWolter2006}.
...
\begin{enumerate}
  \item \textbf{``No results'':}
    The cited paper~\cite{Wessel2001} already contains undecidability results
    for a closely related family, $\mathcal{ALC}_{\mathcal{RA}^\ominus}$ (ALC
    with role axioms in the non-disjoint-roles variant, via reduction from
    non-empty CFG intersection), though not for the RCC8-based logics
    themselves.  The uncited technical
    report~\cite{Wessel2003} goes significantly further, for exactly those
    logics: it proves
  ...
  \item \textbf{The even-odd chain:}
    ...
    Reynolds--Zakharyaschev~\cite{ReynoldsZakharyaschev2001}, who used similar
    techniques in temporal logic, and crediting a technique to its origin is
    standard.  What goes unmentioned is its earlier application to RCC8
    description logics---three to four years before Lutz--Wolter, in the 2001
    paper they cite and in the report.
  ...
  \item \textbf{The grid construction and coincidence obstruction:}
    ...
    table allows multiple relations at the diagonal point, and no concept term
    enforcing the grid was found (``we have found no way to \emph{enforce} the
    depicted model(s) by means of a concept term'').  These are the same issues Lutz and
  ...
  \item \textbf{The undecidability boundary:}
    ...
    binding operator $\downarrow$ as the precise extension that crosses the
    boundary.  This is the same boundary the Lutz--Wolter result sits astride
    from the other side: they obtain undecidability precisely where Wessel
    showed the \emph{abstract} semantics cannot, by drawing on the
    \emph{concrete} topological rigidity Wessel's setting lacks---a different
    route to the boundary (a linearized tiling enumeration, \S5), not a
    development of Wessel's grid.
\end{enumerate}
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lcc@{}}
    \toprule
    \textbf{Ingredient} & \textbf{Wessel (2002/2003)} & \textbf{Lutz--Wolter (2006)} \\
    \midrule
    Functional successor via TPP/NTPP$^\dagger$ & \cite{Wessel2003}, \S4.4.2 & \cite{LutzWolter2006}, \S5 \\
    Binary counter ($2^n$ path) & \cite{Wessel2003}, \S4.4.2 & --- \\
    $\EXPTIME$-hardness of an RCC8 logic & \cite{Wessel2003}, \S4.4.2 & --- \\
    Native 2-D $TPP$ grid (two successors) & \cite{Wessel2003}, Fig.~10 & --- \\
    Coincidence obstruction identified & \cite{Wessel2003}, Fig.~11 & --- \\
    Linearized plane enumeration$^\dagger$ & --- & \cite{LutzWolter2006}, \S5, Fig.~4 \\
    Coincidence avoided via topology & --- & \cite{LutzWolter2006}, Thm.~5 \\
    Domino reduction completed & --- & \cite{LutzWolter2006}, Thm.~5, Cor.~6 \\
    \bottomrule
  \end{tabular}

  \smallskip
  {\footnotesize $^\dagger$~Shared technique; both trace it to
  Marx--Reynolds~\cite{MarxReynolds1999} (compass/temporal logics) and to PDL
  counter encodings, not to each other.}
\end{table}

We emphasize that the Lutz--Wolter contribution is genuine: showing that the
topological setting circumvents the coincidence obstruction (via the linearized
enumeration) is a non-trivial result.  But the prior identification of the
obstruction itself, the
...
four years.  While the Lutz--Wolter result is genuinely new (circumventing the
coincidence via a linearized topological encoding is non-trivial), the prior
analysis that \emph{identified} the obstruction and \emph{demonstrated} that it
blocks the reduction in the abstract setting deserved more explicit
acknowledgment.
...
\bibitem{ReynoldsZakharyaschev2001}
M.~Reynolds and M.~Zakharyaschev.
\newblock On the products of linear modal logics.
\newblock \emph{Journal of Logic and Computation}, 11(6):909--931, 2001.
