...
\begin{itemize}[nosep]
  ...
  \item \textbf{Modal logics of RCC relations are undecidable.}\;
    Kontchakov, Wolter, and Zakharyaschev~\cite{KontchakovWZ2010}
    defined modal logics with modalities interpreted by RCC8
    relations.  These logics are undecidable over the real line,
    including fragments involving containment and overlap.
...
\begin{remark}[Why the BD undecidability does not block our approach]
The undecidability of the BD fragment concerns the \emph{satisfiability
problem for HS formulas}---an infinite family of sentences parameterized
by modal nesting depth.  Our MSO encoding is different: for each fixed
concept~$C_0$, we produce a \emph{single, fixed} $\MSO(\reals, <)$
sentence~$\Phi_{C_0}$ and ask whether it is true in $(\reals, <)$.
This is a question about the \emph{theory} of $(\reals, <)$, not about
satisfiability of a modal logic.  The non-elementary complexity of
Shelah's decision procedure compensates for the expressive gap: what
HS fragments cannot decide efficiently, $\MSO(\reals, <)$ can decide
with brute-force model theory.
\end{remark}
...
line~\cite{KontchakovWZ2010}.  The MSO encoding avoids these
...
D.~Bresolin, D.~Della~Monica, V.~Goranko, A.~Montanari, and G.~Sala.
...
\newblock In \emph{Proc.\ LPAR}, LNCS 5330, pages 590--604, 2009.
...
\bibitem{KontchakovWZ2010}
R.~Kontchakov, F.~Wolter, and M.~Zakharyaschev.
\newblock Logic of metric spaces and $\mathsf{LS}$-spaces.
\newblock \emph{Logical Methods in Computer Science}, 6(3), 2010.
\newblock (See also: ``Modal logics of topological relations.'')
...
\newblock Maximal tractable fragments of the Region Connection
  Calculus.
\newblock In \emph{Proc.\ IJCAI}, pages 56--61, 1999.
