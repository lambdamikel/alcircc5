...
**Status (2026-05-31): strongly supported, NOT certified. Theorem B (the finite-abstraction keystone) was cold-reviewed (6th cold review) and found unproved; GPT-5.5's round-12 patchwork-bag repair is now canonical.** The decidability theorem is plausible and the split-forest *semantic normal form* (witness thinning → generated containment covers → split copies → side-interface mosaics → typed-EQ quotient = **Theorem A**) has been repeatedly found sound. What no manuscript has yet done is survive a cold review without a new defect (**6 reviews, 6 defects**). The repaired automata proof's **Theorem B** (finite abstraction adequacy) was cold-reviewed by a fresh, genuinely cold Opus 4.8 — a different model lineage from GPT-5.5, who authored it (`papers/automata_route_repairs/referee_report_ALCI_RCC5.md`) — and found *not proved*: its interface-transformer machinery derives a globally consistent labelling of an infinite tree from purely local checks, and the global coherence is asserted, not proved. GPT-5.5's **round-12 repair** took the reviewer's recommended route — delete the bespoke transformer layer; rest global consistency on the **RCC5 patchwork property** + a **two-way parity automaton**. The **canonical statement is now `papers/automata_route_repairs/split_forest_patchwork_automata_repaired.tex`** (14pp, round-12); the 5th-review repair (`split_forest_automata_repaired_full_proof` + the A/B/C companion) is its immediate predecessor, and the original 21-page automata proof is superseded. **Do not describe the theorem as proven or certified, and do not describe round-12 as cold-reviewed — it has not been.**
...
- `papers/automata_route_repairs/split_forest_patchwork_automata_repaired.tex` -- **the canonical decidability statement** (round-12, 14pp): split-forest normal form (Theorem A) → **patchwork-bag** finite abstraction (global RCC5 consistency via the patchwork property) → **two-way** alternating parity automaton emptiness. See "Current Decidability Argument" above. NOT cold-reviewed in turn; new (smaller) keystone = patchwork-bag adequacy; depends on two standard external theorems (RCC5 patchwork property; two-way parity-automaton emptiness, Vardi). The 6th cold review that prompted it: `referee_report_ALCI_RCC5.md`; GPT-5.5's response: `formal_response_referee_round12.tex`.
