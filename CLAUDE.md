...
**Status (2026-05-28, late evening): strongly supported, NOT proven.** GPT-5.5's round-8 split-forest proof is the most developed statement of the architecture, but a cold Opus 4.8 review (`papers/opus4.8_review/`) found a genuine **completeness gap (D-1, medium-high severity)**: round-8's cross side/tree exhaustion lemma closes the side/tree gap only on the finite, depth-bounded boundary `B_k(u)`, while a DR/PO side witness can be RCC5-composition-forced to be a proper part of an *infinite* pumped tower whose tail the bounded saturated side context cannot carry. Concrete satisfiable witness: `C0' = (∃PP.G) ⊓ (∃PO.¬X)` with `G = (∀PO.X) ⊓ (∃PP.G)` — the round-8 completeness construction emits an invalid certificate for it (machine-checked, `papers/opus4.8_review/rcc5_compose.py`). A proposed repair (composition-forced verticalization: splice the forced witness into `E_up` at a bounded threshold height) is in `papers/opus4.8_review/repair_completeness_round8.tex`; it has machine-checked linchpins (monotone+bounded threshold, inherited consistency) but **open bookkeeping obligations O1-O4 and is NOT ratified by GPT-5.5 or human experts**. Do not describe the theorem as proven. The soundness direction and the round-7 blocking/equality machinery are unaffected; the theorem is most likely still true (an alternative presentation appears to certify C0'), but the proof as written has a completeness hole pending repair-ratification or a GPT-5.5 round-9 fix.

The most developed statement is **GPT-5.5's round-8 split-forest proof** (May 28, 2026, evening). Two documents form the no-automata pillar, plus one independent witness via parity-tree automata:
