...
**Status (2026-05-29, late): strongly supported, NOT mechanically certified. Primary statement switched to the automata route.** The decidability theorem is plausible and the split-forest *semantic normal form* (witness thinning → generated containment covers → split copies → side-interface mosaics → typed-EQ quotient) has been repeatedly found sound. What no manuscript has yet done is survive a cold review without a new defect. After the no-automata thread reached round-10 (below), the **canonical statement is now GPT-5.5's split-forest + parity-tree-automata proof**, `papers/gpt5.5_final/split_forest_automata_decidability_proof_detailed.tex` (21 pages). **Do not describe the theorem as proven or mechanically certified, and do not describe the automata proof as cold-reviewed — it has not been.**
...
**This is a strategic switch, not a proof upgrade.** The automata proof has **not** been cold-reviewed — its automaton construction, the soundness/completeness-of-the-automaton lemmas, and the finite-alphabet bound are unscrutinized, and its split-forest-normal-form half inherits whatever issues the shared construction has. Status remains *strongly supported, not certified*. The natural next step is to cold-review the automata proof: the `papers/cold_review_round9/` packet repurposes (swap in the automata PDF with a neutralized title, keep the withhold-list).
...
- `papers/gpt5.5_final/split_forest_automata_decidability_proof_detailed.tex` -- **the canonical decidability statement** (split-forest normal form + parity tree automaton, 21 pages). See "Current Decidability Argument" above. NOT yet cold-reviewed.
