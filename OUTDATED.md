...
**Honest caveat on the promotion (and what happened next).** Switching to the automata route was a *strategic* clarity move, **not** a proof upgrade -- and it played out exactly as the caveat warned (next section).

---

## The original automata proof -- superseded after the 5th cold review (2026-05-30)

`papers/gpt5.5_final/split_forest_automata_decidability_proof_detailed.tex` (the 21-page automata proof promoted to primary on 2026-05-29) was cold-reviewed by a fresh GPT-5.5 session (`papers/automata_route_repairs/rcc5_split_forest_referee_report.tex`, the 5th cold review on the project). The review found that the automata route's `Patch` invariant -- that a finite *local* check guarantees *global* RCC5 composition + universal propagation to all composition-forced targets + equality congruence -- was **assumed, not constructed**, and that the "automaton" was prose, not a total finite transition function. Concrete UNSAT concepts it would wrongly accept: `C_force = EPP.(EDR.A) & ADR.~A` (`x PP y, y DR z => x DR z` forced via `comp(PP,DR)={DR}`), `C_split`, `C_recursive`. So the automata route had the no-automata route's keystone gap all along; the "decided by non-emptiness" advantage was illusory until the automaton was shown genuinely finite.

GPT-5.5 repaired it (`papers/automata_route_repairs/split_forest_automata_repaired_full_proof.tex`, 19pp) with **finite pair/triple product-state representatives** -- the same convergent idea as round-10's product-state exhaustion -- plus an A/B/C companion (`split_forest_companion_ABC_paper.tex`, 18pp) isolating **Theorem A** (split-forest normal form), **Theorem B** (finite abstraction adequacy -- the keystone), and **Theorem C** (automaton accepts exactly the valid abstractions). The original 21-page automata proof is superseded. Both routes had now converged on **Theorem B** — making it the clear target for the next cold review.

## The transformer-based repaired automata proof (Theorem B) -- superseded after the 6th cold review (2026-05-31)

`papers/automata_route_repairs/split_forest_automata_repaired_full_proof.tex` + `split_forest_companion_ABC_paper.tex` (the 5th-review repair, with its finite pair/triple product-state representatives) were cold-reviewed by a fresh, genuinely cold **Opus 4.8** — a different model lineage from GPT-5.5, who authored the proof — via the [`cold_review_theoremB/`](papers/cold_review_theoremB/) packet (`papers/automata_route_repairs/referee_report_ALCI_RCC5.md`, the 6th cold review). Verdict: **genuine gap; theorem most likely true; repairable.** The keystone (Lemma 6.9, finite pair/triple representation) derives a globally consistent RCC5 labelling of an *infinite* tree from purely *local* interface-transformer checks, and the required **confluence law** (initialise-at-ancestor-then-push-down = initialise-at-LCA) is **asserted, not checked** (C1) — so the read-off labelling may be route-dependent and need not satisfy composition. Same gap in the multi-parent split `C_split` (C2) and equality-port congruence (C3); plus M1–M4. The reviewer could *not* build an accepted-unsat witness, re-verified the composition table + witness thinning + every diagnostic verdict, and recommended the **patchwork** route (cite the RCC5 patchwork property instead of hand-deriving global coherence) + a **two-way** automaton (Vardi).

GPT-5.5's **round-12** (`papers/automata_route_repairs/split_forest_patchwork_automata_repaired.tex`, 14pp) takes exactly that route: it **deletes the transformer layer** and rebuilds on a **patchwork-bag abstraction** (finite bags = complete atomic RCC5 networks, glued by the **RCC5 patchwork property**), a **two-way alternating parity automaton** (Vardi emptiness), exact tree-decomposition identity for EQ (no semantic-EQ quotienting), and exact relation-vector shadows for universals (no may-analysis). This replaces the unproved *bespoke* keystone with a **standard citable theorem the project has stress-tested all along** (the RCC5 patchwork property — WP6). The transformer-based repaired proof + ABC companion are its predecessor. Remaining dependence localizes to two standard external results (RCC5 patchwork property; two-way parity-automaton emptiness, Vardi 1998), the new (smaller) keystone is **patchwork-bag adequacy**, and round-12 has **not** been cold-reviewed in turn. Status: strongly supported, not certified (6 reviews, 6 defects — trajectory convergent, onto standard tools).

**Round-12 expanded edition (2026-05-31, later) — `papers/automata_route_repairs/split_forest_automata_with_appendices.tex` (29pp).** At the project's request, GPT-5.5 extended the 14-page round-12 into a self-contained 29-page edition: **same theorem spine** (A: split-forest normal form → B: patchwork-bag adequacy → C: two-way automaton correctness → Decidability), now with **21 appendices (A–U)** discharging every load-bearing step. Notably App A proves the finite→countable patchwork passage (finite tree-patching by induction on bags + a **compactness argument over the finite relation alphabet** for the countable completion), and App E derives the global RCC5 labelling `ρ_T` from that compactness + finite patchwork **with no route-dependent transformer** — i.e. it spells out, rather than assumes, exactly the global-coherence step the 6th cold review faulted in the round-11 transformer layer. Other appendices: effective `K(C₀)` width bound (C/L), finite-abstraction extraction (D), relation-vector shadows (F), exact occurrence identity / running-intersection replacing the EQ quotient (G/R), automaton states + parity (H/N), effective bounds (I), audit table (J), implementation-ready decision procedure + local legality tests (K/Q), diagnostic examples incl. the `PPI;PP` DR-exclusion (O), two-wayness rationale (S), dependency graph (T), failure-mode list (U). This is **not a new repair round** (no new defect was found; the goal was self-containedness) and is **not itself cold-reviewed**. It is the current canonical manuscript; the 14-page theorem-spine version is retained as its compact form. Status unchanged: strongly supported, not certified.
