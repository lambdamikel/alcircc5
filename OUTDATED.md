...
GPT-5.5 repaired it (`papers/automata_route_repairs/split_forest_automata_repaired_full_proof.tex`, 19pp) with **finite pair/triple product-state representatives** -- the same convergent idea as round-10's product-state exhaustion -- plus an A/B/C companion (`split_forest_companion_ABC_paper.tex`, 18pp) isolating **Theorem A** (split-forest normal form), **Theorem B** (finite abstraction adequacy -- the keystone), and **Theorem C** (automaton accepts exactly the valid abstractions). The repaired proof + ABC paper are the current canonical statement; the original 21-page automata proof is superseded. Both routes have now converged on **Theorem B**; whether it is proved is open, and the repaired proof has not been cold-reviewed in turn. Status: strongly supported, not certified.
