...
We close this gap by identifying the precise algebraic root cause---the
unique \emph{self-absorption failure}
$\PPI \notin \comp(\DR, \PPI) = \{\DR\}$---and showing that the
tableau's own composition filtering already forces exactly the edge
reassignments needed to resolve it.  We give a direct model
construction from open completion graphs that bypasses the quasimodel
abstraction, using the \emph{full RCC5 tractability} theorem to assign
cross-subtree edges in the tree unraveling.  Combined with the
termination and completeness results from~\cite{CompanionPaper}, this
establishes:

\medskip
\noindent\textbf{Main Theorem.}\; Concept satisfiability in
$\ALCIRCC{5}$ is decidable.  The tableau calculus
of~\cite{CompanionPaper} is a sound and complete decision procedure
running in \EXPTIME.
\end{abstract}
...
$R \in \comp(S, R)$ fails for exactly two pairs:
\[
  \PPI \notin \comp(\DR, \PPI) = \{\DR\},
  \qquad
  \PP \notin \comp(\DR, \PP) = \{\DR\}.
\]
These are duals under the inverse map: $\inv(\DR) = \DR$,
$\inv(\PPI) = \PP$.  For all other $(S, R)$ pairs, $R \in \comp(S,R)$.
\end{lemma}
...
\smallskip\noindent
The two failures are in the $S = \DR$ row:
$\comp(\DR, \PP) = \{\DR\}$ (row~$\PP$, column~$\DR$) and
$\comp(\DR, \PPI) = \{\DR\}$ (row~$\PPI$, column~$\DR$).
\end{proof}
...
\begin{theorem}[Path-consistency]
\label{thm:path-consistent}
The disjunctive constraint network $\mathcal{N}$ defined in
...
\begin{theorem}[Soundness: open branch implies satisfiability]
\label{thm:soundness}
...
\label{thm:main}
Concept satisfiability in $\ALCIRCC{5}$ is decidable.  The tableau
...
\begin{proof}
\begin{itemize}[nosep]
  \item \textbf{Termination}: Theorem~\ref{thm:term-complete}
    (from~\cite{CompanionPaper}).
  \item \textbf{Completeness} (satisfiable $\Rightarrow$ open branch):
    Theorem~\ref{thm:term-complete} (from~\cite{CompanionPaper}).
  \item \textbf{Soundness} (open branch $\Rightarrow$ satisfiable):
    Theorem~\ref{thm:soundness} (this paper).
\end{itemize}
The \EXPTIME{} bound follows from the $2^{O(|C_0|)}$ bound on the
number of nodes (Theorem~\ref{thm:term-complete}) and the polynomial
cost of each rule application (including (CF) checking via path
consistency in $O(|V|^3)$).
\end{proof}

\begin{corollary}
\label{cor:complexity}
$\PSPACE \leq \ALCIRCC{5} \leq \EXPTIME$.
\end{corollary}

\begin{proof}
The \PSPACE{} lower bound is from Wessel~\cite{Wessel2003}.  The
\EXPTIME{} upper bound is from the tableau procedure.
\end{proof}
...
The proof of Theorem~\ref{thm:path-consistent} (path-consistency of
the constraint network) is the most technical part of this paper.
Case~A is fully rigorous: for triples of non-adjacent elements, the
completion graph's (CF) condition provides explicit witnesses.  Cases~B
and~C, involving parent--child (tree) edges, require a more delicate
argument connecting the tree structure of the unraveling to the
composition constraints in the completion graph.  The argument relies
on the observation that the tree edges correspond to specific
tableau edges, and the (CF) condition on the completion graph
constrains how these edges interact with the non-tree edges.

We believe the argument is correct, but the formalization of Cases~B
and~C could benefit from a more explicit construction showing how the
(CF) witnesses in the completion graph translate to path-consistency
witnesses in the constraint network.  We invite scrutiny of this step.
